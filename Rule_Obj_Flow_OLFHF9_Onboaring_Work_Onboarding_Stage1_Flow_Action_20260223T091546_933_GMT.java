...
import java.util.*;
public class Rule_Obj_Flow_OLFHF9_Onboaring_Work_Onboarding_Stage1_Flow_Action_20260223T091546_933_GMT  extends com.pega.pegarules.priv.AbstractFUASupport implements Activity, FUASupport {
	private static final LogHelper  oLog = new LogHelper("Rule_Obj_Flow.Stage1_Flow.OLFHF9_Onboaring_Work_Onboarding.Action");
	public Rule_Obj_Flow_OLFHF9_Onboaring_Work_Onboarding_Stage1_Flow_Action_20260223T091546_933_GMT(PegaAPI aContext) {
		tools = aContext;
		pega = aContext;
	}
...
public void perform() {
		...
		mRuleSet = "Onboaring";
		mFlowHandle = "RULE-OBJ-FLOW OLFHF9-ONBOARING-WORK-ONBOARDING STAGE1_FLOW #20260223T091546.933 GMT";
		
		flowType = "Stage1_Flow";
		...
		if (GetFlowInfo != null && GetFlowInfo.equals("true"))
		{
			...
			tools.putParamValue("FlowInsName", "OLFHF9-ONBOARING-WORK-ONBOARDING!STAGE1_FLOW");
			return;
		}
		...
		if (strCheckFlowSecurity != null && strCheckFlowSecurity.equals("true"))
		{
			...
			tools.putParamValue("pyLabel", "Stage 1");
			return;
		}
		...
		if (IsConnectorFlowAction != null && IsConnectorFlowAction.equals("true"))
		{
			...
			if (strTaskName.equals("Assignment1"))
				bIsConnector = IsConnectorFlowAction_Assignment1_circum0(strAction);
		...
		if (pyLastFlowStep.length() == 0 && pyNextFlowStep.length() == 0)
		{
			...
			if (pyCurrentFlowStep.equals(""))
			{
				...
				currentFlow.putString(".pxFlowInsKey", mFlowHandle);
				currentFlow.putString(".pyFlowTypeLabel", "Stage 1");
...
// Expression: 
		...
		else if (pyLastFlowStep.length() > 0 && pyNextFlowStep.length() == 0)
		{
			...
			if (pyLastFlowStep.equals("Assignment1"))
				TaskConnectors_Assignment1_circum0();
		...
		while (!nextFlowStepToRun.equals(""))
		{
			...
			if (nextFlowStepToRun.equals("END52"))
				Task_END52_circum0();
			else if (nextFlowStepToRun.equals("Assignment1"))
				Task_Assignment1_circum0();
		...
		if (!strIssuedFromStage.equals("")){
			...
			ParamsPg.putString("CurrentProcess", "Stage1_Flow");
...
return "Action";
	...
	private boolean[] TaskLocalActions_Assignment1_circum0(ClipboardPage assignPage, boolean isFirstAction, boolean isFirstLocalAction)
	{
	...
	private void Task_Assignment1_circum0()
	{
		String taskName = "Assignment1";
		lastFlowStepRun = nextFlowStepToRun;
		nextFlowStepToRun = ""; // reset it
		bCalledNonExistentRule = false; // reset it
		currentFlow.putString("pyCurrentFlowStep", "Assignment1");
		currentFlow.putString(".pyCurrentFlowStepLabel", "Collect information");
		...
		// Now add this task's new contexts 
		ClipboardProperty cpContext;
		ParameterPage newParamPage = createNewParamsPage(taskName);
		HashStringMap keys = new HashStringMap();
		keys.putString("pyActivityName", "UpdateStatus");
// Expression: Pending-information
		try
		{
		newParamPage.putString("StatusWork", "Pending-information");
		}
		...
		tools.doActivity(keys, workPage, newParamPage);
		...
		else if (!RunTaskActivity(taskName, "ToCurrentOperator" , workPage, newParamsPage))
			return;
		...
		else
		{
			...
			if (passThrough.equals("true"))
			{
				...
				{
					actionToTake = "CollectInformation";
				}
		...
		newParamsPage.putString("TaskLabel","Collect information");
...
// Expression: 
		...
		try
		{
// Expression: Collect information
			newParamsPage.putString("Instructions", "Collect information");
		}
		catch (java.lang.IllegalArgumentException e)
		{
			if (!bDraftMode)
...
// Expression: pyStepRoutedConfirmation
		...
		pg_newAssignPage.putString("pxProcessName", "Stage 1");
		...
		if (currentFlow != null)
		{
			...
			currentFlow.putString(".pyLastFlowStepLabel", "Collect information");
	...
	private boolean IsConnectorFlowAction_Assignment1_circum0(String strAction)
	{
		if (strAction.equals("CollectInformation"))
			return true;
		return false;
	}

	// TaskFlowActions returns false if none of the actions could be added to the assignment
	private boolean TaskFlowActions_Assignment1_circum0(ClipboardPage assignPage)
	{
		boolean isFirstAction = true;
		if (AddFlowAction("CollectInformation", "FlowAction", isFirstAction, false))
			isFirstAction = false;
				assignPage.getProperty(".pyAssignActions(<LAST>).pyToShape").setValue("END52" );
				assignPage.getProperty(".pyAssignActions(<LAST>).pyStepName").setValue("Collect information" );
				if(assignPage.getString(".pyAssignActions(<LAST>).pyActionLabel").equals("")){
					assignPage.getProperty(".pyAssignActions(<LAST>).pyActionLabel").setValue("Collect information" );
				}
		return !isFirstAction;
	}

	private void TaskConnectors_Assignment1_circum0()
	{
		...
		if (bCalledNonExistentRule == true)
		{
			...
			DraftModeFlow("DraftModeStep", "Assignment1", "END52", "(name not specified)");
			return;
		}
		...
		strConnectorValue = "CollectInformation";
	...
	private void TaskConnectors_Start1_circum0()
	{
		...
		{
			// Send the user to a special assignment to pick the connector
			DraftModeFlow("DraftModeStep", "Start1", "Assignment1", "Collect information");
			return;
		}
		...
		{
			strConnectorType = "STATUS";
			strConnectorValue = "[DRAFTMODE]Assignment1";
		}
		else
		{
	...
	private void PopulateFlowActions_circum0(String pyLastFlowStep, String strAssignmentPageName)
	{
		...
		if (pyLastFlowStep.equals("Assignment1"))
		{
			if (TaskFlowActions_Assignment1_circum0(pg_newAssignPage))
				isFirstAction = false;
			else
			...
			boolean[] retParams;
			retParams = TaskLocalActions_Assignment1_circum0(pg_newAssignPage, isFirstAction, isFirstLocalAction);
	...
	private void GetOrderedFlowSteps_circum0(String stepPageName) {
		...
		pgStep.putString(".pyMOId", "Assignment1");
			pgStep.putString(".pyMOName", "Collect information");
			pgStep.putString(".pyImplementation", "WorkList");
		pgStep.putString(".pyCoordX", "2.08");
		pgStep.putString(".pyCoordY", "0.096");
		...
		pgStep.putString(".pyMOId", "END52");
			pgStep.putString(".pyMOName", "");
			pgStep.putString(".pyImplementation", "");
		pgStep.putString(".pyCoordX", "4.08");
		pgStep.putString(".pyCoordY", "0.096");
	...
	private void GetNavigationPath_circum0(String pageName, String parentFlowPath, boolean deferredLoad)
	{
		...
		pgNavPath.putString("pyFlowType", "Stage1_Flow");
		pgNavPath.putString("pyClassName", "OLFHF9-Onboaring-Work-Onboarding");
		pgNavPath.putString("pyLabel", "Stage 1");
		...
		toShapeValue = (String)hashShapes.get("Assignment1");
		...
		hashShapes.put("Assignment1", newHashShapeValue);
	
		// *****************************************************************************
		// Examining step "Assignment1"
		...
		pgStep.putString("pyFlowStep", "Assignment1");
		pgStep.putString("pyStepLabel", "Collect information");
		pgStep.putString("pyStepType", "ASSIGNMENT");
		bIsInFlowPath = pega_procom_breadcrumbtrail.isInFlowPath2(tools, "Assignment1", "Stage1_Flow", parentFlowPath, flowNameToSearchForTaskIn, interestPage.getTopLevelPage());
		if (bIsInFlowPath)
		{
			if (!bFoundFlowPath)
			{
			...
			if (newAssignPage.getString("pxTaskName").equals("Assignment1") && newAssignPage.getString("pyFlowType").equals("Stage1_Flow") && newAssignPage.getString("pyParentFlowPath").equals(parentFlowPath))
		...
		// First we check if the connector's origin shape is valid
		fromShapeValue = (String)hashShapes.get("Assignment1");
		...
		if (((String)hashShapes.get("END52")).equals("true")) {
			...
			feedingShapes.addElement("Assignment1");
	...
	private boolean CheckFlowSecurity_circum0()
	{
		...
		if (!hasSecurity && tools.getAuthorizationHandle().havePrivilege(tools, "AM:RULE-OBJ-FLOW:OLFHF9-ONBOARING-WORK-ONBOARDING!STAGE1_FLOW",primaryPageClassName, interestPage)) 
			 hasSecurity = true;
			 if(!hasSecurity) {
			 if(tools.getAuthorizationHandle().havePrivilege(tools,  "AM:RULE-OBJ-FLOW:OLFHF9-ONBOARING-WORK-ONBOARDING!STAGE1_FLOW",primaryPageClassName, interestPage,true))
			 isAllowedAccess = true;
			 else return false;
		}
			 if(isAllowedAccess && !hasSecurity) {		com.pega.pegarules.priv.tracer.RuleTraceInfo ruleTraceInfo = new com.pega.pegarules.priv.tracer.RuleTraceInfo                                                                   ("RULE-OBJ-FLOW OLFHF9-ONBOARING-WORK-ONBOARDING STAGE1_FLOW #20260223T091546.933 GMT","Stage1_Flow","Onboaring","","20260223T091824.843 GMT");
...
private static final com.pega.pegarules.priv.tracer.RuleTraceInfo oTraceInfo_0 = new com.pega.pegarules.priv.tracer.RuleTraceInfo("RULE-OBJ-FLOW OLFHF9-ONBOARING-WORK-ONBOARDING STAGE1_FLOW #20260223T091546.933 GMT", "OLFHF9-Onboaring-Work-Onboarding Stage1_Flow", "Onboaring", "01-01-01", "20260223T091824.843 GMT");
}
