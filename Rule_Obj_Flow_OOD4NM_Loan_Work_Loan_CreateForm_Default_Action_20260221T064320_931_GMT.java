...
import java.util.*;
public class Rule_Obj_Flow_OOD4NM_Loan_Work_Loan_CreateForm_Default_Action_20260221T064320_931_GMT  extends com.pega.pegarules.priv.AbstractFUASupport implements Activity, FUASupport {
	private static final LogHelper  oLog = new LogHelper("Rule_Obj_Flow.CreateForm_Default.OOD4NM_Loan_Work_Loan.Action");
	public Rule_Obj_Flow_OOD4NM_Loan_Work_Loan_CreateForm_Default_Action_20260221T064320_931_GMT(PegaAPI aContext) {
		tools = aContext;
		pega = aContext;
	}
...
public void perform() {
		...
		mRuleSet = "Loan";
		mFlowHandle = "RULE-OBJ-FLOW OOD4NM-LOAN-WORK-LOAN CREATEFORM_DEFAULT #20260221T064320.931 GMT";
...
tools.getStepStatus().backoutWorst();
		...
		{
			tools.putParamValue("FlowHandle", mFlowHandle);
			tools.putParamValue("FlowInsName", "OOD4NM-LOAN-WORK-LOAN!CREATEFORM_DEFAULT");
			return;
		}
		...
		if (strCheckFlowSecurity != null && strCheckFlowSecurity.equals("true"))
		{
			...
			tools.putParamValue("pyLabel", "GetUserDetails");
			return;
		}
		...
		if (pyLastFlowStep.length() == 0 && pyNextFlowStep.length() == 0)
		{
			...
			if (pyCurrentFlowStep.equals(""))
			{
				...
				currentFlow.putString(".pxFlowInsKey", mFlowHandle);
				currentFlow.putString(".pyFlowTypeLabel", "GetUserDetails");
...
// Expression: 
		...
		else if (pyLastFlowStep.length() > 0 && pyNextFlowStep.length() == 0)
		{
			...
			if (pyLastFlowStep.equals("Start62"))
...
// Expression: 
	...
	private void TaskConnectors_Start62_circum0()
	{
		...
		if (bCalledNonExistentRule == true)
		{
			...
			DraftModeFlow("DraftModeStep", "Start62", "AssignmentSF1", "User Details");
			return;
		}
	...
	private void Task_AssignmentSF1_circum0()
	{
		...
		currentFlow.putString(".pyCurrentFlowStepLabel", "User Details");
		...
		newParamsPage.putString("TaskLabel","User Details");
...
// Expression: USE_CASE_POLICY
		...
		pg_newAssignPage.putString("pxProcessName", "GetUserDetails");
		...
		if (currentFlow != null)
		{
			...
			currentFlow.putString(".pyLastFlowStepLabel", "User Details");
	...
	private boolean TaskFlowActions_AssignmentSF1_circum0(ClipboardPage assignPage)
	{
		...
		if (AddFlowAction("Create", "FlowAction", isFirstAction, false))
			isFirstAction = false;
				...
				assignPage.getProperty(".pyAssignActions(<LAST>).pyStepName").setValue("User Details" );
				if(assignPage.getString(".pyAssignActions(<LAST>).pyActionLabel").equals("")){
					assignPage.getProperty(".pyAssignActions(<LAST>).pyActionLabel").setValue("User Details" );
				}
	...
	private void GetOrderedFlowSteps_circum0(String stepPageName) {
		...
		pgStep.putString(".pyMOId", "AssignmentSF1");
			pgStep.putString(".pyMOName", "User Details");
			pgStep.putString(".pyImplementation", "WorkList");
	...
	private void GetNavigationPath_circum0(String pageName, String parentFlowPath, boolean deferredLoad)
	{
		...
		pgNavPath.putString("pyClassName", "OOD4NM-Loan-Work-Loan");
		pgNavPath.putString("pyLabel", "GetUserDetails");
		...
		pgStep.putString("pyStepLabel", "User Details");
	...
	private boolean CheckFlowSecurity_circum0()
	{
		boolean hasSecurity = false;
		 ...
		 primaryPageClassName = "OOD4NM-Loan-Work-Loan";
		 }
		...
		if (!hasSecurity && tools.getAuthorizationHandle().havePrivilege(tools, "AM:RULE-OBJ-FLOW:OOD4NM-LOAN-WORK-LOAN!CREATEFORM_DEFAULT",primaryPageClassName, interestPage)) 
			 hasSecurity = true;
			 if(!hasSecurity) {
			 if(tools.getAuthorizationHandle().havePrivilege(tools,  "AM:RULE-OBJ-FLOW:OOD4NM-LOAN-WORK-LOAN!CREATEFORM_DEFAULT",primaryPageClassName, interestPage,true))
			 isAllowedAccess = true;
			 else return false;
		}
			 if(isAllowedAccess && !hasSecurity) {		com.pega.pegarules.priv.tracer.RuleTraceInfo ruleTraceInfo = new com.pega.pegarules.priv.tracer.RuleTraceInfo                                                                   ("RULE-OBJ-FLOW OOD4NM-LOAN-WORK-LOAN CREATEFORM_DEFAULT #20260221T064320.931 GMT","CreateForm_Default","Loan","","20260221T064322.892 GMT");
...
private static final com.pega.pegarules.priv.tracer.RuleTraceInfo oTraceInfo_0 = new com.pega.pegarules.priv.tracer.RuleTraceInfo("RULE-OBJ-FLOW OOD4NM-LOAN-WORK-LOAN CREATEFORM_DEFAULT #20260221T064320.931 GMT", "OOD4NM-Loan-Work-Loan CreateForm_Default", "Loan", "01-01-01", "20260221T064322.892 GMT");
}
