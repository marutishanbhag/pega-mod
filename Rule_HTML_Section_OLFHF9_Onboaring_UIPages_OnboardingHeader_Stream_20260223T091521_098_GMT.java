 ...
 * Builds JSP stream .
 */
public class Rule_HTML_Section_OLFHF9_Onboaring_UIPages_OnboardingHeader_Stream_20260223T091521_098_GMT extends StreamBuilderSection implements StreamBuilder, FUASupport, FUAReusable, WebAllowable {
	private static final LogHelper  oLog = new LogHelper("Rule_HTML_Section.OnboardingHeader.OLFHF9_Onboaring_UIPages.Stream");
private static Map<String,String[]> oCellRuntimeParamsMap = new HashMap<String,String[]>();
private static Map<String, String> oControlPathsMap = new HashMap<String, String>();
private static Map<String, AssemblyParameters> oAssemblyParamsMap = null;
private static final int pzChecksum = -2108050329;
 ...
 */
	public Rule_HTML_Section_OLFHF9_Onboaring_UIPages_OnboardingHeader_Stream_20260223T091521_098_GMT(com.pega.pegarules.priv.PegaAPI aContext) {
...
private static String pxUniqueStreamHash = "8e84670a2bf8816e926ef8a2e7d3f45a6062077c";
...
try {
pxUniqueStreamHash = "8e84670a2bf8816e926ef8a2e7d3f45a6062077c";
...
//	RULE-UTILITY-FUNCTION UTILITIES VALIDATERULEEXISTENCE2 #20180713T131446.070 GMT:20180713T131446.070 GMT
	...
	public static String getFUAContentHash() {
		return "314b565dde23ad18cd0137f7f4a3eab1";
	}
...
config.put("className", "OLFHF9-Onboaring-Work-Onboarding");
...
tools.appendString("<div class='layout layout-noheader layout-noheader-mobile_header'>");
...
boolean bDisabled = false;boolean bOptimizedMarkup = pzAuto.getUIComponentRuntime().isHarnessTemplatized() && pzAuto.isOffline();
	 ...
	 String securedPropValue = null;
		...
		tools.appendFormatted("OLFHF9-Onboaring-Work-Onboarding",StreamBuilder.FMT_NORMAL);		tools.appendString("\",\"\",\"");
...
public void simpleLayout_2() {
	...
	if(currentTemplatingStatus != 'Y'){
		...
		tools.appendString("" + pzAuto.getDataUIMetaAttribute("{'type':'Layout','subType':'DYNAMICLAYOUT','pgRef':'HeaderTemplate.pySections(2)'}") + " class='" + "flex  content  layout-content-inline_middle content-inline_middle " + "'  ");
	...
	metadataPage.put("prefix","-mobile_header");
	...
	boolean pyVisibility = false;pyVisibility = runtimeContextTree.beginVisibleWhen(" wxpyIsMobilePhone", pxUniqueStreamHash+"_17");metadataPage.put("lMode","Flexbox");
...
try {
	...
	String pyLabel = "";
	if(pega.getPackageRuntime().canPackage()){
		tools.getLocalizedTextForString(".pyButtonLabel","New\t"+repeatIndx,StreamBuilder.FMT_LITERAL);
	} else {
		pzAuto.getUIComponentRuntime().getRuntimeContextTree().addLocalizedValue("New", ".pyButtonLabel");
	}
	pyLabel = "New";
	modePage2.put("label",pyLabel);
...
boolean bDisabled = false;boolean bOptimizedMarkup = pzAuto.getUIComponentRuntime().isHarnessTemplatized() && pzAuto.isOffline();
	 ...
	 String securedPropValue = null;
		...
		tools.appendString("<i aria-hidden='true'  data-click='.' class='" + classProp + "'></i>");
		boolean hasLineBreaks = false;
		 hasLineBreaks ="New".matches("(.*)<([Bb][Rr])\\s*\\/*\\s*>(.*)");
		if(hasLineBreaks) {
		tools.appendString("<span class='pzbtn-label'  data-click='.' ");}if(bOptimizedMarkup && hasLineBreaks){
		tools.appendString("  data-bindprops='title,innerHTML' ");}
		if(hasLineBreaks) {
		tools.appendString("  > "); }
		tools.appendString("");pega_uiengine_harness.pzGenAKey(tools.getLocalizedTextForString(".pyButtonLabel","New",StreamBuilder.FMT_LITERAL)); checkForScriptTags_1(tools.getParamValue("pzULabel")); 
		tools.appendString("</button>");	}
...
public int simpleLayoutCell_1( int index) {
	...
	String baseRef ="";
	generateClientWhenDiv_1();
	tools.appendString("' " + pega.getUIEngine().getFVInspectorUtils().startCollectingFieldValueData() + pzCell.getInspectorDataDynamic("Label",null,"HeaderTemplate.pySections(1).pySectionBody(1).pyTable.pyRows(1).pyCells(1)") + " class='content-item content-label item-" + Integer.toString(index) + "   ");
...
public int simpleLayoutCell_2( int index) {
	...
	String baseRef ="";
	generateClientWhenDiv_1();
	tools.appendString("' " + pega.getUIEngine().getFVInspectorUtils().startCollectingFieldValueData() + pzCell.getInspectorDataDynamic("pxButton",".pyTemplateInputBox","HeaderTemplate.pySections(1).pySectionBody(1).pyTable.pyRows(1).pyCells(2)") + " class='content-item content-field item-" + Integer.toString(index) + "    " + pzSection.getCustomStyle(false, "","flex-grow-1 right-aligned","flex-grow-1 right-aligned")  + " " +  pzSection.getCustomStyle(false, "","dataValueRead","dataValueWrite")  + " flex flex-row '  STRING_TYPE='field' RESERVE_SPACE='false'>");
	pzAuto.getUIComponentRuntime().insertComponentPlaceholderPage();
...
public void simpleLayout_1() {
	...
	if(currentTemplatingStatus != 'Y'){
		...
		tools.appendString("" + pzAuto.getDataUIMetaAttribute("{'type':'Layout','subType':'DYNAMICLAYOUT','pgRef':'HeaderTemplate.pySections(1)'}") + " class='" + "flex  content  layout-content-inline_middle content-inline_middle " + "'  ");
		if(!tools.getParamValue("pyPegaDesignMode").equals("true")){
		tools.appendString(" ");
		}
		tools.appendString(" " + refreshAttributes + ">");
	...
	IUIComponent containerComponent = pzAuto.getUIComponent();
	com.pega.pegarules.priv.runtime.ui.IRuntimeContextTree runtimeContextTree = pzAuto.getUIComponentRuntime().getRuntimeContextTree();
	boolean pyVisibility = false;pyVisibility = runtimeContextTree.beginVisibleWhen(" o1! wxpyIsMobilePhone", pxUniqueStreamHash+"_9");metadataPage.put("lMode","Flexbox");
	IUIComponentMetadata pxWhenIdentifiers = pzAuto.getUIComponentRuntime().createMetadata();
...
private static final com.pega.pegarules.priv.tracer.RuleTraceInfo oTraceInfo_0 = new com.pega.pegarules.priv.tracer.RuleTraceInfo("", "OLFHF9-Onboaring-UIPages OnboardingHeader", "Onboaring", "01-01-01", "20180713T135157.914 GMT");
}
