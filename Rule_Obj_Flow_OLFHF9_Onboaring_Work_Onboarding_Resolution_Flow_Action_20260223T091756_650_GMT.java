...
import java.util.*;
public class Rule_Obj_Flow_OLFHF9_Onboaring_Work_Onboarding_Resolution_Flow_Action_20260223T091756_650_GMT  extends com.pega.pegarules.priv.AbstractFUASupport implements Activity, FUASupport {
	private static final LogHelper  oLog = new LogHelper("Rule_Obj_Flow.Resolution_Flow.OLFHF9_Onboaring_Work_Onboarding.Action");
	public Rule_Obj_Flow_OLFHF9_Onboaring_Work_Onboarding_Resolution_Flow_Action_20260223T091756_650_GMT(PegaAPI aContext) {
		tools = aContext;
		pega = aContext;
	}
...
public void perform() {
		...
		mRuleSet = "Onboaring";
		mFlowHandle = "RULE-OBJ-FLOW OLFHF9-ONBOARING-WORK-ONBOARDING RESOLUTION_FLOW #20260223T091756.650 GMT";
		...
		if (GetFlowInfo != null && GetFlowInfo.equals("true"))
		{
			...
			tools.putParamValue("FlowInsName", "OLFHF9-ONBOARING-WORK-ONBOARDING!RESOLUTION_FLOW");
			return;
		}
...
// Expression: 
		...
		else if (pyLastFlowStep.length() > 0 && pyNextFlowStep.length() == 0)
		{
			...
			if (pyLastFlowStep.equals("Utility1"))
		...
		while (!nextFlowStepToRun.equals(""))
		{
			...
			if (nextFlowStepToRun.equals("Utility1"))
...
//	RULE-UTILITY-FUNCTION UTILITIES ISORINHERITSFROM #20180713T131444.415 GMT:20180713T131444.415 GMT
	...
	public static String getFUAContentHash() {
		return "087205787f89625d5d35e7a7c6085d74";
	}
...
return "Action";
	...
	private void Task_Utility1_circum0()
	{
		...
		currentFlow.putString(".pyCurrentFlowStepLabel", "Run data transform");
		String taskActivity = "pzRunDataTransform";
...
// Expression: ResolveCase
			newParamsPage.putString("DataTransformName", "ResolveCase");
		}
		...
		LogTraceEvent(mRuleSet,
				"Rule-Obj-Activity " + interestPage.getClassName() + "." + taskActivity,
				flowName + " - " + taskName,
				"Flow Utility Start",
				"",
				mFlowHandle,
		...
		boolean bRanSuccessfully = RunTaskActivity(taskName, taskActivity, interestPage, newParamsPage);
		
		String strTraceMessage = "Rule-Obj-Activity " + interestPage.getClassName() + "." + taskActivity;
		...
		if (currentFlow != null)
		{
			...
			currentFlow.putString(".pyLastFlowStepLabel", "Run data transform");
			String strTaskLabel = currentFlow.getString(".pyLastFlowStepLabel");
			AddTaskToFlowPath(taskName, "TASK", strTaskLabel, false, "", false);
		}
	...
	private void TaskConnectors_Utility1_circum0()
	{
		...
		if (bCalledNonExistentRule == true)
		{
			...
			DraftModeFlow("DraftModeStep", "Utility1", "END52", "(name not specified)");
			return;
		}
		...
		if (bMoreThanAlwaysInDraftMode == true)
		{
			...
			strConnectorValue = "[DRAFTMODE]END52";
		}
		else
		{
	...
	private void TaskConnectors_Start1_circum0()
	{
		...
		{
			// Send the user to a special assignment to pick the connector
			DraftModeFlow("DraftModeStep", "Start1", "Utility1", "Run data transform");
			return;
		}
		...
		{
			strConnectorType = "STATUS";
			strConnectorValue = "[DRAFTMODE]Utility1";
		}
		else
		{
			strConnectorType = "ALWAYS";
	...
	private void GetOrderedFlowSteps_circum0(String stepPageName) {
		...
		pgStep.putString(".pyMOId", "Utility1");
			pgStep.putString(".pyMOName", "Run data transform");
			pgStep.putString(".pyImplementation", "pzRunDataTransform");
	...
	private boolean CheckFlowSecurity_circum0()
	{
		...
		if (!hasSecurity && tools.getAuthorizationHandle().havePrivilege(tools, "AM:RULE-OBJ-FLOW:OLFHF9-ONBOARING-WORK-ONBOARDING!RESOLUTION_FLOW",primaryPageClassName, interestPage)) 
			 hasSecurity = true;
			 if(!hasSecurity) {
			 if(tools.getAuthorizationHandle().havePrivilege(tools,  "AM:RULE-OBJ-FLOW:OLFHF9-ONBOARING-WORK-ONBOARDING!RESOLUTION_FLOW",primaryPageClassName, interestPage,true))
			 isAllowedAccess = true;
			 else return false;
		}
			 if(isAllowedAccess && !hasSecurity) {		com.pega.pegarules.priv.tracer.RuleTraceInfo ruleTraceInfo = new com.pega.pegarules.priv.tracer.RuleTraceInfo                                                                   ("RULE-OBJ-FLOW OLFHF9-ONBOARING-WORK-ONBOARDING RESOLUTION_FLOW #20260223T091756.650 GMT","Resolution_Flow","Onboaring","","20260223T091825.135 GMT");
