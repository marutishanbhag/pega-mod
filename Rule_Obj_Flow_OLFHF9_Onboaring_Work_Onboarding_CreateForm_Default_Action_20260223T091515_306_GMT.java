...
import java.util.*;
public class Rule_Obj_Flow_OLFHF9_Onboaring_Work_Onboarding_CreateForm_Default_Action_20260223T091515_306_GMT  extends com.pega.pegarules.priv.AbstractFUASupport implements Activity, FUASupport {
	private static final LogHelper  oLog = new LogHelper("Rule_Obj_Flow.CreateForm_Default.OLFHF9_Onboaring_Work_Onboarding.Action");
	public Rule_Obj_Flow_OLFHF9_Onboaring_Work_Onboarding_CreateForm_Default_Action_20260223T091515_306_GMT(PegaAPI aContext) {
		tools = aContext;
		pega = aContext;
	}
...
public void perform() {
		...
		mRuleSet = "Onboaring";
		mFlowHandle = "RULE-OBJ-FLOW OLFHF9-ONBOARING-WORK-ONBOARDING CREATEFORM_DEFAULT #20260223T091515.306 GMT";
		
		flowType = "CreateForm_Default";
...
tools.getStepStatus().backoutWorst();
		...
		{
			tools.putParamValue("FlowHandle", mFlowHandle);
			tools.putParamValue("FlowInsName", "OLFHF9-ONBOARING-WORK-ONBOARDING!CREATEFORM_DEFAULT");
			return;
		}
		...
		{
			tools.putParamValue("StructureType", "");
			return;
		}
		...
		if (strCheckFlowSecurity != null && strCheckFlowSecurity.equals("true"))
		{
			...
			tools.putParamValue("pyLabel", "Create");
			return;
		}
		...
		if (IsConnectorFlowAction != null && IsConnectorFlowAction.equals("true"))
		{
			...
			if (strTaskName.equals("AssignmentSF1"))
				bIsConnector = IsConnectorFlowAction_AssignmentSF1_circum0(strAction);
		...
		else if (pyNextFlowStep.length() > 0)
		{
			...
			if (pyLastFlowStep.length() == 0)
			{
				...
				currentFlow.putString(".pyCategory", "ScreenFlow");
				currentFlow.putString(".pyScreenFlowTemplate", "TabbedScreenFlow7");
		...
		if (currentFlow.getString(".pxLastActivityStatus").equals("ActionStubDraftMode"))
			currentFlow.putString(".pxLastActivityStatus", tools.findPage("pxThread").getString(".pxTaskStatusInfo"));
		...
		if (pyLastFlowStep.length() == 0 && pyNextFlowStep.length() == 0)
		{
			...
			if (pyCurrentFlowStep.equals(""))
			{
				...
				currentFlow.putString(".pxFlowInsKey", mFlowHandle);
				currentFlow.putString(".pyFlowTypeLabel", "Create");
...
// Expression: 
		...
		else if (pyLastFlowStep.length() > 0 && pyNextFlowStep.length() == 0)
		{
			...
			if (pyLastFlowStep.equals("AssignmentSF1"))
			...
			if (currentFlow.getString(".pxLastActivityStatus").startsWith("[DRAFTMODE]"))
			{
				currentFlow.putString(".pxLastActivityStatus", "");
				tools.findPage("pxThread").putString("pxTaskStatus", "");
				tools.findPage("pxThread").putString("pxTaskStatusInfo", "");
			}
		...
		while (!nextFlowStepToRun.equals(""))
		{
			...
			if (nextFlowStepToRun.equals("END66"))
				Task_END66_circum0();
		...
		if (!strIssuedFromStage.equals("")){
			...
			ParamsPg.putString("CurrentProcess", "CreateForm_Default");
...
private static final String oDependencies [] = {
	...
	private static final DependentRuleInfo[] oDependentRuleInfos = {
		new DependentRuleInfo("DOFUASSEMBLY","Rule-Obj-Activity","",true,"Rule-Obj-Flow","","","","!DOFUASSEMBLY",true,false,"ABSOLUTE_IS_NOT_PRIMARY",1), 
...
//	RULE-UTILITY-FUNCTION PEGARULESUTILITIES CREATEREFERENCESFORPARAMETERS--(9CE1EB0AC70676F8A514EA00342440FC #20180713T131441.407 GMT:20180713T131441.407 GMT
...
//	RULE-UTILITY-FUNCTION UTILITIES ISORINHERITSFROM #20180713T131444.415 GMT:20180713T131444.415 GMT
	...
	public static String getFUAContentHash() {
		return "053296031398e19883c5c533b80ae66f";
	}

	public String getDefinitionAppliesToClass() {
		return "OLFHF9-Onboaring-Work-Onboarding";
	}
public String getAspect() {
return "Action";
}
	private void Task_END66_circum0()
	{
		String taskName = "END66";
		lastFlowStepRun = nextFlowStepToRun;
		nextFlowStepToRun = ""; // reset it
		bCalledNonExistentRule = false; // reset it
		currentFlow.putString("pyCurrentFlowStep", "END66");
	...
	private boolean[] TaskLocalActions_AssignmentSF1_circum0(ClipboardPage assignPage, boolean isFirstAction, boolean isFirstLocalAction)
	{
	...
	private void Task_AssignmentSF1_circum0()
	{
		String taskName = "AssignmentSF1";
		lastFlowStepRun = nextFlowStepToRun;
		nextFlowStepToRun = ""; // reset it
		bCalledNonExistentRule = false; // reset it
		currentFlow.putString("pyCurrentFlowStep", "AssignmentSF1");
		currentFlow.putString(".pyCurrentFlowStepLabel", "Create");
		String taskActivity = "WorkList";
		...
		String defaultAssignTo = pyFlowParametersPage.getString("DefaultAssignTo");
		if (defaultAssignTo != null && defaultAssignTo.length() > 0)
			newParamsPage.putString("AssignTo", defaultAssignTo);
		else{
				if (!RunTaskActivity(taskName, "ToCurrentOperator", workPage, newParamsPage))
			return;
		}
		...
		newParamsPage.putString("TaskLabel","Create");
		
		try
		{
// Expression: 
			newParamsPage.putString("HarnessPurpose", "");
		}
		...
		{
// Expression: 
			newParamsPage.putString("StatusWork", "");
		}
		catch (java.lang.IllegalArgumentException e)
		{
			if (!bDraftMode)
		...
		{
// Expression: 
			newParamsPage.putString("DoNotPerform", "");
		}
		catch (java.lang.IllegalArgumentException e)
		{
			if (!bDraftMode)
		...
		{
// Expression: 
			newParamsPage.putString("StatusAssign", "");
		}
		catch (java.lang.IllegalArgumentException e)
		{
			if (!bDraftMode)
		...
		{
// Expression: 
			newParamsPage.putString("Instructions", "");
		}
		catch (java.lang.IllegalArgumentException e)
		{
			if (!bDraftMode)
		...
		try
		{
// Expression: pyStepRoutedConfirmation
			newParamsPage.putString("ConfirmationNote", "pyStepRoutedConfirmation");
		}
		catch (java.lang.IllegalArgumentException e)
		{
			if (!bDraftMode)
		...
		try
		{
// Expression: USE_CASE_POLICY
			newParamsPage.putString("ViewPolicy", "USE_CASE_POLICY");
		}
		catch (java.lang.IllegalArgumentException e)
		{
			if (!bDraftMode)
		...
		pg_newAssignPage.getProperty(".pxSystemFlow").setValue(false);
		pg_newAssignPage.putString(".pxFlowInsKey", mFlowHandle);
		
		pg_newAssignPage.putString("pxProcessName", "Create");
		pg_newAssignPage.getProperty("pxIsMultiStep").setValue(true);
		...
		if (currentFlow != null)
		{
			...
			currentFlow.putString(".pyLastFlowStepLabel", "Create");
			String strTaskLabel = currentFlow.getString(".pyLastFlowStepLabel");
			AddTaskToFlowPath(taskName, "ASSIGNMENT", strTaskLabel, true, "", false);
		}
		...
		if (!isTemporaryObject)
		{
			// Add history for this assignment
			String strRouteTo = currentFlow.getString(".pxRouteToUserName");
			if (strRouteTo.equals(""))
				strRouteTo = currentFlow.getString(".pxRouteTo");
			String histMemo = "AssignmentRouted\t"
					+ strRouteTo
					+ "\t"
					+ currentFlow.getString(".pyInstructions");
			pega_procom_flow.AddHistory(tools, workPage, 'A', pg_newAssignPage,histMemo);
		}
		
				//The send notification option is set to USE CASE TYPE POLICY on this assignment. Call the case type to get the notification settings
			ParameterPage notificationParams = new ParameterPage();
			pega_rules_utilities.pxCallCaseType(tools.getStepPage(), "GETCASENOTIFICATIONS", notificationParams);
			String isEmailNotificationEnabled = notificationParams.getString("EmailNotificationIsEnabled");
			String isPushNotificationEnabled = notificationParams.getString("PushNotificationIsEnabled"); 
			if(isEmailNotificationEnabled != null){
				if(isEmailNotificationEnabled.equals("true")||isPushNotificationEnabled.equals("true") ){
					//Check if the current user is not same as the assignee
					String strCurrentUser = tools.findPage("pxRequestor").getString(".pyUserIdentifier");
					String strAssignee = tools.findPage("newAssignPage").getString(".pxAssignedOperatorID");
					if(!strCurrentUser.equals(strAssignee)){
						StringMap notificationKeys = new HashStringMap(); 
						notificationKeys.putString("pxObjClass","Rule-Obj-Activity");
						notificationKeys.putString("pyClassName","Work-");
						notificationKeys.putString("pyActivityName","pzCaseNotification");
						try{
							tools.doActivity(notificationKeys,tools.getStepPage(),notificationParams);
						}
						catch(com.pega.pegarules.pub.generator.RuleNotFoundException ex){
							tools.getStepStatus().backoutWorst();
						}
	...
	private boolean IsConnectorFlowAction_AssignmentSF1_circum0(String strAction)
	{
	...
	private boolean TaskFlowActions_AssignmentSF1_circum0(ClipboardPage assignPage)
	{
		boolean isFirstAction = true;
		if (AddFlowAction("Create", "FlowAction", isFirstAction, false))
			isFirstAction = false;
				assignPage.getProperty(".pyAssignActions(<LAST>).pyToShape").setValue("END66" );
				assignPage.getProperty(".pyAssignActions(<LAST>).pyStepName").setValue("Create" );
				if(assignPage.getString(".pyAssignActions(<LAST>).pyActionLabel").equals("")){
					assignPage.getProperty(".pyAssignActions(<LAST>).pyActionLabel").setValue("Create" );
				}
		return !isFirstAction;
	}

	private void TaskConnectors_AssignmentSF1_circum0()
	{
		...
		if (bCalledNonExistentRule == true)
		{
			// Send the user to a special assignment to pick the connector
			DraftModeFlow("DraftModeStep", "AssignmentSF1", "END66", "(name not specified)");
			return;
		}
...
// Expression: 
		...
		// Flow Route Start tracer message
		LogTraceEvent(mRuleSet,
				"Rule-Obj-Activity " + workPage.getClassName() + ".ToCurrentOperator",
				flowName + " - " + taskName,
				"Flow Route Start",
				"",
				mFlowHandle,
...
// Expression: 
		...
		if (pg_tempAssignPage.getString(".pxAssignedOperatorID").length() > 0)
			pyFlowParametersPage.putString("DefaultAssignTo", newParamsPage.getString("AssignTo"));
		else if (newParamsPage.getString("AssignTo").length() > 0){
			pyFlowParametersPage.putString("DefaultAssignTo", newParamsPage.getString("AssignTo"));
			if(newParamsPage.isDefined("SwitchToWorkbasket"))
				pyFlowParametersPage.putString("SwitchToWorkbasket", newParamsPage.getString("SwitchToWorkbasket"));
		}else
		...
		LogTraceEvent(mRuleSet,
				...
				flowName + " - " + taskName,
				"Flow Route End",
				pyFlowParametersPage.getString("DefaultAssignTo"),
				mFlowHandle,
				taskName);
	...
	private void TaskConnectors_Start62_circum0()
	{
		...
		{
			// Send the user to a special assignment to pick the connector
			DraftModeFlow("DraftModeStep", "Start62", "AssignmentSF1", "Create");
			return;
		}
		
		
		// Evaluating the connectors (in order of likelihood)
		if (bMoreThanAlwaysInDraftMode == true)
		{
			strConnectorType = "STATUS";
			strConnectorValue = "[DRAFTMODE]AssignmentSF1";
		}
		else
		{
	...
	private void PopulateFlowActions_circum0(String pyLastFlowStep, String strAssignmentPageName)
	{
		...
		if (pyLastFlowStep.equals("AssignmentSF1"))
		{
			if (TaskFlowActions_AssignmentSF1_circum0(pg_newAssignPage))
				isFirstAction = false;
			else
			...
			boolean[] retParams;
			retParams = TaskLocalActions_AssignmentSF1_circum0(pg_newAssignPage, isFirstAction, isFirstLocalAction);
			...
			try
			{
				String formName = pega_processengine_workutilities.pzGetViewPurpose(tools.getPrimaryPage());
	...
	private void GetCoordinates_circum0(String pageName)
	{
		...
		pgFlow.addMessage("Flow_InDraftMode");
		return;
	}
	...
	private void GetOrderedFlowSteps_circum0(String stepPageName) {
		...
		pgStep = tools.createPage("Data-MO-Event-Start-StartScreenFlow", "");
		pgStep.putString(".pyMOId", "Start62");
			pgStep.putString(".pyMOName", "");
			pgStep.putString(".pyImplementation", "WorkList");
		pgStep.putString(".pyCoordX", "0.24");
		pgStep.putString(".pyCoordY", "0");
		pgStep.putString(".pyFromMODefName", "Start");
		pxResults.add(pgStep);
		pgStep = tools.createPage("Data-MO-Activity-Assignment-WorkAction", "");
		pgStep.putString(".pyMOId", "AssignmentSF1");
			pgStep.putString(".pyMOName", "Create");
			pgStep.putString(".pyImplementation", "WorkList");
		pgStep.putString(".pyCoordX", "2.24");
		pgStep.putString(".pyCoordY", "0");
		pgStep.putString(".pyFromMODefName", "AssignmentSF");
		pxResults.add(pgStep);
		pgStep = tools.createPage("Data-MO-Event-End", "");
		pgStep.putString(".pyMOId", "END66");
			pgStep.putString(".pyMOName", "");
			pgStep.putString(".pyImplementation", "");
		pgStep.putString(".pyCoordX", "4.24");
		pgStep.putString(".pyCoordY", "0");
	...
	private void GetNavigationPath_circum0(String pageName, String parentFlowPath, boolean deferredLoad)
	{
		...
		pgNavPath.putString("pyClassName", "OLFHF9-Onboaring-Work-Onboarding");
		...
		pgNavPath.getProperty("pyIsScreenFlow").setValue(pega_procom_pegaprocomutilities.IsScreenFlow("ScreenFlow"));
		...
		pgStep.putString("pyStepLabel", "Create");
		pgStep.putString("pyStepType", "ASSIGNMENT");
		bIsInFlowPath = pega_procom_breadcrumbtrail.isInFlowPath2(tools, "AssignmentSF1", "CreateForm_Default", parentFlowPath, flowNameToSearchForTaskIn, interestPage.getTopLevelPage());
		if (bIsInFlowPath)
		{
			if (!bFoundFlowPath)
			{
				bFoundFlowPath = true;
				flowNameToSearchForTaskIn = tools.getParamValue("FoundInFlowName");
			}
			if (newAssignPage.getString("pxTaskName").equals("AssignmentSF1") && newAssignPage.getString("pyFlowType").equals("CreateForm_Default") && newAssignPage.getString("pyParentFlowPath").equals(parentFlowPath))
		...
		if (!strFlowAction.equals(""))
		{
			...
			{
				ClipboardProperty cp_pyFlowAction = (ClipboardProperty)iter_pyFlowActions.next();
				ClipboardPage pg_pyFlowAction = cp_pyFlowAction.getPageValue();
				if (pg_pyFlowAction.getString("pyInterestPage").equals(interestPage.getReference()))
				{
					pg = pg_pyFlowAction;
					break;
				}
		...
		if (((String)hashShapes.get("END66")).equals("true")) {
			...
			feedingShapes.addElement("AssignmentSF1");
	...
	private boolean CheckFlowSecurity_circum0()
	{
		boolean hasSecurity = false;
		 ...
		 primaryPageClassName = "OLFHF9-Onboaring-Work-Onboarding";
		 }
		...
		if (!hasSecurity && tools.getAuthorizationHandle().havePrivilege(tools, "AM:RULE-OBJ-FLOW:OLFHF9-ONBOARING-WORK-ONBOARDING!CREATEFORM_DEFAULT",primaryPageClassName, interestPage)) 
			 hasSecurity = true;
			 if(!hasSecurity) {
			 if(tools.getAuthorizationHandle().havePrivilege(tools,  "AM:RULE-OBJ-FLOW:OLFHF9-ONBOARING-WORK-ONBOARDING!CREATEFORM_DEFAULT",primaryPageClassName, interestPage,true))
			 isAllowedAccess = true;
			 else return false;
		}
			 if(isAllowedAccess && !hasSecurity) {		com.pega.pegarules.priv.tracer.RuleTraceInfo ruleTraceInfo = new com.pega.pegarules.priv.tracer.RuleTraceInfo                                                                   ("RULE-OBJ-FLOW OLFHF9-ONBOARING-WORK-ONBOARDING CREATEFORM_DEFAULT #20260223T091515.306 GMT","CreateForm_Default","Onboaring","","20260223T091515.572 GMT");
...
private static final com.pega.pegarules.priv.tracer.RuleTraceInfo oTraceInfo_0 = new com.pega.pegarules.priv.tracer.RuleTraceInfo("RULE-OBJ-FLOW OLFHF9-ONBOARING-WORK-ONBOARDING CREATEFORM_DEFAULT #20260223T091515.306 GMT", "OLFHF9-Onboaring-Work-Onboarding CreateForm_Default", "Onboaring", "01-01-01", "20260223T091515.572 GMT");
}
