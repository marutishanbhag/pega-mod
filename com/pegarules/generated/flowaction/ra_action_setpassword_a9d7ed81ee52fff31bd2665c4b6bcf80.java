...
import java.util.*;
public class ra_action_setpassword_a9d7ed81ee52fff31bd2665c4b6bcf80  extends com.pega.pegarules.priv.AbstractFUASupport implements Activity, FUASupport {
	private static final LogHelper  oLog = new LogHelper("Rule_Obj_FlowAction.SetPassword.Data_Admin_Operator_ID.Action");
	public ra_action_setpassword_a9d7ed81ee52fff31bd2665c4b6bcf80(PegaAPI aContext) {
		tools = aContext;
		pega = aContext;
	}
...
/* Instance RULE-OBJ-FLOWACTION DATA-ADMIN-OPERATOR-ID SETPASSWORD #20180713T135447.781 GMT	Pega-Desktop:08-01-01	<no Circumstance>	<no Circumstance Date>	<no date range>	available=Yes	baserule=false */
		...
		mFlowActionHandle = "RULE-OBJ-FLOWACTION DATA-ADMIN-OPERATOR-ID SETPASSWORD #20180713T135447.781 GMT";
		...
		else if (purpose.equals("ACTIVITY"))
		{
			...
			tools.putParamValue("pyActionLabel", "Change Operator ID Password");
		}
		...
		else if (purpose.equals("VALIDATE"))
		{
			...
			tools.putParamValue("confirmHarness", "AutoClose");
		}
		...
		{
			tools.putParamValue("pzFALabel", "Change Operator ID Password");
		}
		else if (purpose.equals("NEXTHARNESS"))
		{
			// Put the harness name into the parameter HarnessName
			tools.putParamValue("HarnessName", "AutoClose");
		}
}
...
//	RULE-UTILITY-FUNCTION UTILITIES ESCAPESPECIALCHARSANDQUOTE #20180713T131443.047 GMT:20180713T131443.047 GMT
	...
	public static String getFUAContentHash() {
		return "c8d4942970661e0c2336e9c4e73fed2f";
	}
...
return "Action";
	...
	private boolean CheckPrivileges_circum0()
	{
		boolean hasSecurity = false;
		 ...
		 primaryPageClassName = "Data-Admin-Operator-ID";
			 ...
			 if(!hasSecurity && tools.getAuthorizationHandle().havePrivilege(tools,  "AM:RULE-OBJ-FLOWACTION:DATA-ADMIN-OPERATOR-ID!SETPASSWORD",primaryPageClassName, interestPage,true))
			 hasSecurity = true;
		if (!hasSecurity)
			return false;
	...
	{
		String strActionName = "SetPassword";
		...
		String strActionLabel = tools.getLocalizedTextForString("pyCaption", "Change Operator ID Password");
		AssignActionPage.putString(".pyActionLabel", strActionLabel);
		AssignActionPage.putString(".pyLabel", "Change Operator ID Password");
		...
		AssignActionPage.putString(".pySectionReference", "pySetPassword");
	...
	private void RunFlowActionActivity_circum0(String actType)
	{
		...
		else if (actType.startsWith("POST"))
		{
			...
			strActivityToRun = "pzValidateOperatorPassword";
			if (actType.equals("POSTCONNECTOR"))
			{
