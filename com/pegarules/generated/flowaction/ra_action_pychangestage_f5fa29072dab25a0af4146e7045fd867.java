...
import java.util.*;
public class ra_action_pychangestage_f5fa29072dab25a0af4146e7045fd867  extends com.pega.pegarules.priv.AbstractFUASupport implements Activity, FUASupport {
	private static final LogHelper  oLog = new LogHelper("Rule_Obj_FlowAction.pyChangeStage.Work_.Action");
	public ra_action_pychangestage_f5fa29072dab25a0af4146e7045fd867(PegaAPI aContext) {
		tools = aContext;
		pega = aContext;
	}
...
{
/* Instance RULE-OBJ-FLOWACTION WORK- PYCHANGESTAGE #20200318T183137.685 GMT	Pega-EndUserUI:08-05-01	<no Circumstance>	<no Circumstance Date>	<no date range>	available=Yes	baserule=false */
		mTraceInfo = oTraceInfo_0;
		mRuleSet = "Pega-EndUserUI";
		mFlowActionHandle = "RULE-OBJ-FLOWACTION WORK- PYCHANGESTAGE #20200318T183137.685 GMT";
		...
		else if (purpose.equals("ACTIVITY"))
		{
			...
			tools.putParamValue("pyActionLabel", "Change stage");
		}
		...
		else if (purpose.equals("VALIDATE"))
		{
			...
			ClipboardPage otherPageInTransaction;
			ClipboardPage newAssignPage = tools.findPage("newAssignPage");
			if (newAssignPage == null || (newAssignPage != null && !pega_procom_pegaprocomutilities.determineDeferErrors(newAssignPage, workPage)))
			{
				// Before validating, clear the messages from all the other pages in the transaction
				if (!workPageName.equals("pyStageList"))
				{
					otherPageInTransaction = tools.findPage("pyStageList");
					if (otherPageInTransaction != null)
						otherPageInTransaction.clearMessages();
				}
			}
			...
			{
				otherPageInTransaction = tools.findPage("pyStageList");
				if (otherPageInTransaction != null && otherPageInTransaction.hasMessages())
				{
					ParameterPage newParamsPage = new ParameterPage();
					newParamsPage.putString("OutputPageName", tools.getParamValue("OutputPageName"));
					newParamsPage.putString("OutputPageClass", tools.getParamValue("OutputPageClass"));
					newParamsPage.putString("pagename", "pyStageList");
					newParamsPage.putString("Message", otherPageInTransaction.getMessagesAll());
					newParamsPage.putString("MessageCount", new Integer(otherPageInTransaction.getMessageCountAll()).toString());
					pega_rules_utilities.callActivity(workPage, "setPageErrors", newParamsPage);
				}
			}
			...
			tools.putParamValue("keepWorkingOnSameWorkBasket", "true");
		...
		{
			tools.putParamValue("pzFALabel", "Change stage");
		}
...
private static final String oDependencies [] = {
	...
	"Rule-Obj-FlowAction:PYCHANGESTAGE", 
	"Rule-Obj-Property:PYCHANGESTAGEOPTION", 
	"Rule-Obj-Property:PYAUDITNOTE", 
	"Rule-Obj-Property:PYGOTOSTAGE"
	};
	...
	private static final DependentRuleInfo[] oDependentRuleInfos = {
		...
		new DependentRuleInfo("PYCHANGESTAGE","Rule-Obj-FlowAction","WORK-",false,"","Pega-EndUserUI","08-05-01","RULE-OBJ-FLOWACTION WORK- PYCHANGESTAGE #20200318T183137.685 GMT","!PYCHANGESTAGE",true,false,"ABSOLUTE_ULTIMATE_STEP_PAGE",1170528359), 
		new DependentRuleInfo("PYCHANGESTAGEOPTION","Rule-Obj-Property","WORK-",true,"Work-","Theme-Cosmos","04-01-01","RULE-OBJ-PROPERTY WORK- PYCHANGESTAGEOPTION #20210506T204032.860 GMT","!PYCHANGESTAGEOPTION",true,false,"ABSOLUTE_IS_NOT_PRIMARY",0), 
		new DependentRuleInfo("PYAUDITNOTE","Rule-Obj-Property","WORK-",true,"Work-","Pega-EndUserUI","08-05-01","RULE-OBJ-PROPERTY WORK- PYAUDITNOTE #20200617T142248.201 GMT","!PYAUDITNOTE",true,false,"ABSOLUTE_IS_NOT_PRIMARY",0), 
		new DependentRuleInfo("PYGOTOSTAGE","Rule-Obj-Property","WORK-",true,"Work-","Theme-Cosmos","04-01-01","RULE-OBJ-PROPERTY WORK- PYGOTOSTAGE #20210506T204032.875 GMT","!PYGOTOSTAGE",true,false,"ABSOLUTE_IS_NOT_PRIMARY",0)
	};
...
//	RULE-OBJ-FLOWACTION WORK- PYCHANGESTAGE #20200318T183137.685 GMT:20200318T183137.685 GMT
//	RULE-OBJ-PROPERTY WORK- PYAUDITNOTE #20200617T142248.201 GMT:20200617T142248.201 GMT
//	RULE-OBJ-PROPERTY WORK- PYGOTOSTAGE #20210506T204032.875 GMT:20210506T204032.875 GMT
//	RULE-OBJ-PROPERTY WORK- PYCHANGESTAGEOPTION #20210506T204032.860 GMT:20210506T204032.860 GMT
...
//	RULE-UTILITY-FUNCTION UTILITIES ESCAPESPECIALCHARSANDQUOTE #20180713T131443.047 GMT:20180713T131443.047 GMT
	...
	public static String getFUAContentHash() {
		return "f5a669c05f895f7295c3a8b1aaa326bd";
	}
...
return "Action";
	...
	private boolean CheckPrivileges_circum0()
	{
		boolean hasSecurity = false;
		 ...
		 primaryPageClassName = "Work-";
			 ...
			 if(!hasSecurity && tools.getAuthorizationHandle().havePrivilege(tools,  "AM:RULE-OBJ-FLOWACTION:WORK-!PYCHANGESTAGE",primaryPageClassName, interestPage,true))
			 hasSecurity = true;
		if (!hasSecurity)
			return false;
	...
	{
		String strActionName = "pyChangeStage";
		...
		String strActionLabel = tools.getLocalizedTextForString("pyCaption", "Change stage");
		AssignActionPage.putString(".pyActionLabel", strActionLabel);
		AssignActionPage.putString(".pyLabel", "Change stage");
		...
		AssignActionPage.putString(".pySectionReference", "pyChangeStage");
	...
	private void RunFlowActionActivity_circum0(String actType)
	{
		...
		{
			strActivityToRun = "pzPreChangeStage";
			try {
		...
		else if (actType.startsWith("POST"))
		{
			...
			strActivityToRun = "pyChangeStage";
			if (actType.equals("POSTCONNECTOR"))
			{
...
// Expression: .pyAuditNote
			newParamsPage.putString("AuditNote", scalarValueQuery_1.resolveToString(tools, myStepPage, ImmutablePropertyInfo.TYPE_TEXT));
// Expression: true
String pz_2 = String.valueOf(true);
			newParamsPage.putString("CleanUpProcesses", pz_2);
// Expression: .pyGotoStage
			newParamsPage.putString("ChangeToStage", scalarValueQuery_3.resolveToString(tools, myStepPage, ImmutablePropertyInfo.TYPE_TEXT));
// Expression: .pyChangeStageOption
			newParamsPage.putString("ChangeToNextStage", scalarValueQuery_4.resolveToString(tools, myStepPage, ImmutablePropertyInfo.TYPE_TEXT));
			try {
				pega_rules_utilities.callActivity(interestPage, strActivityToRun, newParamsPage);
			} catch (ActivityTerminateException e) {
				throw e;
...
private static final ScalarValueQuery scalarValueQuery_4 = FUAUtil.createQueryBuilder().allowMissingPropertyException().scalarProperty("pyChangeStageOption").buildScalarValueQuery();
private static final ScalarValueQuery scalarValueQuery_3 = FUAUtil.createQueryBuilder().allowMissingPropertyException().scalarProperty("pyGotoStage").buildScalarValueQuery();
private static final ScalarValueQuery scalarValueQuery_1 = FUAUtil.createQueryBuilder().allowMissingPropertyException().scalarProperty("pyAuditNote").buildScalarValueQuery();

private static final com.pega.pegarules.priv.tracer.RuleTraceInfo oTraceInfo_0 = new com.pega.pegarules.priv.tracer.RuleTraceInfo("RULE-OBJ-FLOWACTION WORK- PYCHANGESTAGE #20200318T183137.685 GMT", "Work- pyChangeStage", "Pega-EndUserUI", "08-05-01", "20200318T183137.685 GMT");
}
