...
import java.util.*;
public class ra_action_pysetupusermenu_3ad150dee90a243ef49352a5d7eedb2a  extends com.pega.pegarules.priv.AbstractFUASupport implements Activity, FUASupport {
	private static final LogHelper  oLog = new LogHelper("Rule_Navigation.pySetupUserMenu.PegaAccel_Task_BuildApplication.Action");
	public ra_action_pysetupusermenu_3ad150dee90a243ef49352a5d7eedb2a(PegaAPI aContext) {
		tools = aContext;
		pega = aContext;
	}
...
{
/* Instance RULE-NAVIGATION PEGAACCEL-TASK-BUILDAPPLICATION PYSETUPUSERMENU #20230618T001753.336 GMT	Pega-ProcessArchitect:08-23-01	<no Circumstance>	<no Circumstance Date>	<no date range>	available=Yes	baserule=false */
...
ClipboardPage fOutputPage = null;if (StringUtils.isBlank(tools.getParamValue("deferPagePath")) && !tools.getParamValue("usingPageRef").equals("true")) {
	...
	if(true){
		fOutputPage.putString("pyLabel", tools.getLocalizedTextForString("pyLabel", "Setup User menu"));
	}else{
		fOutputPage.getProperty("pyLabel").setValue("Setup User menu");
	}
}
...
private static final String oDependencies [] = {
	...
	"Rule-Navigation:PYSETUPUSERMENU"
	};
	...
	private static final DependentRuleInfo[] oDependentRuleInfos = {
		...
		new DependentRuleInfo("PYSETUPUSERMENU","Rule-Navigation","PEGAACCEL-TASK-BUILDAPPLICATION",false,"","Pega-ProcessArchitect","08-23-01","RULE-NAVIGATION PEGAACCEL-TASK-BUILDAPPLICATION PYSETUPUSERMENU #20230618T001753.336 GMT","!PYSETUPUSERMENU",true,false,"ABSOLUTE_ULTIMATE_STEP_PAGE",553625128)
	};
...
//	RULE-NAVIGATION PEGAACCEL-TASK-BUILDAPPLICATION PYSETUPUSERMENU #20230618T001753.336 GMT:20230618T001753.336 GMT
...
//	RULE-UTILITY-FUNCTION STRING WHATCOMESAFTERFIRST #20180713T131442.842 GMT:20180713T131442.842 GMT
//	RULE-UTILITY-FUNCTION STRING WHATCOMESBEFOREFIRST #20180713T131442.854 GMT:20180713T131442.854 GMT
	...
	public static String getFUAContentHash() {
		return "2e99e64664a1feab6132d66931052a7e";
	}
...
/* Method generated for Top Level Node -"" for Navigation Rule :"Setup User menu" */
...
actionPage1.putString("pyHidden",String.valueOf(hideElement));actionPage1.putString("pyDisabled",String.valueOf(disableElement));actionPage1.putString( "pyName","ID1294358612688000");actionPage1.putString( "pyImageSource","styleclass");actionPage1.putString( "pyIconStyle","Custom_HeaderNav");propertyValue = "OperatorID.pyUserName";
if(!isDynamicFieldNameForMenu){actionPage1.putString("pyCaption",tools.getLocalizedTextForString("pyCaption", tools.getProperty(propertyValue).getStringValue()));}else{actionPage1.putString("pyCaption",tools.getLocalizedTextForString("OperatorID.pyUserName", "\t["+tools.getProperty(tools.getParamValue("ApplyToPage")+".pxObjClass").getStringValue()+"OperatorID.pyUserName]"+tools.getProperty(propertyValue).getStringValue()));}actionPage1.putString("pySummary", tools.getLocalizedTextForString("pySummary", ""));actionPage1.putString("pyBadgeProperty", tools.getLocalizedTextForString("pyBadgeProperty", ""));actionPage1.putString("pyToolTip", tools.getLocalizedTextForString("pyToolTip", ""));if(testIDWhen || functionalTestingWhen){actionPage1.putString( "pyAutomationID","201711011301500141158");}actionPage1.putString( "pyBadgeFormat","Standard (label)");actionPage1.putString( "pyDeferLoad","false");actionPage1.putString( "pyImage","Custom_HeaderNav");actionPage1.putString("pyType","Action");
...
deferLoad = false;
if((!deferLoad && StringUtils.isBlank(deferLoadPath)) || (!StringUtils.isBlank(deferLoadPath) && ( deferIndex>=deferPathArr.length || deferPathArr[deferIndex].equalsIgnoreCase("ID1294358612688000")))){
deferIndex ++;
...
if(processAction||("ID1305182556502000".equals(nodePathArr[currentElement])&& processShortcut && !isNodePathEmpty)){ 
matchFound=true;
...
/*Node Generated for caption -  and Name - ID1305182556502000*/
...
String whenResolved ="pxHasWorkSpacePortals";
...
actionPage2.putString("pyHidden",String.valueOf(hideElement));actionPage2.putString("pyDisabled",String.valueOf(disableElement));actionPage2.putString( "pyName","ID1305182556502000");actionPage2.putString("pyCaption", tools.getLocalizedTextForString("pyCaption", "Switch portal"));actionPage2.putString("pySummary", tools.getLocalizedTextForString("pySummary", ""));actionPage2.putString("pyBadgeProperty", tools.getLocalizedTextForString("pyBadgeProperty", ""));actionPage2.putString("pyToolTip", tools.getLocalizedTextForString("pyToolTip", "Switch portal"));if(testIDWhen || functionalTestingWhen){actionPage2.putString( "pyAutomationID","201711011301480920895");}actionPage2.putString( "pyBadgeFormat","Standard (label)");actionPage2.putString( "pyDeferLoad","false");actionPage2.putString( "pyImage","");if (!hideElement) { 
...
deferLoad = false;
if((!deferLoad && StringUtils.isBlank(deferLoadPath)) || (!StringUtils.isBlank(deferLoadPath) && ( deferIndex>=deferPathArr.length || deferPathArr[deferIndex].equalsIgnoreCase("ID1305182556502000")))){
deferIndex ++;
if(!isNodePathEmpty && matchFound){ 
if(currentElement < nodePathArr.length-1)
currentElement++;
...
preActParamMap.put("pyActivityName","pxEnsureUserPortalHasLabel");
tools.doActivity(preActParamMap, tools.getStepPage(), newParamsPage);
nodePageList = tools.getIfPresent("pxThread.pyUserPortals");
...
String actionListParentPage3 = tools.getParamValue("ApplyToPage");	Iterator actionListIter3 = nodePageList.iterator();
	while (actionListIter3.hasNext()) {		iterListResult = (ClipboardProperty)actionListIter3.next();
		...
		if(strChildListPageRef3.lastIndexOf(".") == 0) {
			strChildListPageRef3 = "pxThread" + strChildListPageRef3;
		}
...
showWhenCondition = true;
String whenResolved ="pxIsAWorkspacePortal";
if (!whenResolved.isEmpty()) {
try{showWhenCondition = pega_uiengine_pznavigation.evaluateWhen(whenResolved,tools.getStepPage().getClassName(),tools);
}catch(Exception e){oLog.error("Failed to resolve when rule:" + e);}
}
hideElement = !showWhenCondition;
if (!hideElement) {
actionPage3.getProperty("pyChecked").setValue(pega_uiengine_pznavigation.evaluateWhen("pzIsCurrentPortal",tools.getStepPage().getClassName(),tools));
actionPage3.getProperty("pyGroup").getPageValue().putString("pyGroupID","PortalGroup");
actionPage3.getProperty("pyGroup").getPageValue().putString("pyIsGroup","true");
}
actionPage3.putString("pyHidden",String.valueOf(hideElement));actionPage3.putString("pyDisabled",String.valueOf(disableElement));actionPage3.putString( "pyName","");propertyValue = ".pyLabel";
propertyValue = tools.getParamValue("ApplyToPage")+".pyLabel";if(!isDynamicFieldNameForMenu){actionPage3.putString("pyCaption",tools.getLocalizedTextForString("pyCaption", tools.getProperty(propertyValue).getStringValue()));}else{actionPage3.putString("pyCaption",tools.getLocalizedTextForString(".pyLabel", "\t["+tools.getProperty(tools.getParamValue("ApplyToPage")+".pxObjClass").getStringValue()+".pyLabel]"+tools.getProperty(propertyValue).getStringValue()));}actionPage3.putString("pySummary", tools.getLocalizedTextForString("pySummary", ""));actionPage3.putString("pyBadgeProperty", tools.getLocalizedTextForString("pyBadgeProperty", ""));propertyValue = ".pyLabel";
propertyValue = tools.getParamValue("ApplyToPage")+".pyLabel";if(!isDynamicFieldNameForMenu){actionPage3.putString("pyToolTip",tools.getLocalizedTextForString("pyToolTip", tools.getProperty(propertyValue).getStringValue()));}else{actionPage3.putString("pyToolTip",tools.getLocalizedTextForString(".pyLabel", "\t["+tools.getProperty(tools.getParamValue("ApplyToPage")+".pxObjClass").getStringValue()+".pyLabel]"+tools.getProperty(propertyValue).getStringValue()));}actionPage3.putString( "pyBadgeFormat","Standard (label)");actionPage3.putString( "pyDeferLoad","");propertyValue = ".pxListSubscript";
propertyValue = tools.getParamValue("ApplyToPage")+".pxListSubscript";actionPage3.putString("pySubscript",tools.getProperty(propertyValue).getStringValue());if(testIDWhen || functionalTestingWhen){if(!"".equals(tools.getProperty(tools.getParamValue("ApplyToPage")+".pxListSubscript").getStringValue())) {actionPage3.putString("pyAutomationID","201711011301480901296" + "_" + tools.getProperty(tools.getParamValue("ApplyToPage")+".pxListSubscript").getStringValue());}else if (!"".equals(tools.getProperty(tools.getParamValue("ApplyToPage")+".pxSubscript").getStringValue())) {actionPage3.putString("pyAutomationID","201711011301480901296" + "_" + tools.getProperty(tools.getParamValue("ApplyToPage")+".pxSubscript").getStringValue());}else {actionPage3.putString( "pyAutomationID","201711011301480901296");}}actionPage3.putString( "pyImage","");if (!hideElement) { 
...
propertyValue = ".pyPortalLayout";
propertyValue = tools.getParamValue("ApplyToPage")+".pyPortalLayout";commandPage3.getProperty("pyParameters").getPageValue(1).putString("pyValue",tools.getProperty(propertyValue).getStringValue());commandPage3.getProperty("pyParameters").getPageValue(1).putString( "pyName","param1");propertyValue = "pyPortal.pyOwner";
commandPage3.getProperty("pyParameters").getPageValue(2).putString("pyValue",tools.getProperty(propertyValue).getStringValue());commandPage3.getProperty("pyParameters").getPageValue(2).putString( "pyName","param2");propertyValue = ".pyLabel";
propertyValue = tools.getParamValue("ApplyToPage")+".pyLabel";commandPage3.getProperty("pyParameters").getPageValue(3).putString("pyValue",tools.getProperty(propertyValue).getStringValue());commandPage3.getProperty("pyParameters").getPageValue(3).putString( "pyName","param3");commandPage3.putString( "pyFunctionName","changePortal");executePage.putPage("pyCustom",commandPage3);
...
if(processAction||("ID1294358612678000".equals(nodePathArr[currentElement])&& processShortcut && !isNodePathEmpty)){ 
matchFound=true;
if(processShortcut && !isNodePathEmpty && dontExitNow ){ 
if(nodeDepth==nodePathArr.length){ 
nodePathValidated=true;
...
if(dontExitNow){ //start of dnt exit now condition
/*Node Generated for caption -  and Name - ID1294358612678000*/
...
actionPage6.putString("pyHidden",String.valueOf(hideElement));actionPage6.putString("pyDisabled",String.valueOf(disableElement));actionPage6.putString( "pyName","ID1294358612678000");actionPage6.putString("pyCaption", tools.getLocalizedTextForString("pyCaption", "Log off"));actionPage6.putString("pySummary", tools.getLocalizedTextForString("pySummary", ""));actionPage6.putString("pyBadgeProperty", tools.getLocalizedTextForString("pyBadgeProperty", ""));actionPage6.putString("pyToolTip", tools.getLocalizedTextForString("pyToolTip", ""));if(testIDWhen || functionalTestingWhen){actionPage6.putString( "pyAutomationID","201711011301500120490");}actionPage6.putString( "pyBadgeFormat","");actionPage6.putString( "pyDeferLoad","");actionPage6.putString( "pyImage","");actionPage6.putString("pyType","Action");
...
behaviorPage6.putString("pyAction","logOff");((PegaAPI)tools).getUIEngine().getUIAction(actionPage6.getPage("pyBehaviors(<LAST>)")).register();
...
private static final com.pega.pegarules.priv.tracer.RuleTraceInfo oTraceInfo_0 = new com.pega.pegarules.priv.tracer.RuleTraceInfo("RULE-NAVIGATION PEGAACCEL-TASK-BUILDAPPLICATION PYSETUPUSERMENU #20230618T001753.336 GMT", "PegaAccel-Task-BuildApplication pySetupUserMenu", "Pega-ProcessArchitect", "08-23-01", "20230618T001753.336 GMT");
}
