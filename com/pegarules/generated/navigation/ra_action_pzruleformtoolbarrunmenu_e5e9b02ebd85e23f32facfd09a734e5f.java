...
import java.util.*;
public class ra_action_pzruleformtoolbarrunmenu_e5e9b02ebd85e23f32facfd09a734e5f  extends com.pega.pegarules.priv.AbstractFUASupport implements Activity, FUASupport {
	private static final LogHelper  oLog = new LogHelper("Rule_Navigation.pzRuleFormToolbarRunMenu.Rule_Obj_Flow.Action");
	public ra_action_pzruleformtoolbarrunmenu_e5e9b02ebd85e23f32facfd09a734e5f(PegaAPI aContext) {
		tools = aContext;
		pega = aContext;
	}
...
{
/* Instance RULE-NAVIGATION RULE-OBJ-FLOW PZRULEFORMTOOLBARRUNMENU #20180713T135317.459 GMT	Pega-Desktop:08-01-01	<no Circumstance>	<no Circumstance Date>	<no date range>	available=Final	baserule=false */
...
private static final String oDependencies [] = {
	...
	private static final DependentRuleInfo[] oDependentRuleInfos = {
		...
		new DependentRuleInfo("PZRULEFORMTOOLBARRUNMENU","Rule-Navigation","RULE-OBJ-FLOW",false,"","Pega-Desktop","08-01-01","RULE-NAVIGATION RULE-OBJ-FLOW PZRULEFORMTOOLBARRUNMENU #20180713T135317.459 GMT","!PZRULEFORMTOOLBARRUNMENU",true,false,"ABSOLUTE_ULTIMATE_STEP_PAGE",182872147)
	};
...
//	RULE-NAVIGATION RULE-OBJ-FLOW PZRULEFORMTOOLBARRUNMENU #20180713T135317.459 GMT:20180713T135317.459 GMT
...
//	RULE-UTILITY-FUNCTION STRING ISINTEGER #20180713T131441.652 GMT:20180713T131441.652 GMT
	...
	public static String getFUAContentHash() {
		return "3085f6f24991386b6d093aecb999c342";
	}
...
if( processAction ||(processShortcut && "ID1381181124083000".equals(nodePathArr[0])  && !isNodePathEmpty)){
...
int deferIndex = 0;
int nodeDepth=0;
if(!StringUtils.isBlank(deferLoadPath) && deferPathArr.length>0 && !deferPathArr[deferIndex].equalsIgnoreCase("ID1381181124083000")){
return;}
if(processShortcut &&"ID1381181124083000".equals(nodePathArr[0]) && !isNodePathEmpty){
nodeDepth++;
...
String whenResolved ="pzRuleFormToolbarShowRunForFlow";
...
actionPage1.putString("pyHidden",String.valueOf(hideElement));actionPage1.putString("pyDisabled",String.valueOf(disableElement));actionPage1.putString( "pyName","ID1381181124083000");actionPage1.putString( "pyImageSource","styleclass");actionPage1.putString( "pyIconStyle","hotkey-ruleform-run");actionPage1.putString( "pyCaption","Run");actionPage1.putString( "pySummary","");actionPage1.putString( "pyBadgeProperty","");actionPage1.putString( "pyToolTip","");if(testIDWhen || functionalTestingWhen){actionPage1.putString( "pyAutomationID","");}actionPage1.putString( "pyBadgeFormat","Standard (label)");actionPage1.putString( "pyDeferLoad","false");actionPage1.putString( "pyImage","hotkey-ruleform-run");if (!hideElement) { 
...
behaviorPage1.putString("pyAction","createWork");ClipboardPage commandPage1 = tools.createPage("Embed-DesktopAPI-CreateNewWork","commandType");
propertyValue = ".pyClassName";
propertyValue = tools.getParamValue("ApplyToPage")+".pyClassName";commandPage1.putString("pyClassName",tools.getProperty(propertyValue).getStringValue());propertyValue = ".pyFlowType";
propertyValue = tools.getParamValue("ApplyToPage")+".pyFlowType";commandPage1.putString("pyFlowName",tools.getProperty(propertyValue).getStringValue());commandPage1.putString( "pyElementName","");commandPage1.putString( "pyTargetElement","");commandPage1.putString( "pyIsTargetMicroDC","");commandPage1.putString( "pyACName","");commandPage1.putString( "pySelectedMobileTransitionType","");commandPage1.getProperty("pyFlowParameters").getPageValue(1).putString( "pyValue","");commandPage1.getProperty("pyFlowParameters").getPageValue(1).putString( "pyName","");commandPage1.getProperty("pyFlowParameters").getPageValue(1).putString( "pxObjClass","Embed-MethodParams");executePage.putPage("pyCreateNewWork",commandPage1);
...
boolean matchFound=true;
int currentElement=0;
if( processAction ||(processShortcut && "ID1482842314423000".equals(nodePathArr[0])  && !isNodePathEmpty)){
String uniqueSuffix = new Long(tools.getDateTimeUtils().currentTimeUnique()).toString();
String commonPrefix = "Nav_";
boolean showWhenCondition = true;
boolean isActiveNavItem = false;
...
int deferIndex = 0;
int nodeDepth=0;
if(!StringUtils.isBlank(deferLoadPath) && deferPathArr.length>0 && !deferPathArr[deferIndex].equalsIgnoreCase("ID1482842314423000")){
return;}
if(processShortcut &&"ID1482842314423000".equals(nodePathArr[0]) && !isNodePathEmpty){
nodeDepth++;
if(nodeDepth==nodePathArr.length){
nodePathValidated=true;
}
...
showWhenCondition = true;
String whenResolved ="pzShowRecordOptionForFlowRule";
if (!whenResolved.isEmpty()) {
try{showWhenCondition = pega_uiengine_pznavigation.evaluateWhen(whenResolved,tools.getStepPage().getClassName(),tools);
}catch(Exception e){oLog.error("Failed to resolve when rule:" + e);}
}
...
actionPage2.putString("pyHidden",String.valueOf(hideElement));actionPage2.putString("pyDisabled",String.valueOf(disableElement));actionPage2.putString( "pyName","ID1482842314423000");actionPage2.putString( "pyCaption","Record test case");actionPage2.putString( "pySummary","");actionPage2.putString( "pyBadgeProperty","");actionPage2.putString( "pyToolTip","");if(testIDWhen || functionalTestingWhen){actionPage2.putString( "pyAutomationID","");}actionPage2.putString( "pyBadgeFormat","Standard (label)");actionPage2.putString( "pyDeferLoad","false");actionPage2.putString( "pyImage","");if (!hideElement) { 
...
commandPage2.getProperty("pyPreDataTransform").getPageValue().putString( "pyName","pzSetRecordingMode");commandPage2.putString( "pyUsingPage", tools.getStepPage().getReference());commandPage2.getPage("pyPreDataTransform").getProperty("pyDataTransformParams").getPageValue(1).putString( "pyValue","");commandPage2.getPage("pyPreDataTransform").getProperty("pyDataTransformParams").getPageValue(1).putString( "pyName","");executePage.putPage("pyRunDataTransform",commandPage2);
...
propertyValue = tools.getParamValue("ApplyToPage")+".pyFlowType";commandPage3.putString("pyFlowName",tools.getProperty(propertyValue).getStringValue());commandPage3.putString( "pyElementName","");commandPage3.putString( "pyTargetElement","");commandPage3.putString( "pyIsTargetMicroDC","");commandPage3.putString( "pyACName","");commandPage3.putString( "pySelectedMobileTransitionType","pega.mobile.transitions.NONE");commandPage3.getProperty("pyFlowParameters").getPageValue(1).putString( "pyValue","");commandPage3.getProperty("pyFlowParameters").getPageValue(1).putString( "pyName","");commandPage3.getProperty("pyFlowParameters").getPageValue(1).putString( "pxObjClass","Embed-MethodParams");behaviorPage3.putPage("pyActionAPI",commandPage3);
((PegaAPI)tools).getUIEngine().getUIAction(actionPage2.getPage("pyBehaviors(<LAST>)")).register();
...
private static final com.pega.pegarules.priv.tracer.RuleTraceInfo oTraceInfo_0 = new com.pega.pegarules.priv.tracer.RuleTraceInfo("RULE-NAVIGATION RULE-OBJ-FLOW PZRULEFORMTOOLBARRUNMENU #20180713T135317.459 GMT", "Rule-Obj-Flow pzRuleFormToolbarRunMenu", "Pega-Desktop", "08-01-01", "20180713T135317.459 GMT");
}
