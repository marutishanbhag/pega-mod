...
import java.util.*;
public class ra_action_pzopenportal_b9678824f5ecb7e5170c19a056b3268b  extends com.pega.pegarules.priv.AbstractFUASupport implements Activity, FUASupport {
	private static final LogHelper  oLog = new LogHelper("Rule_Navigation.pzOpenPortal.Data_Portal.Action");
	public ra_action_pzopenportal_b9678824f5ecb7e5170c19a056b3268b(PegaAPI aContext) {
		tools = aContext;
		pega = aContext;
	}
...
{
/* Instance RULE-NAVIGATION DATA-PORTAL PZOPENPORTAL #20200512T155845.000 GMT	Pega-Desktop:08-05-01	<no Circumstance>	<no Circumstance Date>	<no date range>	available=Final	baserule=false */
...
ClipboardPage fOutputPage = null;if (StringUtils.isBlank(tools.getParamValue("deferPagePath")) && !tools.getParamValue("usingPageRef").equals("true")) {
	...
	if(true){
		fOutputPage.putString("pyLabel", tools.getLocalizedTextForString("pyLabel", "pzOpenPortal"));
	}else{
		fOutputPage.getProperty("pyLabel").setValue("pzOpenPortal");
	}
}
...
private static final String oDependencies [] = {
	...
	private static final DependentRuleInfo[] oDependentRuleInfos = {
		...
		new DependentRuleInfo("PZOPENPORTAL","Rule-Navigation","DATA-PORTAL",false,"","Pega-Desktop","08-05-01","RULE-NAVIGATION DATA-PORTAL PZOPENPORTAL #20200512T155845.000 GMT","!PZOPENPORTAL",true,false,"ABSOLUTE_ULTIMATE_STEP_PAGE",-1110406556)
	};
...
//	RULE-NAVIGATION DATA-PORTAL PZOPENPORTAL #20200512T155845.000 GMT:20200512T155845.000 GMT
...
//	RULE-UTILITY-FUNCTION STRING ISINTEGER #20180713T131441.652 GMT:20180713T131441.652 GMT
	...
	public static String getFUAContentHash() {
		return "2bbebca9ba0df0de9f417c2d8683fc5f";
	}
...
/* Method generated for Top Level Node -"" for Navigation Rule :"pzOpenPortal" */
...
if( processAction ||(processShortcut && "ID1441984198194000".equals(nodePathArr[0])  && !isNodePathEmpty)){
...
int deferIndex = 0;
int nodeDepth=0;
if(!StringUtils.isBlank(deferLoadPath) && deferPathArr.length>0 && !deferPathArr[deferIndex].equalsIgnoreCase("ID1441984198194000")){
return;}
if(processShortcut &&"ID1441984198194000".equals(nodePathArr[0]) && !isNodePathEmpty){
nodeDepth++;
...
String whenResolved ="pzNoSecondaryPortals";
...
actionPage1.putString("pyHidden",String.valueOf(hideElement));actionPage1.putString("pyDisabled",String.valueOf(disableElement));actionPage1.putString( "pyName","ID1441984198194000");actionPage1.putString("pyCaption", tools.getLocalizedTextForString("pyCaption", "No other portals"));actionPage1.putString("pySummary", tools.getLocalizedTextForString("pySummary", ""));actionPage1.putString("pyBadgeProperty", tools.getLocalizedTextForString("pyBadgeProperty", ""));actionPage1.putString("pyToolTip", tools.getLocalizedTextForString("pyToolTip", ""));if(testIDWhen || functionalTestingWhen){actionPage1.putString( "pyAutomationID","202005071302360388567");}actionPage1.putString( "pyBadgeFormat","Standard (label)");actionPage1.putString( "pyDeferLoad","");actionPage1.putString( "pyImage","");if (!hideElement) { 
...
boolean matchFound=true;
int currentElement=0;
if( processAction ||(processShortcut && "openPortal".equals(nodePathArr[0])  && !isNodePathEmpty)){
String uniqueSuffix = new Long(tools.getDateTimeUtils().currentTimeUnique()).toString();
String commonPrefix = "Nav_";
boolean showWhenCondition = true;
boolean isActiveNavItem = false;
...
int deferIndex = 0;
int nodeDepth=0;
if(!StringUtils.isBlank(deferLoadPath) && deferPathArr.length>0 && !deferPathArr[deferIndex].equalsIgnoreCase("openPortal")){
return;}
if(processShortcut &&"openPortal".equals(nodePathArr[0]) && !isNodePathEmpty){
nodeDepth++;
if(nodeDepth==nodePathArr.length){
nodePathValidated=true;
}
...
showWhenCondition = true;
String whenResolved ="pzHasSecondaryPortals";
if (!whenResolved.isEmpty()) {
try{showWhenCondition = pega_uiengine_pznavigation.evaluateWhen(whenResolved,tools.getStepPage().getClassName(),tools);
}catch(Exception e){oLog.error("Failed to resolve when rule:" + e);}
}
...
actionPage2.putString("pyHidden",String.valueOf(hideElement));actionPage2.putString("pyDisabled",String.valueOf(disableElement));actionPage2.putString( "pyName","openPortal");actionPage2.putString("pyCaption", tools.getLocalizedTextForString("pyCaption", ""));actionPage2.putString("pySummary", tools.getLocalizedTextForString("pySummary", ""));actionPage2.putString("pyBadgeProperty", tools.getLocalizedTextForString("pyBadgeProperty", ""));actionPage2.putString("pyToolTip", tools.getLocalizedTextForString("pyToolTip", ""));if(testIDWhen || functionalTestingWhen){actionPage2.putString( "pyAutomationID","20200507130236041188");}actionPage2.putString( "pyBadgeFormat","Standard (label)");actionPage2.putString( "pyDeferLoad","");actionPage2.putString( "pyImage","");if (!hideElement) { 
...
deferLoad = false;
if((!deferLoad && StringUtils.isBlank(deferLoadPath)) || (!StringUtils.isBlank(deferLoadPath) && ( deferIndex>=deferPathArr.length || deferPathArr[deferIndex].equalsIgnoreCase("openPortal")))){
deferIndex ++;
...
if(processAction||("this.pyPortalLayout".equals(nodePathArr[currentElement])&& processShortcut && !isNodePathEmpty)){ 
matchFound=true;
...
/*Node Generated for caption -  and Name - this.pyPortalLayout*/

nodePageList = tools.getIfPresent("pyPortalsList.pyUserPortals");
...
String actionListParentPage3 = tools.getParamValue("ApplyToPage");	Iterator actionListIter3 = nodePageList.iterator();
	while (actionListIter3.hasNext()) {		iterListResult = (ClipboardProperty)actionListIter3.next();
		...
		if(strChildListPageRef3.lastIndexOf(".") == 0) {
			strChildListPageRef3 = "pyPortalsList" + strChildListPageRef3;
		}
...
showWhenCondition = true;
String whenResolved ="pzFilterDSLaunchMenu";
if (!whenResolved.isEmpty()) {
try{showWhenCondition = pega_uiengine_pznavigation.evaluateWhen(whenResolved,tools.getStepPage().getClassName(),tools);
}catch(Exception e){oLog.error("Failed to resolve when rule:" + e);}
}
...
actionPage3.putString("pyHidden",String.valueOf(hideElement));actionPage3.putString("pyDisabled",String.valueOf(disableElement));propertyValue = "this.pyPortalLayout";
actionPage3.putString("pyName",tools.getProperty(propertyValue).getStringValue());propertyValue = ".pyPortalLayout.pyLabel";
propertyValue = tools.getParamValue("ApplyToPage")+".pyPortalLayout.pyLabel";if(!isDynamicFieldNameForMenu){actionPage3.putString("pyCaption",tools.getLocalizedTextForString("pyCaption", tools.getProperty(propertyValue).getStringValue()));}else{actionPage3.putString("pyCaption",tools.getLocalizedTextForString(".pyPortalLayout.pyLabel", "\t["+tools.getProperty(tools.getParamValue("ApplyToPage")+".pxObjClass").getStringValue()+".pyPortalLayout.pyLabel]"+tools.getProperty(propertyValue).getStringValue()));}actionPage3.putString("pySummary", tools.getLocalizedTextForString("pySummary", ""));actionPage3.putString("pyBadgeProperty", tools.getLocalizedTextForString("pyBadgeProperty", ""));actionPage3.putString("pyToolTip", tools.getLocalizedTextForString("pyToolTip", ""));actionPage3.putString( "pyBadgeFormat","Standard (label)");actionPage3.putString( "pyDeferLoad","");propertyValue = ".pxListSubscript";
propertyValue = tools.getParamValue("ApplyToPage")+".pxListSubscript";actionPage3.putString("pySubscript",tools.getProperty(propertyValue).getStringValue());if(testIDWhen || functionalTestingWhen){if(!"".equals(tools.getProperty(tools.getParamValue("ApplyToPage")+".pxListSubscript").getStringValue())) {actionPage3.putString("pyAutomationID","202005071302360405991" + "_" + tools.getProperty(tools.getParamValue("ApplyToPage")+".pxListSubscript").getStringValue());}else if (!"".equals(tools.getProperty(tools.getParamValue("ApplyToPage")+".pxSubscript").getStringValue())) {actionPage3.putString("pyAutomationID","202005071302360405991" + "_" + tools.getProperty(tools.getParamValue("ApplyToPage")+".pxSubscript").getStringValue());}else {actionPage3.putString( "pyAutomationID","202005071302360405991");}}actionPage3.putString( "pyImage","");if (!hideElement) { 
...
commandPage3.getProperty("pyPreDataTransform").getPageValue().putString( "pyName","pzSetIsExpressModeEnabled");commandPage3.putString( "pyUsingPage", "Declare_pyDisplay");commandPage3.getPage("pyPreDataTransform").getProperty("pyDataTransformParams").getPageValue(1).putString( "pyValue","false");commandPage3.getPage("pyPreDataTransform").getProperty("pyDataTransformParams").getPageValue(1).putString( "pyName","ValueToSet");executePage.putPage("pyRunDataTransform",commandPage3);
...
propertyValue = ".pyPortalThreadName";
propertyValue = tools.getParamValue("ApplyToPage")+".pyPortalThreadName";commandPage4.putString("pyWindowName",tools.getProperty(propertyValue).getStringValue());commandPage4.putString( "pyOptions","top=0,left=0,location=1,menubar=1,toolbar=1,status=1,resizable=1,location=1,scrollbars=1");commandPage4.putString( "pyReplace","false");commandPage4.putString( "pyUseAlternateUrlBase","false");commandPage4.putString( "pyNoEncodingUrl","false");commandPage4.putString( "pyUrl",tools.findPage("pxThread").getString("pxReqURI"));commandPage4 .putString( "pyActivity", tools.getProperty(".pxObjClass").getStringValue() + ".RedirectAndRun");
 propertyValue = ".pyPortalThreadName";
propertyValue = tools.getParamValue("ApplyToPage")+".pyPortalThreadName";commandPage4.getProperty("pyQueryString").getPageValue(1).putString("pyValue",tools.getProperty(propertyValue).getStringValue());commandPage4.getProperty("pyQueryString").getPageValue(1).putString( "pyName","ThreadName");propertyValue = ".pyPortalURL";
propertyValue = tools.getParamValue("ApplyToPage")+".pyPortalURL";commandPage4.getProperty("pyQueryString").getPageValue(2).putString("pyValue",tools.getProperty(propertyValue).getStringValue());commandPage4.getProperty("pyQueryString").getPageValue(2).putString( "pyName","Location");commandPage4.getProperty("pyQueryString").getPageValue(3).putString( "pyValue","true");commandPage4.getProperty("pyQueryString").getPageValue(3).putString( "pyName","bPurgeTargetThread");behaviorPage4.putPage("pyActionAPI",commandPage4);
((PegaAPI)tools).getUIEngine().getUIAction(actionPage3.getPage("pyBehaviors(<LAST>)")).register();
...
private static final com.pega.pegarules.priv.tracer.RuleTraceInfo oTraceInfo_0 = new com.pega.pegarules.priv.tracer.RuleTraceInfo("RULE-NAVIGATION DATA-PORTAL PZOPENPORTAL #20200512T155845.000 GMT", "Data-Portal pzOpenPortal", "Pega-Desktop", "08-05-01", "20200512T155845.000 GMT");
}
