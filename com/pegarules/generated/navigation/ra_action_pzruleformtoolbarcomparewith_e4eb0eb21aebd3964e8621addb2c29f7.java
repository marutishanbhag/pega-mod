...
import java.util.*;
public class ra_action_pzruleformtoolbarcomparewith_e4eb0eb21aebd3964e8621addb2c29f7  extends com.pega.pegarules.priv.AbstractFUASupport implements Activity, FUASupport {
	private static final LogHelper  oLog = new LogHelper("Rule_Navigation.pzRuleFormToolbarCompareWith._baseclass.Action");
	public ra_action_pzruleformtoolbarcomparewith_e4eb0eb21aebd3964e8621addb2c29f7(PegaAPI aContext) {
		tools = aContext;
		pega = aContext;
	}
...
{
/* Instance RULE-NAVIGATION @BASECLASS PZRULEFORMTOOLBARCOMPAREWITH #20180713T135314.085 GMT	Pega-Desktop:08-01-01	<no Circumstance>	<no Circumstance Date>	<no date range>	available=Final	baserule=false */
...
ClipboardPage fOutputPage = null;if (StringUtils.isBlank(tools.getParamValue("deferPagePath")) && !tools.getParamValue("usingPageRef").equals("true")) {
	...
	if(true){
		fOutputPage.putString("pyLabel", tools.getLocalizedTextForString("pyLabel", "pzRuleFormToolbarCompareWith"));
	}else{
		fOutputPage.getProperty("pyLabel").setValue("pzRuleFormToolbarCompareWith");
	}
}
...
private static final String oDependencies [] = {
	...
	"Rule-Navigation:PZRULEFORMTOOLBARCOMPAREWITH"
	};
	...
	private static final DependentRuleInfo[] oDependentRuleInfos = {
		...
		new DependentRuleInfo("PZRULEFORMTOOLBARCOMPAREWITH","Rule-Navigation","@BASECLASS",false,"","Pega-Desktop","08-01-01","RULE-NAVIGATION @BASECLASS PZRULEFORMTOOLBARCOMPAREWITH #20180713T135314.085 GMT","!PZRULEFORMTOOLBARCOMPAREWITH",true,false,"ABSOLUTE_ULTIMATE_STEP_PAGE",1568993533)
	};
...
//	RULE-NAVIGATION @BASECLASS PZRULEFORMTOOLBARCOMPAREWITH #20180713T135314.085 GMT:20180713T135314.085 GMT
...
//	RULE-UTILITY-FUNCTION STRING ISINTEGER #20180713T131441.652 GMT:20180713T131441.652 GMT
	...
	public static String getFUAContentHash() {
		return "583db308ab466a958ca9768c56924db3";
	}
...
/* Method generated for Top Level Node -"" for Navigation Rule :"pzRuleFormToolbarCompareWith" */
...
if( processAction ||(processShortcut && "ID1371751388401000".equals(nodePathArr[0])  && !isNodePathEmpty)){
...
int deferIndex = 0;
int nodeDepth=0;
if(!StringUtils.isBlank(deferLoadPath) && deferPathArr.length>0 && !deferPathArr[deferIndex].equalsIgnoreCase("ID1371751388401000")){
return;}
if(processShortcut &&"ID1371751388401000".equals(nodePathArr[0]) && !isNodePathEmpty){
nodeDepth++;
...
String whenResolved ="pzShowViewVersions";
...
deferLoad = false;
if((!deferLoad && StringUtils.isBlank(deferLoadPath)) || (!StringUtils.isBlank(deferLoadPath) && ( deferIndex>=deferPathArr.length || deferPathArr[deferIndex].equalsIgnoreCase("ID1371751388401000")))){
deferIndex ++;
...
if(processAction||("ID1371746767486000".equals(nodePathArr[currentElement])&& processShortcut && !isNodePathEmpty)){ 
matchFound=true;
...
actionPage2.getProperty("pyChecked").setValue(pega_uiengine_pznavigation.evaluateWhen("pzCurrentRuleFormToolbarVersion",tools.getStepPage().getClassName(),tools));
actionPage2.putString("pyHidden",String.valueOf(hideElement));actionPage2.putString("pyDisabled",String.valueOf(disableElement));actionPage2.putString( "pyName","ID1371746767486000");propertyValue = ".pyLabel";
propertyValue = tools.getParamValue("ApplyToPage")+".pyLabel";if(!isDynamicFieldNameForMenu){actionPage2.putString("pyCaption",tools.getLocalizedTextForString("pyCaption", tools.getProperty(propertyValue).getStringValue()));}else{actionPage2.putString("pyCaption",tools.getLocalizedTextForString(".pyLabel", "\t["+tools.getProperty(tools.getParamValue("ApplyToPage")+".pxObjClass").getStringValue()+".pyLabel]"+tools.getProperty(propertyValue).getStringValue()));}actionPage2.putString("pySummary", tools.getLocalizedTextForString("pySummary", ""));actionPage2.putString("pyBadgeProperty", tools.getLocalizedTextForString("pyBadgeProperty", ""));propertyValue = ".pyMenuTooltip";
propertyValue = tools.getParamValue("ApplyToPage")+".pyMenuTooltip";if(!isDynamicFieldNameForMenu){actionPage2.putString("pyToolTip",tools.getLocalizedTextForString("pyToolTip", tools.getProperty(propertyValue).getStringValue()));}else{actionPage2.putString("pyToolTip",tools.getLocalizedTextForString(".pyMenuTooltip", "\t["+tools.getProperty(tools.getParamValue("ApplyToPage")+".pxObjClass").getStringValue()+".pyMenuTooltip]"+tools.getProperty(propertyValue).getStringValue()));}actionPage2.putString( "pyBadgeFormat","Standard (label)");actionPage2.putString( "pyDeferLoad","");propertyValue = ".pxListSubscript";
propertyValue = tools.getParamValue("ApplyToPage")+".pxListSubscript";actionPage2.putString("pySubscript",tools.getProperty(propertyValue).getStringValue());actionPage2.putString( "pyImage","");actionPage2.putString("pyType","Action");
...
commandPage2.putString( "pyFunctionName","postRuleFormValues");executePage.putPage("pyCustom",commandPage2);
...
commandPage3.putString( "pyActivity","pxLaunchComparePages");commandPage3.putString( "pyUsingPage", tools.getStepPage().getReference());behaviorPage3.putPage("pyActionAPI",commandPage3);
((PegaAPI)tools).getUIEngine().getUIAction(actionPage2.getPage("pyBehaviors(<LAST>)")).register();
...
commandPage4.putString( "pyFunctionName","pega.desktop.rulecompare.show");behaviorPage4.putPage("pyActionAPI",commandPage4);
((PegaAPI)tools).getUIEngine().getUIAction(actionPage2.getPage("pyBehaviors(<LAST>)")).register();
...
private static final com.pega.pegarules.priv.tracer.RuleTraceInfo oTraceInfo_0 = new com.pega.pegarules.priv.tracer.RuleTraceInfo("RULE-NAVIGATION @BASECLASS PZRULEFORMTOOLBARCOMPAREWITH #20180713T135314.085 GMT", "@baseclass pzRuleFormToolbarCompareWith", "Pega-Desktop", "08-01-01", "20180713T135314.085 GMT");
}
