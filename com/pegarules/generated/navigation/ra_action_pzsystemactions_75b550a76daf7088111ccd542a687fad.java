...
import java.util.*;
public class ra_action_pzsystemactions_75b550a76daf7088111ccd542a687fad  extends com.pega.pegarules.priv.AbstractFUASupport implements Activity, FUASupport {
	private static final LogHelper  oLog = new LogHelper("Rule_Navigation.pzSystemActions.Pega_Ext_AdminStudio_Explorer_Settings_System.Action");
	public ra_action_pzsystemactions_75b550a76daf7088111ccd542a687fad(PegaAPI aContext) {
		tools = aContext;
		pega = aContext;
	}
...
{
/* Instance RULE-NAVIGATION PEGA-EXT-ADMINSTUDIO-EXPLORER-SETTINGS-SYSTEM PZSYSTEMACTIONS #20180713T135317.189 GMT	Pega-Desktop:08-01-01	<no Circumstance>	<no Circumstance Date>	<no date range>	available=Final	baserule=false */
...
ClipboardPage fOutputPage = null;if (StringUtils.isBlank(tools.getParamValue("deferPagePath")) && !tools.getParamValue("usingPageRef").equals("true")) {
	...
	if(true){
		fOutputPage.putString("pyLabel", tools.getLocalizedTextForString("pyLabel", "pzSystemActions"));
	}else{
		fOutputPage.getProperty("pyLabel").setValue("pzSystemActions");
	}
}
...
private static final String oDependencies [] = {
	...
	"Rule-Navigation:PZSYSTEMACTIONS"
	};
	...
	private static final DependentRuleInfo[] oDependentRuleInfos = {
		...
		new DependentRuleInfo("PZSYSTEMACTIONS","Rule-Navigation","PEGA-EXT-ADMINSTUDIO-EXPLORER-SETTINGS-SYSTEM",false,"","Pega-Desktop","08-01-01","RULE-NAVIGATION PEGA-EXT-ADMINSTUDIO-EXPLORER-SETTINGS-SYSTEM PZSYSTEMACTIONS #20180713T135317.189 GMT","!PZSYSTEMACTIONS",true,false,"ABSOLUTE_ULTIMATE_STEP_PAGE",-827564433)
	};
...
//	RULE-NAVIGATION PEGA-EXT-ADMINSTUDIO-EXPLORER-SETTINGS-SYSTEM PZSYSTEMACTIONS #20180713T135317.189 GMT:20180713T135317.189 GMT
...
//	RULE-UTILITY-FUNCTION UIENGINE PZGETANIMATIONSJSONSTRING--(CLIPBOARDPAGE) #20190417T070159.744 GMT:20190430T131934.330 GMT
	...
	public static String getFUAContentHash() {
		return "6582989740ba661d4c9e10c9f641a1e1";
	}
...
/* Method generated for Top Level Node -"" for Navigation Rule :"pzSystemActions" */
...
if( processAction ||(processShortcut && "ID1524029971108000".equals(nodePathArr[0])  && !isNodePathEmpty)){
...
int deferIndex = 0;
int nodeDepth=0;
if(!StringUtils.isBlank(deferLoadPath) && deferPathArr.length>0 && !deferPathArr[deferIndex].equalsIgnoreCase("ID1524029971108000")){
return;}
if(processShortcut &&"ID1524029971108000".equals(nodePathArr[0]) && !isNodePathEmpty){
nodeDepth++;
...
enableWhenCondition = pega_uiengine_pznavigation.evaluateWhen("pzCanDownloadSystemState",tools.getStepPage().getClassName(),tools);
disableElement = !enableWhenCondition;
if(!disableElement) {
}
actionPage1.putString("pyHidden",String.valueOf(hideElement));actionPage1.putString("pyDisabled",String.valueOf(disableElement));actionPage1.putString( "pyName","ID1524029971108000");actionPage1.putString("pyCaption", tools.getLocalizedTextForString("pyCaption", "Download system state"));actionPage1.putString("pySummary", tools.getLocalizedTextForString("pySummary", ""));actionPage1.putString("pyBadgeProperty", tools.getLocalizedTextForString("pyBadgeProperty", ""));actionPage1.putString("pyToolTip", tools.getLocalizedTextForString("pyToolTip", ""));if(testIDWhen || functionalTestingWhen){actionPage1.putString( "pyAutomationID","201804180139310086497");}actionPage1.putString( "pyBadgeFormat","Standard (label)");actionPage1.putString( "pyDeferLoad","false");actionPage1.putString( "pyImage","");actionPage1.putString("pyType","Action");
...
commandPage1.getProperty("pyPreDataTransform").getPageValue().putString( "pyName","pzInitializeSystemStatePages");commandPage1.putString( "pyUsingPage", "D_pzClusterMembersInformation");commandPage1.getPage("pyPreDataTransform").getProperty("pyDataTransformParams").getPageValue(1).putString( "pyValue","");commandPage1.getPage("pyPreDataTransform").getProperty("pyDataTransformParams").getPageValue(1).putString( "pyName","");executePage.putPage("pyRunDataTransform",commandPage1);
...
commandPage2Map.putString("pyActionName","pzSystemStateSection");
commandPage2Map.putString("pyClassName", "Log-System-State-");
...
commandPage2.putString("pyLocalAction", "pzSystemStateSection");}else {
...
commandPage2.putString("pyUsingPage","SystemState");
...
commandPage2.putString("pyCustomTemplateName","pzModalTemplateCustom");
...
boolean matchFound=true;
int currentElement=0;
if( processAction ||(processShortcut && "ID1523621848313000".equals(nodePathArr[0])  && !isNodePathEmpty)){
String uniqueSuffix = new Long(tools.getDateTimeUtils().currentTimeUnique()).toString();
String commonPrefix = "Nav_";
boolean showWhenCondition = true;
boolean isActiveNavItem = false;
...
int deferIndex = 0;
int nodeDepth=0;
if(!StringUtils.isBlank(deferLoadPath) && deferPathArr.length>0 && !deferPathArr[deferIndex].equalsIgnoreCase("ID1523621848313000")){
return;}
if(processShortcut &&"ID1523621848313000".equals(nodePathArr[0]) && !isNodePathEmpty){
nodeDepth++;
if(nodeDepth==nodePathArr.length){
nodePathValidated=true;
}
...
commandPage3.putString( "pyTarget","otherSection");commandPage3.putString( "pyUsingPage","");commandPage3.putString( "pyActivity","");commandPage3.putString( "pySection","pzSystemDetailsMain");commandPage3.putString( "pyDisableSubmit","false");commandPage3.getProperty("pyPreDataTransform").getPageValue().putString( "pyName","pzRemoveNodesList");commandPage3.getProperty("pyActivityParameters").getPageValue(1).putString( "pyValue","");commandPage3.getProperty("pyActivityParameters").getPageValue(1).putString( "pyName","");commandPage3.getProperty("pyActivityParameters").getPageValue(1).putString( "pxObjClass","Embed-NameValuePair");commandPage3.getPage("pyPreDataTransform").getProperty("pyDataTransformParams").getPageValue(1).putString( "pyValue","");commandPage3.getPage("pyPreDataTransform").getProperty("pyDataTransformParams").getPageValue(1).putString( "pyName","");executePage.putPage("pyRefresh",commandPage3);
...
private static final com.pega.pegarules.priv.tracer.RuleTraceInfo oTraceInfo_0 = new com.pega.pegarules.priv.tracer.RuleTraceInfo("RULE-NAVIGATION PEGA-EXT-ADMINSTUDIO-EXPLORER-SETTINGS-SYSTEM PZSYSTEMACTIONS #20180713T135317.189 GMT", "Pega-Ext-AdminStudio-Explorer-Settings-System pzSystemActions", "Pega-Desktop", "08-01-01", "20180713T135317.189 GMT");
}
