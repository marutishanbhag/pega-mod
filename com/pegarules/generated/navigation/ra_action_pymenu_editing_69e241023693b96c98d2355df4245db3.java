...
import java.util.*;
public class ra_action_pymenu_editing_69e241023693b96c98d2355df4245db3  extends com.pega.pegarules.priv.AbstractFUASupport implements Activity, FUASupport {
	private static final LogHelper  oLog = new LogHelper("Rule_Navigation.pyMenu_Editing.Data_MO.Action");
	public ra_action_pymenu_editing_69e241023693b96c98d2355df4245db3(PegaAPI aContext) {
		tools = aContext;
		pega = aContext;
	}
...
{
/* Instance RULE-NAVIGATION DATA-MO PYMENU_EDITING #20180713T140946.778 GMT	Pega-ProcessArchitect:08-01-01	<no Circumstance>	<no Circumstance Date>	<no date range>	available=Yes	baserule=false */
...
ClipboardPage fOutputPage = null;if (StringUtils.isBlank(tools.getParamValue("deferPagePath")) && !tools.getParamValue("usingPageRef").equals("true")) {
	...
	if(true){
		fOutputPage.putString("pyLabel", tools.getLocalizedTextForString("pyLabel", "Contextual Editing Menu"));
	}else{
		fOutputPage.getProperty("pyLabel").setValue("Contextual Editing Menu");
	}
}
...
private static final String oDependencies [] = {
	...
	"Rule-Navigation:PYMENU_EDITING"
	};
	...
	private static final DependentRuleInfo[] oDependentRuleInfos = {
		...
		new DependentRuleInfo("PYMENU_EDITING","Rule-Navigation","DATA-MO",false,"","Pega-ProcessArchitect","08-01-01","RULE-NAVIGATION DATA-MO PYMENU_EDITING #20180713T140946.778 GMT","!PYMENU_EDITING",true,false,"ABSOLUTE_ULTIMATE_STEP_PAGE",262554307)
	};
...
//	RULE-NAVIGATION DATA-MO PYMENU_EDITING #20180713T140946.778 GMT:20180713T140946.778 GMT
...
//	RULE-UTILITY-FUNCTION STRING ISINTEGER #20180713T131441.652 GMT:20180713T131441.652 GMT
	...
	public static String getFUAContentHash() {
		return "3dc82c3d3881a08a00a6367b3e8f4c74";
	}
...
/* Method generated for Top Level Node -"" for Navigation Rule :"Contextual Editing Menu" */
...
if( processAction ||(processShortcut && "ID1369751988179000".equals(nodePathArr[0])  && !isNodePathEmpty)){
...
int deferIndex = 0;
int nodeDepth=0;
if(!StringUtils.isBlank(deferLoadPath) && deferPathArr.length>0 && !deferPathArr[deferIndex].equalsIgnoreCase("ID1369751988179000")){
return;}
if(processShortcut &&"ID1369751988179000".equals(nodePathArr[0]) && !isNodePathEmpty){
nodeDepth++;
...
actionPage1.putString("pyHidden",String.valueOf(hideElement));actionPage1.putString("pyDisabled",String.valueOf(disableElement));actionPage1.putString( "pyName","ID1369751988179000");actionPage1.putString( "pyImageSource","styleclass");actionPage1.putString( "pyIconStyle","MxGraphViewerCtxMenuCut");actionPage1.putString("pyCaption", tools.getLocalizedTextForString("pyCaption", "Cut"));actionPage1.putString("pySummary", tools.getLocalizedTextForString("pySummary", ""));actionPage1.putString("pyBadgeProperty", tools.getLocalizedTextForString("pyBadgeProperty", ""));actionPage1.putString("pyToolTip", tools.getLocalizedTextForString("pyToolTip", ""));if(testIDWhen || functionalTestingWhen){actionPage1.putString( "pyAutomationID","");}actionPage1.putString( "pyBadgeFormat","Standard (label)");actionPage1.putString( "pyDeferLoad","");actionPage1.putString( "pyImage","MxGraphViewerCtxMenuCut");actionPage1.putString("pyType","Action");
...
commandPage1.getProperty("pyParameters").getPageValue(1).putString( "pyValue","cut");commandPage1.getProperty("pyParameters").getPageValue(1).putString( "pyName","");commandPage1.putString( "pyFunctionName","ViewerManager.executeAction");executePage.putPage("pyCustom",commandPage1);
...
boolean matchFound=true;
int currentElement=0;
if( processAction ||(processShortcut && "ID1346897998573003".equals(nodePathArr[0])  && !isNodePathEmpty)){
String uniqueSuffix = new Long(tools.getDateTimeUtils().currentTimeUnique()).toString();
String commonPrefix = "Nav_";
boolean showWhenCondition = true;
boolean isActiveNavItem = false;
...
int deferIndex = 0;
int nodeDepth=0;
if(!StringUtils.isBlank(deferLoadPath) && deferPathArr.length>0 && !deferPathArr[deferIndex].equalsIgnoreCase("ID1346897998573003")){
return;}
if(processShortcut &&"ID1346897998573003".equals(nodePathArr[0]) && !isNodePathEmpty){
nodeDepth++;
if(nodeDepth==nodePathArr.length){
nodePathValidated=true;
}
...
actionPage2.putString("pyHidden",String.valueOf(hideElement));actionPage2.putString("pyDisabled",String.valueOf(disableElement));actionPage2.putString( "pyName","ID1346897998573003");actionPage2.putString( "pyImageSource","styleclass");actionPage2.putString( "pyIconStyle","MxGraphViewerCtxMenuCopy");actionPage2.putString("pyCaption", tools.getLocalizedTextForString("pyCaption", "Copy"));actionPage2.putString("pySummary", tools.getLocalizedTextForString("pySummary", ""));actionPage2.putString("pyBadgeProperty", tools.getLocalizedTextForString("pyBadgeProperty", ""));actionPage2.putString("pyToolTip", tools.getLocalizedTextForString("pyToolTip", ""));if(testIDWhen || functionalTestingWhen){actionPage2.putString( "pyAutomationID","");}actionPage2.putString( "pyBadgeFormat","Standard (label)");actionPage2.putString( "pyDeferLoad","");actionPage2.putString( "pyImage","MxGraphViewerCtxMenuCopy");actionPage2.putString("pyType","Action");
...
commandPage2.getProperty("pyParameters").getPageValue(1).putString( "pyValue","copy");commandPage2.getProperty("pyParameters").getPageValue(1).putString( "pyName","");commandPage2.putString( "pyFunctionName","ViewerManager.executeAction");executePage.putPage("pyCustom",commandPage2);
...
boolean matchFound=true;
int currentElement=0;
if( processAction ||(processShortcut && "ID1346897998573004".equals(nodePathArr[0])  && !isNodePathEmpty)){
String uniqueSuffix = new Long(tools.getDateTimeUtils().currentTimeUnique()).toString();
String commonPrefix = "Nav_";
boolean showWhenCondition = true;
boolean isActiveNavItem = false;
...
int deferIndex = 0;
int nodeDepth=0;
if(!StringUtils.isBlank(deferLoadPath) && deferPathArr.length>0 && !deferPathArr[deferIndex].equalsIgnoreCase("ID1346897998573004")){
return;}
if(processShortcut &&"ID1346897998573004".equals(nodePathArr[0]) && !isNodePathEmpty){
nodeDepth++;
if(nodeDepth==nodePathArr.length){
nodePathValidated=true;
}
...
commandPage3.getProperty("pyParameters").getPageValue(1).putString( "pyValue","remove");commandPage3.getProperty("pyParameters").getPageValue(1).putString( "pyName","");commandPage3.putString( "pyFunctionName","ViewerManager.executeAction");executePage.putPage("pyCustom",commandPage3);
...
private static final com.pega.pegarules.priv.tracer.RuleTraceInfo oTraceInfo_0 = new com.pega.pegarules.priv.tracer.RuleTraceInfo("RULE-NAVIGATION DATA-MO PYMENU_EDITING #20180713T140946.778 GMT", "Data-MO pyMenu_Editing", "Pega-ProcessArchitect", "08-01-01", "20180713T140946.778 GMT");
}
