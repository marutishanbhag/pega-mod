...
import java.util.*;
public class ra_action_pzruleformtoolbarrunmenu_246323f82569b881d2e39801af4f3010  extends com.pega.pegarules.priv.AbstractFUASupport implements Activity, FUASupport {
	private static final LogHelper  oLog = new LogHelper("Rule_Navigation.pzRuleFormToolbarRunMenu._baseclass.Action");
	public ra_action_pzruleformtoolbarrunmenu_246323f82569b881d2e39801af4f3010(PegaAPI aContext) {
		tools = aContext;
		pega = aContext;
	}
...
{
/* Instance RULE-NAVIGATION @BASECLASS PZRULEFORMTOOLBARRUNMENU #20200908T072115.454 GMT	Pega-Desktop:08-06-01	<no Circumstance>	<no Circumstance Date>	<no date range>	available=Final	baserule=false */
...
ClipboardPage fOutputPage = null;if (StringUtils.isBlank(tools.getParamValue("deferPagePath")) && !tools.getParamValue("usingPageRef").equals("true")) {
	...
	if(false){
		fOutputPage.putString("pyLabel", tools.getLocalizedTextForString("pyLabel", "pzRuleFormToolbarRunMenu"));
	}else{
		fOutputPage.getProperty("pyLabel").setValue("pzRuleFormToolbarRunMenu");
	}
}
...
private static final String oDependencies [] = {
	...
	"Rule-Navigation:PZRULEFORMTOOLBARRUNMENU"
	};
	...
	private static final DependentRuleInfo[] oDependentRuleInfos = {
		...
		new DependentRuleInfo("PZRULEFORMTOOLBARRUNMENU","Rule-Navigation","@BASECLASS",false,"","Pega-Desktop","08-06-01","RULE-NAVIGATION @BASECLASS PZRULEFORMTOOLBARRUNMENU #20200908T072115.454 GMT","!PZRULEFORMTOOLBARRUNMENU",true,false,"ABSOLUTE_ULTIMATE_STEP_PAGE",-1864673291)
	};
...
//	RULE-NAVIGATION @BASECLASS PZRULEFORMTOOLBARRUNMENU #20200908T072115.454 GMT:20200908T072115.454 GMT
...
//	RULE-UTILITY-FUNCTION UIENGINE PZGETANIMATIONSJSONSTRING--(CLIPBOARDPAGE) #20190417T070159.744 GMT:20190430T131934.330 GMT
	...
	public static String getFUAContentHash() {
		return "9d70ba81dabfd237e0c542949b989eef";
	}
...
/* Method generated for Top Level Node -"" for Navigation Rule :"pzRuleFormToolbarRunMenu" */
...
if( processAction ||(processShortcut && "ID1375179748253000".equals(nodePathArr[0])  && !isNodePathEmpty)){
...
int deferIndex = 0;
int nodeDepth=0;
if(!StringUtils.isBlank(deferLoadPath) && deferPathArr.length>0 && !deferPathArr[deferIndex].equalsIgnoreCase("ID1375179748253000")){
return;}
if(processShortcut &&"ID1375179748253000".equals(nodePathArr[0]) && !isNodePathEmpty){
nodeDepth++;
...
String whenResolved ="pzRuleFormToolbarNonHarnessRun";
...
actionPage1.putString("pyHidden",String.valueOf(hideElement));actionPage1.putString("pyDisabled",String.valueOf(disableElement));actionPage1.putString( "pyName","ID1375179748253000");actionPage1.putString( "pyImageSource","styleclass");actionPage1.putString( "pyIconStyle","hotkey-ruleform-run");actionPage1.putString( "pyCaption","Run");actionPage1.putString( "pySummary","");actionPage1.putString( "pyBadgeProperty","");actionPage1.putString( "pyToolTip","");if(testIDWhen || functionalTestingWhen){actionPage1.putString( "pyAutomationID","202009070721540186163");}actionPage1.putString( "pyBadgeFormat","Standard (label)");actionPage1.putString( "pyDeferLoad","");actionPage1.putString( "pyImage","hotkey-ruleform-run");if (!hideElement) { 
...
propertyValue = tools.getParamValue("ApplyToPage")+".pyLabel";commandPage1.putString("pyWindowName",tools.getProperty(propertyValue).getStringValue());commandPage1.putString( "pyOptions","height=950,width=750,location=0,menubar=0,toolbar=0,status=0,resizable=1,location=0,scrollbars=1");commandPage1.putString( "pyReplace","false");commandPage1.putString( "pyUseAlternateUrlBase","false");commandPage1.putString( "pyNoEncodingUrl","");commandPage1.putString( "pyUrl",tools.findPage("pxThread").getString("pxReqURI"));commandPage1 .putString( "pyActivity", tools.getProperty(".pxObjClass").getStringValue() + ".WBRun");
 propertyValue = ".pzInsKey";
propertyValue = tools.getParamValue("ApplyToPage")+".pzInsKey";commandPage1.getProperty("pyQueryString").getPageValue(1).putString("pyValue",tools.getProperty(propertyValue).getStringValue());commandPage1.getProperty("pyQueryString").getPageValue(1).putString( "pyName","InsHandle");executePage.putPage("pyOpenUrlInWindow",commandPage1);
...
boolean matchFound=true;
int currentElement=0;
if( processAction ||(processShortcut && "ID1386014963583000".equals(nodePathArr[0])  && !isNodePathEmpty)){
String uniqueSuffix = new Long(tools.getDateTimeUtils().currentTimeUnique()).toString();
String commonPrefix = "Nav_";
boolean showWhenCondition = true;
boolean isActiveNavItem = false;
...
int deferIndex = 0;
int nodeDepth=0;
if(!StringUtils.isBlank(deferLoadPath) && deferPathArr.length>0 && !deferPathArr[deferIndex].equalsIgnoreCase("ID1386014963583000")){
return;}
if(processShortcut &&"ID1386014963583000".equals(nodePathArr[0]) && !isNodePathEmpty){
nodeDepth++;
if(nodeDepth==nodePathArr.length){
nodePathValidated=true;
}
...
showWhenCondition = true;
String whenResolved ="pzRuleFormToolbarHarnessRun";
if (!whenResolved.isEmpty()) {
try{showWhenCondition = pega_uiengine_pznavigation.evaluateWhen(whenResolved,tools.getStepPage().getClassName(),tools);
}catch(Exception e){oLog.error("Failed to resolve when rule:" + e);}
}
...
actionPage2.putString("pyHidden",String.valueOf(hideElement));actionPage2.putString("pyDisabled",String.valueOf(disableElement));actionPage2.putString( "pyName","ID1386014963583000");actionPage2.putString( "pyImageSource","styleclass");actionPage2.putString( "pyIconStyle","hotkey-ruleform-run");actionPage2.putString( "pyCaption","Run");actionPage2.putString( "pySummary","");actionPage2.putString( "pyBadgeProperty","");actionPage2.putString( "pyToolTip","");if(testIDWhen || functionalTestingWhen){actionPage2.putString( "pyAutomationID","202009070721540197244");}actionPage2.putString( "pyBadgeFormat","Standard (label)");actionPage2.putString( "pyDeferLoad","");actionPage2.putString( "pyImage","hotkey-ruleform-run");if (!hideElement) { 
...
commandPage2.putString( "pyTarget","popup");commandPage2.putString( "pyTitle","");commandPage2.putString( "pyActivityClass","Pega-RunRecord");commandPage2.putString( "pyHarnessName","pxRunRecord");commandPage2.putString( "pyReadOnly","No");commandPage2.putString( "pyKey","");commandPage2.putString( "pyActivity","pzRunRecordInitializeWrapper");commandPage2.getProperty("pyPreDataTransform").getPageValue().putString( "pyName","");commandPage2.putString( "pyUsingPage","RunRecordWindow");commandPage2.putString( "pyElementName","");commandPage2.putString( "pyTargetElement","");commandPage2.putString( "pySelectedMobileTransitionType","");commandPage2.putString( "pyActivityClass","Pega-RunRecord");if(tools !=null && tools.getPrimaryPage() != null){  strPageName = tools.getPrimaryPage().getReference();}commandPage2.putString("pzPrimaryPage",strPageName);propertyValue = ".pzInsKey";
propertyValue = tools.getParamValue("ApplyToPage")+".pzInsKey";commandPage2.putString("pyWindowName",tools.getProperty(propertyValue).getStringValue());commandPage2.putString( "pyWidth","1000");commandPage2.putString( "pyHeight","700");propertyValue = ".pzInsKey";
propertyValue = tools.getParamValue("ApplyToPage")+".pzInsKey";commandPage2.getProperty("pyActivityParams").getPageValue(1).putString("pyValue",tools.getProperty(propertyValue).getStringValue());commandPage2.getProperty("pyActivityParams").getPageValue(1).putString( "pyName","InsHandle");commandPage2.getPage("pyPreDataTransform").getProperty("pyDataTransformParams").getPageValue(1).putString( "pyValue","");commandPage2.getPage("pyPreDataTransform").getProperty("pyDataTransformParams").getPageValue(1).putString( "pyName","");commandPage2.putString( "pyAlwaysRenderFromServer","");executePage.putPage("pyShowHarness",commandPage2);
...
boolean matchFound=true;
int currentElement=0;
if( processAction ||(processShortcut && "ID1389027205575000".equals(nodePathArr[0])  && !isNodePathEmpty)){
String uniqueSuffix = new Long(tools.getDateTimeUtils().currentTimeUnique()).toString();
String commonPrefix = "Nav_";
boolean showWhenCondition = true;
boolean isActiveNavItem = false;
...
int deferIndex = 0;
int nodeDepth=0;
if(!StringUtils.isBlank(deferLoadPath) && deferPathArr.length>0 && !deferPathArr[deferIndex].equalsIgnoreCase("ID1389027205575000")){
return;}
if(processShortcut &&"ID1389027205575000".equals(nodePathArr[0]) && !isNodePathEmpty){
nodeDepth++;
if(nodeDepth==nodePathArr.length){
nodePathValidated=true;
}
...
showWhenCondition = true;
String whenResolved ="pzRuleFormToolbarHarnessRunDisabled";
if (!whenResolved.isEmpty()) {
try{showWhenCondition = pega_uiengine_pznavigation.evaluateWhen(whenResolved,tools.getStepPage().getClassName(),tools);
}catch(Exception e){oLog.error("Failed to resolve when rule:" + e);}
}
...
behaviorPage3.putString("pyAction","localAction");ClipboardPage commandPage3 = tools.createPage("Embed-DesktopAPI-OpenModalWindow","commandType");

StringMap commandPage3Map = new HashStringMap();
commandPage3Map.putString("pxObjClass", "Rule-Obj-FlowAction");
commandPage3Map.putString("pyActionName","pzRuleNotAvailable");
commandPage3Map.putString("pyClassName",tools.getProperty("pxObjClass").getStringValue());
ParameterPage commandPage3ParamPage = new ParameterPage();
commandPage3ParamPage.putString("pyFlowActionPurpose", "GETSTREAMTYPE");
try{
tools.doAction(commandPage3Map, tools.getStepPage(), commandPage3ParamPage);
}catch(Exception e){
oLog.error(e.getMessage());
}
String commandPage3Type = commandPage3ParamPage.getString("StreamType"); commandPage3ParamPage.putString("pyFlowActionPurpose", "CHECKSECURITY");
try{
tools.doAction(commandPage3Map, tools.getStepPage(), commandPage3ParamPage);
}catch(Exception e){
oLog.error(e.getMessage());
}
String bcommandPage3HasPrivilege = commandPage3ParamPage.getString("CheckSecurityRetVal"); commandPage3ParamPage.putString("pyFlowActionPurpose", "GETLABEL");
try{
tools.doAction(commandPage3Map, tools.getStepPage(), commandPage3ParamPage);
}catch(Exception e){
oLog.error(e.getMessage());
}
ClipboardPage commandPage3targetAnimation = tools.createPage("","");
 commandPage3targetAnimation.putString("pyLaunch","anim-null");
 commandPage3targetAnimation.putString("pyDismiss","anim-null");
ClipboardPage commandPage3newTargetAnimation = tools.createPage("","");
 commandPage3newTargetAnimation.putString("pyValue","%7B%22isCustomMobileAnim%22%3A%22true%22%2C%22desktop%22%3A%7B%22reveal%22%3A%7B%22ease%22%3A%22standard%22%2C%22effect%22%3A%22none%22%2C%22speed%22%3A%22default%22%7D%2C%22isCustomDismiss%22%3A%22false%22%7D%2C%22mobile%22%3A%7B%22reveal%22%3A%7B%22ease%22%3A%22standard%22%2C%22effect%22%3A%22anim-offbottom-open%22%2C%22speed%22%3A%22default%22%7D%2C%22isCustomDismiss%22%3A%22false%22%7D%7D");
String commandPage3FALabel= commandPage3ParamPage.getString("pzFALabel");
 if(commandPage3Type.equals("")) {commandPage3Type = "Rule-Obj-FlowAction";}if("true".equals(bcommandPage3HasPrivilege )){
commandPage3.putString("pyLocalAction", "pzRuleNotAvailable");}else {
commandPage3.putString("pyLocalAction", "pyLocalActionPrivilegeError");
commandPage3.putString("pyActionPrivilegeError",tools.getLocalizedTextForString("pyMessageLabel","pzFlowActionAuthorization"));} 
commandPage3.putString("pyStreamType", commandPage3Type);
commandPage3.putString("pyFALabel", commandPage3FALabel);
commandPage3.putString("pyClassName", commandPage3Map.getString("pyClassName"));
commandPage3.putString("pyUsingPage","");
commandPage3.putString("pyModalDialog","");
commandPage3.putString("pyLATarget","overlay");
commandPage3.putString("pyModalFullScreen","true");
commandPage3.putPage("pyAnimations(DesktopReveal)",commandPage3newTargetAnimation);
commandPage3.putString("pyFormatStyle","");
commandPage3.putString("pyCustomTemplateName","");
commandPage3.putString("pyDisableOverlayClickAway","false");
commandPage3.putString("pyShowOverlayInCenter","false");executePage.putPage("pyOpenModalWindow",commandPage3);
...
showWhenCondition = pega_uiengine_pznavigation.evaluateWhen("pzIsDataFlowSupported",tools.getStepPage().getClassName(),tools);
hideElement = !showWhenCondition;
if (!hideElement) { 
currentDeferPath="";
...
pega_uiengine_pznavigation.pzProcessNavigationNode("@baseclass","pzRuleFormToolbarDataFlowRunMenu",commonPrefix,uniqueSuffix,outputPage,tools, tools.getParamValue("PageName"),"true" ,null);
...
private static final com.pega.pegarules.priv.tracer.RuleTraceInfo oTraceInfo_0 = new com.pega.pegarules.priv.tracer.RuleTraceInfo("RULE-NAVIGATION @BASECLASS PZRULEFORMTOOLBARRUNMENU #20200908T072115.454 GMT", "@baseclass pzRuleFormToolbarRunMenu", "Pega-Desktop", "08-06-01", "20200908T072115.454 GMT");
}
