...
import java.util.*;
public class ra_action_pzaddpersonatostage_2b3229663ebf3ac1ea749ed0182c346d  extends com.pega.pegarules.priv.AbstractFUASupport implements Activity, FUASupport {
	private static final LogHelper  oLog = new LogHelper("Rule_Navigation.pzAddPersonaToStage.Embed_Stage.Action");
	public ra_action_pzaddpersonatostage_2b3229663ebf3ac1ea749ed0182c346d(PegaAPI aContext) {
		tools = aContext;
		pega = aContext;
	}
...
{
/* Instance RULE-NAVIGATION EMBED-STAGE PZADDPERSONATOSTAGE #20230618T001753.365 GMT	Pega-ProcessArchitect:08-23-01	<no Circumstance>	<no Circumstance Date>	<no date range>	available=Final	baserule=false */
...
ClipboardPage fOutputPage = null;if (StringUtils.isBlank(tools.getParamValue("deferPagePath")) && !tools.getParamValue("usingPageRef").equals("true")) {
	...
	if(true){
		fOutputPage.putString("pyLabel", tools.getLocalizedTextForString("pyLabel", "pzAddPersonaToStage"));
	}else{
		fOutputPage.getProperty("pyLabel").setValue("pzAddPersonaToStage");
	}
}
...
private static final String oDependencies [] = {
	...
	"Rule-Navigation:PZADDPERSONATOSTAGE"
	};
	...
	private static final DependentRuleInfo[] oDependentRuleInfos = {
		...
		new DependentRuleInfo("PZADDPERSONATOSTAGE","Rule-Navigation","EMBED-STAGE",false,"","Pega-ProcessArchitect","08-23-01","RULE-NAVIGATION EMBED-STAGE PZADDPERSONATOSTAGE #20230618T001753.365 GMT","!PZADDPERSONATOSTAGE",true,false,"ABSOLUTE_ULTIMATE_STEP_PAGE",621955354)
	};
...
//	RULE-NAVIGATION EMBED-STAGE PZADDPERSONATOSTAGE #20230618T001753.365 GMT:20230618T001753.365 GMT
...
//	RULE-UTILITY-FUNCTION UIENGINE PZGETANIMATIONSJSONSTRING--(CLIPBOARDPAGE) #20190417T070159.744 GMT:20190430T131934.330 GMT
	...
	public static String getFUAContentHash() {
		return "df4672ea6d8c17c3a2ee2e31bb97c2fe";
	}
...
/* Method generated for Top Level Node -"" for Navigation Rule :"pzAddPersonaToStage" */
...
if( processAction ||(processShortcut && "ID1445590024210000".equals(nodePathArr[0])  && !isNodePathEmpty)){
...
int deferIndex = 0;
int nodeDepth=0;
if(!StringUtils.isBlank(deferLoadPath) && deferPathArr.length>0 && !deferPathArr[deferIndex].equalsIgnoreCase("ID1445590024210000")){
return;}
if(processShortcut &&"ID1445590024210000".equals(nodePathArr[0]) && !isNodePathEmpty){
nodeDepth++;
...
actionPage1.putString("pyHidden",String.valueOf(hideElement));actionPage1.putString("pyDisabled",String.valueOf(disableElement));actionPage1.putString( "pyName","ID1445590024210000");actionPage1.putString("pyCaption", tools.getLocalizedTextForString("pyCaption", "New persona"));actionPage1.putString("pySummary", tools.getLocalizedTextForString("pySummary", ""));actionPage1.putString("pyBadgeProperty", tools.getLocalizedTextForString("pyBadgeProperty", ""));actionPage1.putString("pyToolTip", tools.getLocalizedTextForString("pyToolTip", "New persona"));if(testIDWhen || functionalTestingWhen){actionPage1.putString( "pyAutomationID","201712291419150210390");}actionPage1.putString( "pyBadgeFormat","Standard (label)");actionPage1.putString( "pyDeferLoad","false");actionPage1.putString( "pyImage","");actionPage1.putString("pyType","Action");
...
commandPage1.getProperty("pyPreDataTransform").getPageValue().putString( "pyName","pzFlushAISuggestionsForPersonas");commandPage1.putString( "pyUsingPage", tools.getStepPage().getReference());commandPage1.getPage("pyPreDataTransform").getProperty("pyDataTransformParams").getPageValue(1).putString( "pyValue","");commandPage1.getPage("pyPreDataTransform").getProperty("pyDataTransformParams").getPageValue(1).putString( "pyName","");executePage.putPage("pyRunDataTransform",commandPage1);
...
commandPage2Map.putString("pyActionName","pzCreate");
commandPage2Map.putString("pyClassName", "Rule-Persona");
...
ClipboardPage commandPage2newTargetAnimation = tools.createPage("","");
 commandPage2newTargetAnimation.putString("pyValue","%7B%22mobile%22%3A%7B%22reveal%22%3A%7B%22effect%22%3A%22anim-null%22%7D%2C%22isCustomDismiss%22%3A%22true%22%2C%22dismiss%22%3A%7B%22effect%22%3A%22anim-null%22%7D%7D%7D");
...
commandPage2.putString("pyLocalAction", "pzCreate");}else {
...
commandPage2.putString("pyUsingPage","D_pzPersonaDetails");
...
commandPage2.putString("pyFormatStyle","");
commandPage2.putString("pyCustomTemplateName","pzModalTemplate");
commandPage2.putString("pyDisableOverlayClickAway","false");
commandPage2.putString("pyShowOverlayInCenter","false");behaviorPage2.putPage("pyActionAPI",commandPage2);
((PegaAPI)tools).getUIEngine().getUIAction(actionPage1.getPage("pyBehaviors(<LAST>)")).register();
...
commandPage3.putString( "pyActivity","pzAddNewPersona");commandPage3.putString( "pyUsingPage", tools.getStepPage().getReference());behaviorPage3.putPage("pyActionAPI",commandPage3);
((PegaAPI)tools).getUIEngine().getUIAction(actionPage1.getPage("pyBehaviors(<LAST>)")).register();
...
actionPage4.putString("pyHidden",String.valueOf(hideElement));actionPage4.putString("pyDisabled",String.valueOf(disableElement));actionPage4.putString( "pyName","");actionPage4.putString("pyCaption", tools.getLocalizedTextForString("pyCaption", "------------------"));actionPage4.putString("pySummary", tools.getLocalizedTextForString("pySummary", ""));actionPage4.putString("pyBadgeProperty", tools.getLocalizedTextForString("pyBadgeProperty", ""));actionPage4.putString("pyToolTip", tools.getLocalizedTextForString("pyToolTip", ""));actionPage4.putString( "pyBadgeFormat","Standard (label)");actionPage4.putString( "pyDeferLoad","false");actionPage4.putString( "pyImage","");actionPage4.putString("pyType","Separator");
if(!processShortcut){
}else {
if(!(nodePath.indexOf("/")!=-1))
...
return;}
nodePageList = tools.getIfPresent("D_pxPersonas.pxResults");
...
String actionListParentPage5 = tools.getParamValue("ApplyToPage");	Iterator actionListIter5 = nodePageList.iterator();
	while (actionListIter5.hasNext()) {		iterListResult = (ClipboardProperty)actionListIter5.next();
		...
		if(strChildListPageRef5.lastIndexOf(".") == 0) {
			strChildListPageRef5 = "D_pxPersonas" + strChildListPageRef5;
		}
...
String whenResolved ="pzCanSelectPersona";
...
actionPage5.putString("pyHidden",String.valueOf(hideElement));actionPage5.putString("pyDisabled",String.valueOf(disableElement));actionPage5.putString( "pyName","");actionPage5.putString( "pyImageSource","property");propertyValue = ".pyAvatar";
propertyValue = tools.getParamValue("ApplyToPage")+".pyAvatar";actionPage5.putString("pyIconProperty",tools.getProperty(propertyValue).getStringValue());propertyValue = ".pyLabel";
propertyValue = tools.getParamValue("ApplyToPage")+".pyLabel";if(!isDynamicFieldNameForMenu){actionPage5.putString("pyCaption",tools.getLocalizedTextForString("pyCaption", tools.getProperty(propertyValue).getStringValue()));}else{actionPage5.putString("pyCaption",tools.getLocalizedTextForString(".pyLabel", "\t["+tools.getProperty(tools.getParamValue("ApplyToPage")+".pxObjClass").getStringValue()+".pyLabel]"+tools.getProperty(propertyValue).getStringValue()));}actionPage5.putString("pySummary", tools.getLocalizedTextForString("pySummary", ""));actionPage5.putString("pyBadgeProperty", tools.getLocalizedTextForString("pyBadgeProperty", ""));propertyValue = ".pyLabel";
propertyValue = tools.getParamValue("ApplyToPage")+".pyLabel";if(!isDynamicFieldNameForMenu){actionPage5.putString("pyToolTip",tools.getLocalizedTextForString("pyToolTip", tools.getProperty(propertyValue).getStringValue()));}else{actionPage5.putString("pyToolTip",tools.getLocalizedTextForString(".pyLabel", "\t["+tools.getProperty(tools.getParamValue("ApplyToPage")+".pxObjClass").getStringValue()+".pyLabel]"+tools.getProperty(propertyValue).getStringValue()));}actionPage5.putString( "pyBadgeFormat","Standard (label)");actionPage5.putString( "pyDeferLoad","false");propertyValue = ".pxListSubscript";
propertyValue = tools.getParamValue("ApplyToPage")+".pxListSubscript";actionPage5.putString("pySubscript",tools.getProperty(propertyValue).getStringValue());if(testIDWhen || functionalTestingWhen){if(!"".equals(tools.getProperty(tools.getParamValue("ApplyToPage")+".pxListSubscript").getStringValue())) {actionPage5.putString("pyAutomationID","201712291419140564890" + "_" + tools.getProperty(tools.getParamValue("ApplyToPage")+".pxListSubscript").getStringValue());}else if (!"".equals(tools.getProperty(tools.getParamValue("ApplyToPage")+".pxSubscript").getStringValue())) {actionPage5.putString("pyAutomationID","201712291419140564890" + "_" + tools.getProperty(tools.getParamValue("ApplyToPage")+".pxSubscript").getStringValue());}else {actionPage5.putString( "pyAutomationID","201712291419140564890");}}actionPage5.putString( "pyImage","");if (!hideElement) { 
...
behaviorPage5.putString("pyAction","runActivity");ClipboardPage commandPage5 = tools.createPage("Embed-SelectedContextAPI-RunActivity","commandType");
commandPage5.putString( "pyActivity","pzAddToStage");commandPage5.putString( "pyUsingPage", tools.getStepPage().getReference());commandPage5.getProperty("pyActivityParameters").getPageValue(1).putString( "pyValue","");commandPage5.getProperty("pyActivityParameters").getPageValue(1).putString( "pyName","StagePageReference");commandPage5.getProperty("pyActivityParameters").getPageValue(1).putString( "pxObjClass","Embed-NameValuePair");executePage.putPage("pyRunActivity",commandPage5);
...
behaviorPage6.putString("pyAction","runDataTransform");ClipboardPage commandPage6 = tools.createPage("Embed-SelectedContextAPI-RunDataTransform","commandType");
commandPage6.getProperty("pyPreDataTransform").getPageValue().putString( "pyName","pzResetRelease");commandPage6.putString( "pyUsingPage", "D_pzWorkflowViewTypes");commandPage6.getPage("pyPreDataTransform").getProperty("pyDataTransformParams").getPageValue(1).putString( "pyValue","");commandPage6.getPage("pyPreDataTransform").getProperty("pyDataTransformParams").getPageValue(1).putString( "pyName","");behaviorPage6.putPage("pyActionAPI",commandPage6);
((PegaAPI)tools).getUIEngine().getUIAction(actionPage5.getPage("pyBehaviors(<LAST>)")).register();
...
private static final com.pega.pegarules.priv.tracer.RuleTraceInfo oTraceInfo_0 = new com.pega.pegarules.priv.tracer.RuleTraceInfo("RULE-NAVIGATION EMBED-STAGE PZADDPERSONATOSTAGE #20230618T001753.365 GMT", "Embed-Stage pzAddPersonaToStage", "Pega-ProcessArchitect", "08-23-01", "20230618T001753.365 GMT");
}
