...
import java.util.*;
public class ra_action_pycreatecasemenu_e0eb760affa0c0dc2275c170d9e69deb  extends com.pega.pegarules.priv.AbstractFUASupport implements Activity, FUASupport {
	private static final LogHelper  oLog = new LogHelper("Rule_Navigation.pyCreateCaseMenu.Data_Portal.Action");
	public ra_action_pycreatecasemenu_e0eb760affa0c0dc2275c170d9e69deb(PegaAPI aContext) {
		tools = aContext;
		pega = aContext;
	}
...
{
/* Instance RULE-NAVIGATION DATA-PORTAL PYCREATECASEMENU #20210506T204031.359 GMT	Theme-Cosmos:04-01-01	<no Circumstance>	<no Circumstance Date>	<no date range>	available=Yes	baserule=false */
...
ClipboardPage fOutputPage = null;if (StringUtils.isBlank(tools.getParamValue("deferPagePath")) && !tools.getParamValue("usingPageRef").equals("true")) {
	...
	if(true){
		fOutputPage.putString("pyLabel", tools.getLocalizedTextForString("pyLabel", "Create New..."));
	}else{
		fOutputPage.getProperty("pyLabel").setValue("Create New...");
	}
}
...
private static final String oDependencies [] = {
	...
	"Rule-Navigation:PYCREATECASEMENU"
	};
	...
	private static final DependentRuleInfo[] oDependentRuleInfos = {
		...
		new DependentRuleInfo("PYCREATECASEMENU","Rule-Navigation","DATA-PORTAL",false,"","Theme-Cosmos","04-01-01","RULE-NAVIGATION DATA-PORTAL PYCREATECASEMENU #20210506T204031.359 GMT","!PYCREATECASEMENU",true,false,"ABSOLUTE_ULTIMATE_STEP_PAGE",1532038236)
	};
...
//	RULE-NAVIGATION DATA-PORTAL PYCREATECASEMENU #20210506T204031.359 GMT:20210506T204031.359 GMT
...
//	RULE-UTILITY-FUNCTION STRING ISINTEGER #20180713T131441.652 GMT:20180713T131441.652 GMT
	...
	public static String getFUAContentHash() {
		return "bee933c2437da1a7c9aaab184e459e47";
	}
...
/* Method generated for Top Level Node -"" for Navigation Rule :"Create New..." */
...
if( processAction ||(processShortcut && "ID1472744187645000".equals(nodePathArr[0])  && !isNodePathEmpty)){
...
int deferIndex = 0;
int nodeDepth=0;
if(!StringUtils.isBlank(deferLoadPath) && deferPathArr.length>0 && !deferPathArr[deferIndex].equalsIgnoreCase("ID1472744187645000")){
return;}
if(processShortcut &&"ID1472744187645000".equals(nodePathArr[0]) && !isNodePathEmpty){
nodeDepth++;
...
actionPage1.putString("pyHidden",String.valueOf(hideElement));actionPage1.putString("pyDisabled",String.valueOf(disableElement));actionPage1.putString( "pyName","ID1472744187645000");actionPage1.putString( "pyImageSource","styleclass");actionPage1.putString( "pyIconStyle","pi pi-plus");actionPage1.putString("pyCaption", tools.getLocalizedTextForString("pyCaption", "Create"));actionPage1.putString("pySummary", tools.getLocalizedTextForString("pySummary", ""));actionPage1.putString("pyBadgeProperty", tools.getLocalizedTextForString("pyBadgeProperty", ""));actionPage1.putString("pyToolTip", tools.getLocalizedTextForString("pyToolTip", ""));if(testIDWhen || functionalTestingWhen){actionPage1.putString( "pyAutomationID","201812201359010458611");}actionPage1.putString( "pyBadgeFormat","Standard (label)");actionPage1.putString( "pyDeferLoad","false");actionPage1.putString( "pyImage","pi pi-plus");actionPage1.putString("pyType","Action");
...
deferLoad = false;
if((!deferLoad && StringUtils.isBlank(deferLoadPath)) || (!StringUtils.isBlank(deferLoadPath) && ( deferIndex>=deferPathArr.length || deferPathArr[deferIndex].equalsIgnoreCase("ID1472744187645000")))){
deferIndex ++;
...
nodePageList = tools.getIfPresent("D_pzCasesAvailableToCreateForPortal.pxResults");
...
String actionListParentPage2 = tools.getParamValue("ApplyToPage");	Iterator actionListIter2 = nodePageList.iterator();
	while (actionListIter2.hasNext()) {		iterListResult = (ClipboardProperty)actionListIter2.next();
		...
		;
		if(strChildListPageRef2.lastIndexOf(".") == 0) {
			strChildListPageRef2 = "D_pzCasesAvailableToCreateForPortal" + strChildListPageRef2;
		}
...
enableWhenCondition = pega_uiengine_pznavigation.evaluateWhen("pzWhenCasesExist",tools.getStepPage().getClassName(),tools);
disableElement = !enableWhenCondition;
if(!disableElement) {
}
actionPage2.putString("pyHidden",String.valueOf(hideElement));actionPage2.putString("pyDisabled",String.valueOf(disableElement));actionPage2.putString( "pyName","");propertyValue = ".pyLabel";
propertyValue = tools.getParamValue("ApplyToPage")+".pyLabel";if(!isDynamicFieldNameForMenu){actionPage2.putString("pyCaption",tools.getLocalizedTextForString("pyCaption", tools.getProperty(propertyValue).getStringValue()));}else{actionPage2.putString("pyCaption",tools.getLocalizedTextForString(".pyLabel", "\t["+tools.getProperty(tools.getParamValue("ApplyToPage")+".pxObjClass").getStringValue()+".pyLabel]"+tools.getProperty(propertyValue).getStringValue()));}actionPage2.putString("pySummary", tools.getLocalizedTextForString("pySummary", ""));actionPage2.putString("pyBadgeProperty", tools.getLocalizedTextForString("pyBadgeProperty", ""));propertyValue = ".pyLabel";
propertyValue = tools.getParamValue("ApplyToPage")+".pyLabel";if(!isDynamicFieldNameForMenu){actionPage2.putString("pyToolTip",tools.getLocalizedTextForString("pyToolTip", tools.getProperty(propertyValue).getStringValue()));}else{actionPage2.putString("pyToolTip",tools.getLocalizedTextForString(".pyLabel", "\t["+tools.getProperty(tools.getParamValue("ApplyToPage")+".pxObjClass").getStringValue()+".pyLabel]"+tools.getProperty(propertyValue).getStringValue()));}actionPage2.putString( "pyBadgeFormat","Standard (label)");actionPage2.putString( "pyDeferLoad","");propertyValue = ".pxListSubscript";
propertyValue = tools.getParamValue("ApplyToPage")+".pxListSubscript";actionPage2.putString("pySubscript",tools.getProperty(propertyValue).getStringValue());if(testIDWhen || functionalTestingWhen){if(!"".equals(tools.getProperty(tools.getParamValue("ApplyToPage")+".pxListSubscript").getStringValue())) {actionPage2.putString("pyAutomationID","201812201359010337524" + "_" + tools.getProperty(tools.getParamValue("ApplyToPage")+".pxListSubscript").getStringValue());}else if (!"".equals(tools.getProperty(tools.getParamValue("ApplyToPage")+".pxSubscript").getStringValue())) {actionPage2.putString("pyAutomationID","201812201359010337524" + "_" + tools.getProperty(tools.getParamValue("ApplyToPage")+".pxSubscript").getStringValue());}else {actionPage2.putString( "pyAutomationID","201812201359010337524");}}actionPage2.putString( "pyImage","");actionPage2.putString("pyType","Action");
...
propertyValue = tools.getParamValue("ApplyToPage")+".pyClassName";commandPage2.getProperty("pyParameters").getPageValue(1).putString("pyValue",tools.getProperty(propertyValue).getStringValue());commandPage2.getProperty("pyParameters").getPageValue(1).putString( "pyName","ClassName");propertyValue = ".pyFlowType";
propertyValue = tools.getParamValue("ApplyToPage")+".pyFlowType";commandPage2.getProperty("pyParameters").getPageValue(2).putString("pyValue",tools.getProperty(propertyValue).getStringValue());commandPage2.getProperty("pyParameters").getPageValue(2).putString( "pyName","FlowName");commandPage2.getProperty("pyParameters").getPageValue(3).putString( "pyValue","");commandPage2.getProperty("pyParameters").getPageValue(3).putString( "pyName","");propertyValue = ".pySkipNewHarness";
propertyValue = tools.getParamValue("ApplyToPage")+".pySkipNewHarness";commandPage2.getProperty("pyParameters").getPageValue(4).putString("pyValue",tools.getProperty(propertyValue).getStringValue());commandPage2.getProperty("pyParameters").getPageValue(4).putString( "pyName","");propertyValue = ".pyCreatesTemporaryObject";
propertyValue = tools.getParamValue("ApplyToPage")+".pyCreatesTemporaryObject";commandPage2.getProperty("pyParameters").getPageValue(5).putString("pyValue",tools.getProperty(propertyValue).getStringValue());commandPage2.getProperty("pyParameters").getPageValue(5).putString( "pyName","");propertyValue = ".pzCTWithInitializationStage";
propertyValue = tools.getParamValue("ApplyToPage")+".pzCTWithInitializationStage";commandPage2.getProperty("pyParameters").getPageValue(6).putString("pyValue",tools.getProperty(propertyValue).getStringValue());commandPage2.getProperty("pyParameters").getPageValue(6).putString( "pyName","bIsCreateStage");commandPage2.putString( "pyFunctionName","pega.desktop.infinity.createNewWorkInDialogContainer");executePage.putPage("pyCustom",commandPage2);
...
behaviorPage3.putString("pyAction","createWork");ClipboardPage commandPage3 = tools.createPage("Embed-DesktopAPI-CreateNewWork","commandType");
propertyValue = ".pyClassName";
propertyValue = tools.getParamValue("ApplyToPage")+".pyClassName";commandPage3.putString("pyClassName",tools.getProperty(propertyValue).getStringValue());propertyValue = ".pyFlowType";
propertyValue = tools.getParamValue("ApplyToPage")+".pyFlowType";commandPage3.putString("pyFlowName",tools.getProperty(propertyValue).getStringValue());commandPage3.putString( "pyElementName","");commandPage3.putString( "pyTargetElement","");commandPage3.putString( "pyIsTargetMicroDC","");commandPage3.putString( "pyACName","primary");commandPage3.putString( "pySelectedMobileTransitionType","pega.mobile.transitions.NONE");commandPage3.getProperty("pyFlowParameters").getPageValue(1).putString( "pyValue","");commandPage3.getProperty("pyFlowParameters").getPageValue(1).putString( "pyName","");commandPage3.getProperty("pyFlowParameters").getPageValue(1).putString( "pxObjClass","Embed-MethodParams");executePage.putPage("pyCreateNewWork",commandPage3);
...
private static final com.pega.pegarules.priv.tracer.RuleTraceInfo oTraceInfo_0 = new com.pega.pegarules.priv.tracer.RuleTraceInfo("RULE-NAVIGATION DATA-PORTAL PYCREATECASEMENU #20210506T204031.359 GMT", "Data-Portal pyCreateCaseMenu", "Theme-Cosmos", "04-01-01", "20210506T204031.359 GMT");
}
