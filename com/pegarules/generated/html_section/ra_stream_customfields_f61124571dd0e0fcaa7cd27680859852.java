...
/**
 * Builds JSP stream RULE-!CUSTOMFIELDS.
 */
public class ra_stream_customfields_f61124571dd0e0fcaa7cd27680859852 extends StreamBuilderSection implements StreamBuilder, FUASupport, FUAReusable, WebAllowable {
	private static final LogHelper  oLog = new LogHelper("Rule_HTML_Section.CustomFields.Rule_Application.Stream");
private static Map<String,String[]> oCellRuntimeParamsMap = new HashMap<String,String[]>();
private static Map<String, String> oControlPathsMap = new HashMap<String, String>();
private static Map<String, AssemblyParameters> oAssemblyParamsMap = null;
private static final int pzChecksum = -834374402;
 ...
 */
	public ra_stream_customfields_f61124571dd0e0fcaa7cd27680859852(com.pega.pegarules.priv.PegaAPI aContext) {
...
private static String pxUniqueStreamHash = "78b3f7ea510f8f2f1d3abcb957b328020c643188";
...
{
/* Instance RULE-HTML-SECTION RULE- CUSTOMFIELDS #20180713T140905.790 GMT	Pega-ProcessArchitect:08-01-01	<no Circumstance>	<no Circumstance Date>	<no date range>	available=Yes	baserule=false */
pxUniqueStreamHash = "78b3f7ea510f8f2f1d3abcb957b328020c643188";
pzAuto.getUIComponentRuntime().getRuntimeContextTree().pushSection("CustomFields",pxUniqueStreamHash);pxIsNonTemplateStream = hasNonTemplateMethod_1();
...
/*
 * Begin RULE-HTML-SECTION RULE- CUSTOMFIELDS #20180713T140905.790 GMT
 * Set up context.
 */
final String prevRuleKey_1 = pz_CurrentRuleKey;
pz_CurrentRuleKey = "RULE-HTML-SECTION RULE- CUSTOMFIELDS #20180713T140905.790 GMT";final String prevClass_1 = mStreamResponse.getJspBaseClass();
...
/* end RULE-HTML-SECTION RULE- CUSTOMFIELDS #20180713T140905.790 GMT */
}
	} catch (Throwable ex) {
		handleError(ex);
	} finally {
...
private static final String oDependencies [] = {
	...
	"Rule-HTML-Section:CUSTOMFIELDS", 
	...
	private static final DependentRuleInfo[] oDependentRuleInfos = {
		...
		new DependentRuleInfo("CUSTOMFIELDS","Rule-HTML-Section","RULE-",false,"","Pega-ProcessArchitect","08-01-01","RULE-HTML-SECTION RULE- CUSTOMFIELDS #20180713T140905.790 GMT","!CUSTOMFIELDS",true,false,"ABSOLUTE_ULTIMATE_STEP_PAGE",-834374402), 
...
//	RULE-HTML-SECTION RULE- CUSTOMFIELDS #20180713T140905.790 GMT:20180713T140905.790 GMT
...
//	RULE-UTILITY-FUNCTION UIENGINE PZGETANIMATIONSJSONSTRING--(CLIPBOARDPAGE) #20190417T070159.744 GMT:20190430T131934.330 GMT
	...
	public static String getFUAContentHash() {
		return "f4896346d4e51fb4ef5e86f679828f8f";
	}
...
tools.putSaveValue("sectionUniqueID", ((new StringBuilder("SID")).append((((com.pega.pegarules.priv.context.PegaRequestor) tools.getRequestor()).currentTimeUnique()))).toString());
 if(pzAuto.isForOfflineTemplate()) {
  ...
  pzSection.beginSection("CustomFields",1,"Rule-HTML-Section","{'type':'Section','ruleName':'CustomFields','insKey':'RULE-HTML-SECTION RULE- CUSTOMFIELDS #20180713T140905.790 GMT','sectionType':'standard'}");
pzLayout_1();
pzLayout_2();
pzLayout_3();
...
try {
	...
	tools.appendString("<span ");tools.appendString(">");if(pzAuto.getUIComponentRuntime().getRuntimeContextTree().beginNonTemplate(pxUniqueStreamHash+"_8")){boolean bDisabled = false;boolean bOptimizedMarkup = pzAuto.getUIComponentRuntime().isHarnessTemplatized() && pzAuto.isOffline();
		...
		String hrefValue = "#";
		 ...
		 String buttonUID = "name='CustomFields_"+ referenceString+ "_56'";
	 String securedPropValue = null;
		tools.appendString("<button " + pzCell.getTestIdIfEnabled("20140818110124411") + "  data-ctl='Button'  type='button' ");if(!"true".equals(tools.getParamValue("pxAvoidUniqueAttribute")))tools.appendString(buttonUID);
	...
	keyMap.putString("pyActionName", "pzEditAttachments");
	...
	try{
	pzPackageRuntime.packageLocalAction("pzEditAttachments", offlineUsingPage , strLAClassName);
	...
	if(pega_uiengine_offlinesupport.pzIsForOfflineTemplate()){
		try{
			pzPackageRuntime.packageSection("CustomFields",
			...
			if("true".equals(bHasPrivilege )) {
				tools.appendString("[\"processAction\", [\"pzEditAttachments\",\"true\",\":event\",\"\",\""+streamType+"\",\"\",\"\",\"%7B%22mobile%22%3A%7B%22reveal%22%3A%7B%22effect%22%3A%22anim-null%22%7D%2C%22isCustomDismiss%22%3A%22true%22%2C%22dismiss%22%3A%7B%22effect%22%3A%22anim-null%22%7D%7D%7D\",\"true\",\""+strLAClassName+"\",\""+strLAClassName+"\",\"false\",\"false\",\"\",\"\"]");
				} else {
		...
		tools.appendString("]' ");
			...
			String repeatIndx = "";
		if(com.pega.pegarules.jsptags.WhenTag.evaluateWhen(pega, "!pzRuleFormEditable", null, false)){
		...
		boolean hasLineBreaks = false;
		 hasLineBreaks ="Add/Edit attachments".matches("(.*)<([Bb][Rr])\\s*\\/*\\s*>(.*)");
		if(hasLineBreaks) {
		...
		if(hasLineBreaks) {
		tools.appendString("  > "); } checkForScriptTags_1("Add/Edit attachments"); 
...
public int simpleLayoutCell_2( int index) {
	...
	tools.appendString("' " + pega.getUIEngine().getFVInspectorUtils().startCollectingFieldValueData() + pzCell.getInspectorDataDynamic("pxButton",".pyTemplateInputBox",".pySections(3).pySectionBody(1).pyTable.pyRows(1).pyCells(1)") + " class='content-item content-field item-" + Integer.toString(index) + "   '  STRING_TYPE='field' RESERVE_SPACE='false'><div class='content-inner '><div class='field-item " + pzSection.getCustomStyle(false, "","dataValueRead","dataValueWrite") + "'>");
...
public int simpleLayoutTemplateCell_2( int index) {
	...
	pgCells.put("automationId",new StringBuilder(" ").append(pzCell.getTestIdIfEnabled("20140818110124411-Label")).append(" ").toString().replace("data-test-id=","").trim());
...
pzSection.getLayout().setExpandParam("SubSectionCustomFieldsBBB","",false);
}
...
public void simpleLayout_2() {
	...
	if(currentTemplatingStatus != 'Y'){
		...
		tools.appendString("" + pzAuto.getDataUIMetaAttribute("{'type':'Layout','subType':'DYNAMICLAYOUT','pgRef':'.pySections(3)'}") + " class='" + " content  layout-content-default " + "'  ");
	...
	String paramName = "EXPANDEDSubSectionCustomFieldsBBB";
...
public int simpleLayoutCell_1( int index) {
	...
	String baseRef ="";
	generateClientWhenDiv_1();
	tools.appendString("' " + pega.getUIEngine().getFVInspectorUtils().startCollectingFieldValueData() + pzCell.getInspectorDataDynamic("Label",null,".pySections(2).pySectionBody(1).pyTable.pyRows(1).pyCells(1)") + " class='content-item content-label item-" + Integer.toString(index) + "    " + pzSection.getCustomStyle(false, "","standard_dataLabelRead","standard_dataLabelWrite")  + " '  STRING_TYPE='label' RESERVE_SPACE='false'><div class='content-inner '>" + "<div  " + pzCell.getTestIdIfEnabled("20140919070858048294948") + "  class='field-item " + pzSection.getCustomStyle(false, "","dataLabelRead","dataLabelWrite") + "' >");
 ...
 isSecCellReadOnly = !pzAuto.getSection().getControl().isControlEditable(false, null);
	tools.appendString("No attachments are defined</div>" + pega.getUIEngine().getFVInspectorUtils().stopCollectingFieldValueData() + "</div></div>");
	index++;
	return index;
}
...
public int simpleLayoutTemplateCell_1( int index) {
	...
	pgCells.put("automationId",new StringBuilder(" ").append(pzCell.getTestIdIfEnabled("20140919070858048294948")).append(" ").toString().replace("data-test-id=","").trim());
	pgCells.put("spanClass","iconRequired standard_iconRequired");
	pgCells.put("isFlex","false");
	...
	labelName = "No attachments are defined";
	pgCells.put("pyLabelValue",labelName);
	String uiFieldValueMeta = pega.getUIEngine().getFVInspectorUtils().stopCollectingFieldValueData();
	if(!"".equals(uiFieldValueMeta)){
	pgCells.put("stopFV",uiFieldValueMeta);
...
pzSection.getLayout().setExpandParam("SubSectionCustomFieldsBB","",false);
}
...
public void simpleLayout_1() {
	...
	String paramName = "EXPANDEDSubSectionCustomFieldsBB";
	metadataPage.put("paramName", paramName);
	...
	boolean pyVisibility = false;pyVisibility = runtimeContextTree.beginVisibleWhen(" o2&& o1! wxpzHasAttachments o2!= rx.pzInsKey cq", pxUniqueStreamHash+"_6");IUIComponentMetadata pxWhenIdentifiers = pzAuto.getUIComponentRuntime().createMetadata();
...
if (!pzAuto.getUIComponentRuntime().isTemplatingUIEnabled()) {
if(pzAuto.handleEvaluateWhen(" o2&& o1! wxpzHasAttachments o2!= rx.pzInsKey cq","!pzHasAttachments && .pzInsKey!=\'\'", "layout", "visible" )) {pzSetExpandParam_2();
tools.putParamValue("RWClasses","");
if(!pzAuto.getUIComponentRuntime().isTemplatingUIEnabled()) {
...
public void buildSection_1() { 
	...
	pzAuto.getUIComponentRuntime().getRuntimeContextTree().loadSection(pxUniqueStreamHash+"_2","pzDisplayExistingAttachments", secInfo);
}
...
config.put("NewTaskStatus", "pzEditAttachments");
...
public void includeSectionBody_1() { 
	...
	String includedSectionName = "";includedSectionName = "pzDisplayExistingAttachments";if(!"".equals(includedSectionName.trim())){tools.appendString(pzAuto.getSection().getSectionStream(includedSectionName,"",false,""));
	}else{oLog.warn("Section name must be provided.");}}
public void pzSetExpandParam_1() {
pzSection.getLayout().setExpandParam("SubSectionCustomFieldsB","",false);
}
...
String paramName = "EXPANDEDSubSectionCustomFieldsB";
metadataPage.put("paramName", paramName);
String inspectorLayoutData = pzAuto.getDataUIMetaAttribute("{'type':'Layout','subType':'SECTIONINCLUDE','pgRef':'.pySections(1)','clipboardPath':'pzDisplayExistingAttachments'}");
if(!inspectorLayoutData.isEmpty()) {
metadataPage.put("liveUI", inspectorLayoutData);
}
String pyPrefix = null;
...
if(bWrapIncSection){
tools.appendString("<span class='inspector-span' " + pzAuto.getDataUIMetaAttribute("{'type':'Layout','subType':'SECTIONINCLUDE','pgRef':'.pySections(1)','clipboardPath':'pzDisplayExistingAttachments'}") + ">");
}
pzSetExpandParam_1();
tools.putParamValue("RWClasses","");
if(!pzAuto.getUIComponentRuntime().isTemplatingUIEnabled()) {
...
static {
	...
	oStreamProperties_1.put("pzInsKey", "RULE-HTML-SECTION RULE- CUSTOMFIELDS #20180713T140905.790 GMT");
	...
	oStreamProperties_1.put("pyStreamName", "CustomFields");
...
private static final com.pega.pegarules.priv.tracer.RuleTraceInfo oTraceInfo_0 = new com.pega.pegarules.priv.tracer.RuleTraceInfo("RULE-HTML-SECTION RULE- CUSTOMFIELDS #20180713T140905.790 GMT", "Rule- CustomFields", "Pega-ProcessArchitect", "08-01-01", "20180713T140905.790 GMT");
}
