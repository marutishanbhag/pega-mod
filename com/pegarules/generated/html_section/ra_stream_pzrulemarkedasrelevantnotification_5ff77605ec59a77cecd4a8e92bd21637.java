...
/**
 * Builds JSP stream @BASECLASS!PZRULEMARKEDASRELEVANTNOTIFICATION.
 */
public class ra_stream_pzrulemarkedasrelevantnotification_5ff77605ec59a77cecd4a8e92bd21637 extends StreamBuilderSection implements StreamBuilder, FUASupport, FUAReusable, WebAllowable {
	private static final LogHelper  oLog = new LogHelper("Rule_HTML_Section.pzRuleMarkedAsRelevantNotification.Rule_Obj_Property.Stream");
private static Map<String,String[]> oCellRuntimeParamsMap = new HashMap<String,String[]>();
private static Map<String, String> oControlPathsMap = new HashMap<String, String>();
private static Map<String, AssemblyParameters> oAssemblyParamsMap = null;
private static final int pzChecksum = -635031876;
 ...
 */
	public ra_stream_pzrulemarkedasrelevantnotification_5ff77605ec59a77cecd4a8e92bd21637(com.pega.pegarules.priv.PegaAPI aContext) {
...
private static String pxUniqueStreamHash = "e6a8a30f24f662fd9c14c7f03b76b5d6079750f0";
...
{
/* Instance RULE-HTML-SECTION @BASECLASS PZRULEMARKEDASRELEVANTNOTIFICATION #20180713T140806.340 GMT	Pega-ProcessArchitect:08-01-01	<no Circumstance>	<no Circumstance Date>	<no date range>	available=Final	baserule=false */
pxUniqueStreamHash = "e6a8a30f24f662fd9c14c7f03b76b5d6079750f0";
pzAuto.getUIComponentRuntime().getRuntimeContextTree().pushSection("pzRuleMarkedAsRelevantNotification",pxUniqueStreamHash);pxIsNonTemplateStream = hasNonTemplateMethod_1();
...
/*
 * Begin RULE-HTML-SECTION @BASECLASS PZRULEMARKEDASRELEVANTNOTIFICATION #20180713T140806.340 GMT
 * Set up context.
 */
final String prevRuleKey_1 = pz_CurrentRuleKey;
pz_CurrentRuleKey = "RULE-HTML-SECTION @BASECLASS PZRULEMARKEDASRELEVANTNOTIFICATION #20180713T140806.340 GMT";final String prevClass_1 = mStreamResponse.getJspBaseClass();
...
/* end RULE-HTML-SECTION @BASECLASS PZRULEMARKEDASRELEVANTNOTIFICATION #20180713T140806.340 GMT */
}
	} catch (Throwable ex) {
		handleError(ex);
	} finally {
...
private static final String oDependencies [] = {
	...
	"Rule-HTML-Section:PZRULEMARKEDASRELEVANTNOTIFICATION", 
	...
	private static final DependentRuleInfo[] oDependentRuleInfos = {
		...
		new DependentRuleInfo("PZRULEMARKEDASRELEVANTNOTIFICATION","Rule-HTML-Section","@BASECLASS",false,"","Pega-ProcessArchitect","08-01-01","RULE-HTML-SECTION @BASECLASS PZRULEMARKEDASRELEVANTNOTIFICATION #20180713T140806.340 GMT","!PZRULEMARKEDASRELEVANTNOTIFICATION",true,false,"ABSOLUTE_ULTIMATE_STEP_PAGE",-635031876), 
...
//	RULE-HTML-SECTION @BASECLASS PZRULEMARKEDASRELEVANTNOTIFICATION #20180713T140806.340 GMT:20180713T140806.340 GMT
...
//	RULE-UTILITY-FUNCTION STRING STRIPSPECIALCHARS #20180713T131442.797 GMT:20180713T131442.797 GMT
	...
	public static String getFUAContentHash() {
		return "0bb29664cfb53f633ea95a3d1e4d2834";
	}
...
tools.putSaveValue("sectionUniqueID", ((new StringBuilder("SID")).append((((com.pega.pegarules.priv.context.PegaRequestor) tools.getRequestor()).currentTimeUnique()))).toString());
 if(pzAuto.isForOfflineTemplate()) {
  ...
  pzSection.beginSection("pzRuleMarkedAsRelevantNotification",1,"Rule-HTML-Section","{'type':'Section','ruleName':'pzRuleMarkedAsRelevantNotification','insKey':'RULE-HTML-SECTION @BASECLASS PZRULEMARKEDASRELEVANTNOTIFICATION #20180713T140806.340 GMT','sectionType':'standard'}");
pzLayout_4();
...
config.put("paramKeys", "LPContextClass");
config.put("harnessName", "pzLPApplicationInventory");
config.put("action", "Display");
config.put("className", "Pega-Landing-Application");
config.put("model", "pzSetRevRecordClassValid");
config.put("readOnly", "false");
config.put("label", "Application: Inventory");
config.put("levelC", "RelevantRecords");
pega.getUIEngine().getUIAction("openlanding", config).register();
pega.getUIEngine().getUIAction("refresh", null).register();
config = new HashMap<String, String>();
config.put("pyTarget", "thisSection");
config.put("pyActivity", "pzSetConfirmationMessageVisibility");
config.put("StaticActivityParams", "PropertyToSet=pzDisplayMarkedRelevantCnfMsg&ValueToSet=false");
	...
	((PegaStreamAPI)tools).disableDirectStreaming();
		...
		tools.appendString(" data-click='[");if(isBehaviorAdded && true){ tools.appendString(","); }		{com.pega.pegarules.priv.runtime.IActionRequest refreshActionRequest = pzAuto.getActionRequest();
			com.pega.pegarules.priv.runtime.IActionRequest refreshActionRequestD = null;
				...
				refreshActionRequest.registerFixedParameter("StreamList", "pyRuleFormHeaderMessagesInner|Rule-HTML-Section|:");
				staticURLStr.append("pyActivity=ReloadSection&StreamClass=Rule-HTML-Section");
				staticURLStr.append("&PreActivity=pzSetConfirmationMessageVisibility");
				tools.appendString("[\"refresh\", [\"otherSection\",\"pyRuleFormHeaderMessagesInner\", \"pzSetConfirmationMessageVisibility\", \"");
				refreshActionRequestD = refreshActionRequest.createDispatchedActionRequest();
				preActParamStr.append((!"".equals(preActParamStr.toString())?",":"") + "\"PropertyToSet\":\"pzDisplayMarkedRelevantCnfMsg\"");
				refreshActionRequestD.registerFixedParameter("PropertyToSet","pzDisplayMarkedRelevantCnfMsg");
				preActParamStr.append((!"".equals(preActParamStr.toString())?",":"") + "\"ValueToSet\":\"false\"");
				refreshActionRequestD.registerFixedParameter("ValueToSet","false");
				tools.appendString("\", \"\", \",");
				refreshActionRequestD = refreshActionRequest.createDispatchedActionRequest();
...
try {
	...
	cellPage.put("pyStreamName","pzRuleMarkedAsRelevantNotification");
	...
	cellPage.put("automationId"," " + pzCell.getTestIdIfEnabled("20180615130351036164823") + " ");
	...
	ctrlComponent.endComponent();
		 ...
		 String buttonUID = "name='pzRuleMarkedAsRelevantNotification_"+ referenceString+ "_9'";
		 String pyFormatValue = "";
		String hrefValue = "";
		String proprefAttr = "";
		...
		String classProp = "";
		 classProp ="pi pi-times";
		tools.appendString("<i  " + pzCell.getTestIdIfEnabled("20180615130351036164823") + "  href='' onclick='pd(event);' data-ctl='Icon' "); tools.appendString(" class='");
...
public int simpleLayoutCell_5( int index) {
	...
	tools.appendString("' " + pega.getUIEngine().getFVInspectorUtils().startCollectingFieldValueData() + pzCell.getInspectorDataDynamic("pxIcon",".pyTemplateGeneric",".pySections(1).pySectionBody(1).pyTable.pyRows(1).pyCells(2).pySections(1).pySectionBody(1).pyTable.pyRows(1).pyCells(1)") + " class='content-item content-field item-" + Integer.toString(index) + "   " +  pzSection.getCustomStyle(false, "","dataValueRead","dataValueWrite")  + " flex flex-row '  STRING_TYPE='field' RESERVE_SPACE='false'>");
...
public int simpleLayoutTemplateCell_5( int index) {
	...
	pgCells.put("automationId",new StringBuilder(" ").append(pzCell.getTestIdIfEnabled("20180615130351036164823-Label")).append(" ").toString().replace("data-test-id=","").trim());
...
pzSection.getLayout().setExpandParam("SubSectionpzRuleMarkedAsRelevantNotificationBBBB","",false);
}
...
public void simpleLayout_3() {
	...
	pg_dlmeta.put("automationId"," " + pzCell.getTestIdIfEnabled("201806151303390273606") + " ");
	...
	String paramName = "EXPANDEDSubSectionpzRuleMarkedAsRelevantNotificationBBBB";
...
StringBuilder actionsStringBuilder = tools.pushStreamBody();
	((PegaStreamAPI)tools).disableDirectStreaming();
		...
		tools.appendFormatted("Application: Inventory",StreamBuilder.FMT_NORMAL);	tools.appendString("\",\"Display\",{\"harnessName\":\"pzLPApplicationInventory\",\"className\":\"Pega-Landing-Application\",\"model\":\"pzSetRevRecordClassValid\",\"page\":\"\",\"readOnly\":\"false\",\"contentID\":\"");

		tools.appendFormatted("8ab7ea24-485e-4bf9-a0db-ebbed5cfa814",StreamBuilder.FMT_NORMAL);	tools.appendString("\",\"dynamicContainerID\":\"");

		tools.appendFormatted("",StreamBuilder.FMT_NORMAL);	tools.appendString("\",\"customParam\":{\"LPContextClass\":\"");

		tools.appendString("#~"+pzAuto.getUIComponentRuntime().generateActionToken(".pyClassName")+"~#");	tools.appendString("\"}},{\"levelA\":\"");
		...
		tools.appendFormatted("RelevantRecords",StreamBuilder.FMT_NORMAL);	tools.appendString("\"}]");
...
try {
	...
	cellPage.put("automationId",pzCell.getTestIdIfEnabled("2018060407484605107902",true));
	if(false || (pzAuto.isOffline() && false) ){ 
	modePage2.put("pyNavigation",navPage);
	}
	modePage2.put("pyName","pzRuleMarkedAsRelevantNotification_$CTX$_6");
		String repeatIndx = "";
...
boolean bDisabled = false;boolean bOptimizedMarkup = pzAuto.getUIComponentRuntime().isHarnessTemplatized() && pzAuto.isOffline();
		...
		String hrefValue = "#";
		 ...
		 boolean isParameterizedDeclarePage =  pega.getDeclarativePageUtils().isParameterizedDPName(referenceString);
		 if(isParameterizedDeclarePage){ referenceString = tools.getStepPage().getString("pzPageNameBase"); }
		 String buttonUID = "name='pzRuleMarkedAsRelevantNotification_"+ referenceString+ "_6'";
	 String securedPropValue = null;
		tools.appendString("<a  " + pzCell.getTestIdIfEnabled("2018060407484605107902") + " href='"+hrefValue+"'");tools.appendString("onclick='pd(event);'  data-ctl='Link' ");if(!"true".equals(tools.getParamValue("pxAvoidUniqueAttribute")))tools.appendString(buttonUID);if(bOptimizedMarkup){tools.appendString(" data-bindprops='innerHTML,title' ");}
		...
		tools.appendFormatted("94908c04-c12a-43aa-bfcb-48b24ff5baf8",StreamBuilder.FMT_NORMAL);	tools.appendString("\",\"dynamicContainerID\":\"");
...
public int simpleLayoutCell_1( int index) {
	...
	String baseRef ="";
	generateClientWhenDiv_1();
	tools.appendString("' " + pega.getUIEngine().getFVInspectorUtils().startCollectingFieldValueData() + pzCell.getInspectorDataDynamic("Label",null,".pySections(1).pySectionBody(1).pyTable.pyRows(1).pyCells(1).pySections(1).pySectionBody(1).pyTable.pyRows(1).pyCells(1).pySections(1).pySectionBody(1).pyTable.pyRows(1).pyCells(1)") + " class='content-item content-label item-" + Integer.toString(index) + "   ");
	 
	tools.appendString(pzSection.getCustomStyle(false, "","dataLabelRead","dataLabelWrite") + " ");
	 
	tools.appendString("flex flex-row ' " +   pzCell.getTestIdIfEnabled("201806040748460509688")   + " STRING_TYPE='label' RESERVE_SPACE='false'>");
 ...
 isSecCellReadOnly = !pzAuto.getSection().getControl().isControlEditable(false, null);
	...
	pzAuto.getUIComponentRuntime().getRuntimeContextTree().addChangeTrackProperty(".pyClassName");
	try{
	...
	com.pega.pegarules.priv.runtime.IStreamChangeTracker ct_fieldValParam = pzAuto.getChangeTracker(tools.getParamValue("AJAXTrackID"));
		ct_fieldValParam.trackValueChanges(tools.getProperty(".pyClassName"), -1);
	}}catch(Exception e){}
	}
	if(pzAuto.getUIComponentRuntime().getTemplatingUIStatus() !='N' ){
	labelName = "pzRuleMarkedAsRelevantCnfMsg"+"\t"+"^~"+pzAuto.getUIComponentRuntime().generateActionToken(".pyClassName")+"~^";
	pzAuto.getUIComponentRuntime().getRuntimeContextTree().addLocalizedValue(labelName, "pyCaption");
	}else{
	tools.appendString(tools.getLocalizedTextForString(".pyCaption","pzRuleMarkedAsRelevantCnfMsg"+"\t"+tools.getProperty(".pyClassName").getStringValue()));
	}
	tools.appendString(pega.getUIEngine().getFVInspectorUtils().stopCollectingFieldValueData() + "</div>");
	index++;
	return index;
...
public int simpleLayoutTemplateCell_1( int index) {
	...
	pgCells.put("partialClass","flex flex-row");
	pgCells.put("automationId",new StringBuilder(" ").append(pzCell.getTestIdIfEnabled("201806040748460509688")).append(" ").toString().replace("data-test-id=","").trim());
	...
	boolean isOffline = false;
	cc_dlCell.beginComponent("DynamicLayoutCell", pg_dlCellMeta);
	String inspectorData=pzCell.getInspectorDataDynamic("Label",null,".pySections(1).pySectionBody(1).pyTable.pyRows(1).pyCells(1).pySections(1).pySectionBody(1).pyTable.pyRows(1).pyCells(1).pySections(1).pySectionBody(1).pyTable.pyRows(1).pyCells(1)");
	if(!inspectorData.isEmpty()) {
	pgCells.put("liveUI",inspectorData);
	}
	if(pzAuto.getUIComponentRuntime().getTemplatingUIStatus() !='N' ){
...
public int simpleLayoutCell_2( int index) {
	...
	String ariahiddenString = "";
	String baseRef ="";
	if(!pzAuto.handleEvaluateWhen(" wxpzIsRuleAvailable","pzIsRuleAvailable","cell", "visible" )) {
		pzAuto.getUIComponentRuntime().insertComponentPlaceholderPage();
		return index;
	}
	generateClientWhenDiv_1();
	tools.appendString("' " + pega.getUIEngine().getFVInspectorUtils().startCollectingFieldValueData() + pzCell.getInspectorDataDynamic("pxLink",".pyTemplateInputBox",".pySections(1).pySectionBody(1).pyTable.pyRows(1).pyCells(1).pySections(1).pySectionBody(1).pyTable.pyRows(1).pyCells(1).pySections(1).pySectionBody(1).pyTable.pyRows(1).pyCells(2)") + " class='content-item content-field item-" + Integer.toString(index) + "   " +  pzSection.getCustomStyle(false, "","dataValueRead","dataValueWrite")  + " flex flex-row '  STRING_TYPE='field' RESERVE_SPACE='false'>");
	pzAuto.getUIComponentRuntime().insertComponentPlaceholderPage();
...
public int simpleLayoutTemplateCell_2( int index) {
	...
	IUIComponentMetadata pgCells = pzAuto.getUIComponentRuntime().createMetadata();
	pgCells.put("partialClass","flex flex-row");
	pgCells.put("automationId",new StringBuilder(" ").append(pzCell.getTestIdIfEnabled("2018060407484605107902-Label")).append(" ").toString().replace("data-test-id=","").trim());
	...
	pzAuto.getUIComponentRuntime().getRuntimeContextTree().addWhen(" wxpzIsRuleAvailable",pxUniqueStreamHash+"_10");
	pxWhenIdentifiersForCell.put("pyVisibilityWhenId",pxUniqueStreamHash+"_10");
	pxWhenIdentifiersForCell.put("pyVisibilityWhenName","pzIsRuleAvailable");
	...
	pyDLCellVisibility = pzAuto.getUIComponentRuntime().getRuntimeContextTree().beginVisibleWhen(" wxpzIsRuleAvailable", pxUniqueStreamHash+"_10"); 
	}
	if(pyDLCellVisibility) {
	String inspectorData=pzCell.getInspectorDataDynamic("pxLink",".pyTemplateInputBox",".pySections(1).pySectionBody(1).pyTable.pyRows(1).pyCells(1).pySections(1).pySectionBody(1).pyTable.pyRows(1).pyCells(1).pySections(1).pySectionBody(1).pyTable.pyRows(1).pyCells(2)");
	if(!inspectorData.isEmpty()) {
	pgCells.put("liveUI",inspectorData);
	}
	pxLink_1();}
...
pzSection.getLayout().setExpandParam("SubSectionpzRuleMarkedAsRelevantNotificationBBB","",false);
}
...
public void simpleLayout_1() {
	...
	cc_dl = pzAuto.getUIComponent();
	IUIComponentMetadata pg_dlmeta = pzAuto.getUIComponentRuntime().createMetadata();
	pg_dlmeta.put("automationId"," " + pzCell.getTestIdIfEnabled("201806040748460507722") + " ");
	String subscriptionID = null;
	String ackEnabled = null;
	if(currentTemplatingStatus != 'Y'){
		if(bIsIe && "true".equals(bIsGridLayout)&&!"true".equals(bIsWrapperAdded)){
		...
		tools.appendString("" + pzAuto.getDataUIMetaAttribute("{'type':'Layout','subType':'DYNAMICLAYOUT','pgRef':'.pySections(1).pySectionBody(1).pyTable.pyRows(1).pyCells(1).pySections(1).pySectionBody(1).pyTable.pyRows(1).pyCells(1).pySections(1)'}") + " class='" + "flex  content  layout-content-inline_middle content-inline_middle centered" + "'  ");
		if(!tools.getParamValue("pyPegaDesignMode").equals("true")){
		tools.appendString(" ");
		}
		tools.appendString(" " + refreshAttributes + ">");
	...
	String paramName = "EXPANDEDSubSectionpzRuleMarkedAsRelevantNotificationBBB";
...
pzSection.getLayout().setExpandParam("SubSectionpzRuleMarkedAsRelevantNotificationBB","",false);
}
...
public void simpleLayout_2() {
	...
	cc_dl = pzAuto.getUIComponent();
	IUIComponentMetadata pg_dlmeta = pzAuto.getUIComponentRuntime().createMetadata();
	pg_dlmeta.put("automationId"," " + pzCell.getTestIdIfEnabled("201806151303220001689") + " ");
	String subscriptionID = null;
	String ackEnabled = null;
	if(currentTemplatingStatus != 'Y'){
		if(bIsIe && "true".equals(bIsGridLayout)&&!"true".equals(bIsWrapperAdded)){
	...
	String paramName = "EXPANDEDSubSectionpzRuleMarkedAsRelevantNotificationBB";
...
public int simpleLayoutCell_4( int index) {
	...
	String baseRef ="";
	generateClientWhenDiv_1();
	tools.appendString("'  class='content-item content-layout column-1   '  STRING_TYPE='layout' RESERVE_SPACE='false'>");
	pzLayout_2();
 tools.putSaveValue("ContainerID", "");tools.appendString("</div>");
	index++;
	return index;
...
public int simpleLayoutTemplateCell_4( int index) {
	...
	pgCells.put("pyType","layout");
	pgCells.put("RWActive","false");
	pgCells.put("partialClass","column-1");
	pgCells.put("spanClass","iconRequired standard_iconRequired");
...
public int simpleLayoutCell_6( int index) {
	...
	String baseRef ="";
	generateClientWhenDiv_1();
	tools.appendString("'  class='content-item content-layout column-2   '  STRING_TYPE='layout' RESERVE_SPACE='false'>");
	pzLayout_3();
 tools.putSaveValue("ContainerID", "");tools.appendString("</div>");
	index++;
	return index;
...
public int simpleLayoutTemplateCell_6( int index) {
	...
	pgCells.put("pyType","layout");
	pgCells.put("RWActive","false");
	pgCells.put("partialClass","column-2");
	pgCells.put("spanClass","iconRequired standard_iconRequired");
	pgCells.put("isCL","true");
	pgCells.put("cstmreqfor",pyCustomRequireFormat);
	if(!StringUtils.isBlank(expressionId)) 
...
pzSection.getLayout().setExpandParam("SubSectionpzRuleMarkedAsRelevantNotificationB","",false);
}
...
public void simpleLayout_4() {
	...
	cc_dl = pzAuto.getUIComponent();
	IUIComponentMetadata pg_dlmeta = pzAuto.getUIComponentRuntime().createMetadata();
	pg_dlmeta.put("automationId"," " + pzCell.getTestIdIfEnabled("201806151302270104529") + " ");
	String subscriptionID = null;
	String ackEnabled = null;
	if(currentTemplatingStatus != 'Y'){
		if(bIsIe && "true".equals(bIsGridLayout)&&!"true".equals(bIsWrapperAdded)){
		...
		tools.appendString("" + pzAuto.getDataUIMetaAttribute("{'type':'Layout','subType':'DYNAMICCOLUMN','pgRef':'.pySections(1)'}") + " class='" + "flex  content  two-col-main-sidebar two-col-main-sidebar_field_and_icon " + hideColumn + "' data-hidecolumns=right ");
		if(!tools.getParamValue("pyPegaDesignMode").equals("true")){
		tools.appendString(" ");
		}
		tools.appendString(" " + refreshAttributes + ">");
		...
		String inspectorData= new StringBuilder("\"").append("" + pzAuto.getDataUIMetaAttribute("{'type':'Layout','subType':'DYNAMICCOLUMN','pgRef':'.pySections(1)'}") + "").append("\"").toString();
		if(!inspectorData.equals("\"\"")) {
		pg_dlmeta.put("liveUI",inspectorData);
		}
		pg_dlmeta.put("clear","false");
	...
	String paramName = "EXPANDEDSubSectionpzRuleMarkedAsRelevantNotificationB";
	metadataPage.put("paramName", paramName);
	String inspectorLayoutData = pzAuto.getDataUIMetaAttribute("{'type':'Layout','subType':'DYNAMICCOLUMN','pgRef':'.pySections(1)'}");
	if(!inspectorLayoutData.isEmpty()) {
	metadataPage.put("liveUI", inspectorLayoutData);
	}
	String pyPrefix = null;
...
static {
	...
	oStreamProperties_1.put("pzInsKey", "RULE-HTML-SECTION @BASECLASS PZRULEMARKEDASRELEVANTNOTIFICATION #20180713T140806.340 GMT");
	...
	oStreamProperties_1.put("pyStreamName", "pzRuleMarkedAsRelevantNotification");
...
private static final com.pega.pegarules.priv.tracer.RuleTraceInfo oTraceInfo_0 = new com.pega.pegarules.priv.tracer.RuleTraceInfo("RULE-HTML-SECTION @BASECLASS PZRULEMARKEDASRELEVANTNOTIFICATION #20180713T140806.340 GMT", "@baseclass pzRuleMarkedAsRelevantNotification", "Pega-ProcessArchitect", "08-01-01", "20180713T140806.340 GMT");
}
