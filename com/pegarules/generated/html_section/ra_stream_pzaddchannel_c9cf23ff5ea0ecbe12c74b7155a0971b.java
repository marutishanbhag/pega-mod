...
/**
 * Builds JSP stream EMBED-CASEPERSONA!PZADDCHANNEL.
 */
public class ra_stream_pzaddchannel_c9cf23ff5ea0ecbe12c74b7155a0971b extends StreamBuilderSection implements StreamBuilder, FUASupport, FUAReusable, WebAllowable {
	private static final LogHelper  oLog = new LogHelper("Rule_HTML_Section.pzAddChannel.Embed_CasePersona.Stream");
private static Map<String,String[]> oCellRuntimeParamsMap = new HashMap<String,String[]>();
private static Map<String, String> oControlPathsMap = new HashMap<String, String>();
private static Map<String, AssemblyParameters> oAssemblyParamsMap = null;
private static final int pzChecksum = 1366146765;
 ...
 */
	public ra_stream_pzaddchannel_c9cf23ff5ea0ecbe12c74b7155a0971b(com.pega.pegarules.priv.PegaAPI aContext) {
...
private static String pxUniqueStreamHash = "b6f29f8181d4d5017ef31db332874e3384d4a5d8";
...
{
/* Instance RULE-HTML-SECTION EMBED-CASEPERSONA PZADDCHANNEL #20200518T083716.510 GMT	Pega-AppDefinition:08-05-01	<no Circumstance>	<no Circumstance Date>	<no date range>	available=Final	baserule=false */
pxUniqueStreamHash = "b6f29f8181d4d5017ef31db332874e3384d4a5d8";
pzAuto.getUIComponentRuntime().getRuntimeContextTree().pushSection("pzAddChannel",pxUniqueStreamHash);pxIsNonTemplateStream = hasNonTemplateMethod_1();
...
/*
 * Begin RULE-HTML-SECTION EMBED-CASEPERSONA PZADDCHANNEL #20200518T083716.510 GMT
 * Set up context.
 */
final String prevRuleKey_1 = pz_CurrentRuleKey;
pz_CurrentRuleKey = "RULE-HTML-SECTION EMBED-CASEPERSONA PZADDCHANNEL #20200518T083716.510 GMT";final String prevClass_1 = mStreamResponse.getJspBaseClass();
...
/* end RULE-HTML-SECTION EMBED-CASEPERSONA PZADDCHANNEL #20200518T083716.510 GMT */
}
	} catch (Throwable ex) {
		handleError(ex);
	} finally {
...
private static final String oDependencies [] = {
	...
	"Rule-HTML-Section:PZADDCHANNEL", 
	...
	private static final DependentRuleInfo[] oDependentRuleInfos = {
		...
		new DependentRuleInfo("PZADDCHANNEL","Rule-HTML-Section","EMBED-CASEPERSONA",false,"","Pega-AppDefinition","08-05-01","RULE-HTML-SECTION EMBED-CASEPERSONA PZADDCHANNEL #20200518T083716.510 GMT","!PZADDCHANNEL",true,false,"ABSOLUTE_ULTIMATE_STEP_PAGE",1366146765), 
...
//	RULE-HTML-SECTION EMBED-CASEPERSONA PZADDCHANNEL #20200518T083716.510 GMT:20200612T143957.242 GMT
...
//	RULE-UTILITY-FUNCTION STRING STRIPSPECIALCHARS #20180713T131442.797 GMT:20180713T131442.797 GMT
	...
	public static String getFUAContentHash() {
		return "79e0120c33cc6f3bcbd72df0a7360ded";
	}
...
tools.putSaveValue("sectionUniqueID", ((new StringBuilder("SID")).append((((com.pega.pegarules.priv.context.PegaRequestor) tools.getRequestor()).currentTimeUnique()))).toString());
 if(pzAuto.isForOfflineTemplate()) {
  ...
  pzSection.beginSection("pzAddChannel",1,"Rule-HTML-Section","{'type':'Section','ruleName':'pzAddChannel','insKey':'RULE-HTML-SECTION EMBED-CASEPERSONA PZADDCHANNEL #20200518T083716.510 GMT','sectionType':'standard'}");
pzLayout_1();
...
config.put("pzActivity", "pzSetCurrentAndStageContext");
pega.getUIEngine().getUIAction("runActivity", config).register();
config = new HashMap<String, String>();
config.put("navName", "pzAppViewTypes");
pega.getUIEngine().getUIAction("showMenu", config).register();
config = new HashMap<String, String>();
config.put("pyFunctionName", "handleNavigationOfStepPalette");
...
StringBuilder actionsStringBuilder = tools.pushStreamBody();
	((PegaStreamAPI)tools).disableDirectStreaming();
		...
		ractURLStr.append("pyActivity=pzRunActionWrapper&pzActivity=pzSetCurrentAndStageContext");
		...
		tools.appendString("]");isBehaviorAdded = true;if(isBehaviorAdded && true){ tools.appendString(","); }			if(pega_uiengine_pznavigation.pzUseNewMenu() && true) {
			...
			if(pega_uiengine_offlinesupport.pzIsForOfflineTemplate()){
				try{
					pzPackageRuntime.packageSection("pzAddChannel",
					...
					tools.appendString("[\"showMenu\",[{\"dataSource\":\"pzAppViewTypes\", \"isNavNLDeferLoaded\":\"false\", \"isNavTypeCustom\":\"false\", \"className\":\"" + tools.getStepPage().getClassName());
					...
					tools.appendString(", \"ellipsisAfter\":\"999\",\"usingPage\":\"" + pzAuto.getUIComponentRuntime().generateActionTokenForPage() + "\", \"useNewMenu\":\"" + pega_uiengine_pznavigation.pzUseNewMenu() + "\", \"isMobile\":\"" + pega_uiengine_pznavigation.pzIsMobile() + "\", \"navPageName\":\"pyNavigation1771838775143");
					if(pzAuto.isOffline() || false) {
					try{
					pega_uiengine_pzcontrol.pzGetMenu("pzAppViewTypes", "", "pyNavigation1771838775143","", false, false, false, false);
					}catch(Exception e){oLog.error(e);}
					}
					tools.appendString("\"},\":event\"]");

		tools.appendString("]");isBehaviorAdded = true;if(isBehaviorAdded && true){ tools.appendString(","); }					tools.appendString("[\"runScript\", [\"handleNavigationOfStepPalette()\"]");
...
try {
	...
	cellPage.put("automationId",pzCell.getTestIdIfEnabled("20191101072034059513926",true));
	if(false || (pzAuto.isOffline() && true) ){ 
	modePage2.put("pyNavigation",navPage);
	}
	modePage2.put("pyName","pzAddChannel_$CTX$_3");
		String repeatIndx = "";
	modePage2.put("styles","Standard_Ellipsis");
	String pyTooltip = "";
	if(pega.getPackageRuntime().canPackage()){
		tools.getLocalizedTextForString(".pyActionPrompt","Add channel\t"+repeatIndx,StreamBuilder.FMT_LITERAL);
	} else {
		pzAuto.getUIComponentRuntime().getRuntimeContextTree().addLocalizedValue("Add channel", ".pyActionPrompt");
	}
	pyTooltip = "Add channel";
	...
	String pyLabel = "";
	if(pega.getPackageRuntime().canPackage()){
		tools.getLocalizedTextForString(".pyButtonLabel","CHANNEL\t"+repeatIndx,StreamBuilder.FMT_LITERAL);
	} else {
		pzAuto.getUIComponentRuntime().getRuntimeContextTree().addLocalizedValue("CHANNEL", ".pyButtonLabel");
	}
	pyLabel = "CHANNEL";
...
boolean bDisabled = false;boolean bOptimizedMarkup = pzAuto.getUIComponentRuntime().isHarnessTemplatized() && pzAuto.isOffline();
		...
		String hrefValue = "#";
		 ...
		 String buttonUID = "name='pzAddChannel_"+ referenceString+ "_3'";
	 String securedPropValue = null;String localizedValForMenu = ""; String ariaLabel = "";try {localizedValForMenu = tools.getLocalizedTextForString(".pyCaption","menu");ariaLabel = StringUtils.filterRichText("CHANNEL");} catch(Exception e) {}
		tools.appendString("<a  " + pzCell.getTestIdIfEnabled("20191101072034059513926") + " role='link' aria-haspopup='true' href='"+hrefValue+"'");tools.appendString(" aria-label='"+localizedValForMenu+" "+ariaLabel+"' ");tools.appendString("onclick='pd(event);'  data-ctl='Link' ");if(!"true".equals(tools.getParamValue("pxAvoidUniqueAttribute")))tools.appendString(buttonUID);if(bOptimizedMarkup){tools.appendString(" data-bindprops='innerHTML,title' ");}
		...
		tools.appendString("]");isBehaviorAdded = true;if(isBehaviorAdded && true){ tools.appendString(","); }			if(pega_uiengine_pznavigation.pzUseNewMenu() && true) {
			...
			if(pega_uiengine_offlinesupport.pzIsForOfflineTemplate()){
				try{
					...
					tools.appendString(", \"ellipsisAfter\":\"999\",\"usingPage\":\"" + pzAuto.getUIComponentRuntime().generateActionTokenForPage() + "\", \"useNewMenu\":\"" + pega_uiengine_pznavigation.pzUseNewMenu() + "\", \"isMobile\":\"" + pega_uiengine_pznavigation.pzIsMobile() + "\", \"navPageName\":\"pyNavigation1771838775146");
					if(pzAuto.isOffline() || false) {
					try{
					pega_uiengine_pzcontrol.pzGetMenu("pzAppViewTypes", "", "pyNavigation1771838775146","", false, false, false, false);
					}catch(Exception e){oLog.error(e);}
					}
					tools.appendString("\"},\":event\"]");
		...
		tools.appendString("  title= '");
		 toolTipVal = tools.getLocalizedTextForString(".pyActionPrompt","Add channel\t"+repeatIndx ,StreamBuilder.FMT_LITERAL) + "' ";
		tools.appendString(toolTipVal);tools.appendString(" class='Standard_Ellipsis");tools.appendString("'");
		tools.appendString(" >");
		String classProp = "";
		 classProp ="pi pi-plus";
		tools.appendString("<i aria-hidden='true'  data-click='.' class='" + classProp + "'></i>");
		boolean hasLineBreaks = false;
		tools.appendString("");pega_uiengine_harness.pzGenAKey(tools.getLocalizedTextForString(".pyButtonLabel","CHANNEL",StreamBuilder.FMT_LITERAL)); checkForScriptTags_1(tools.getParamValue("pzULabel")); 
...
public int simpleLayoutCell_1( int index) {
	...
	tools.appendString("' " + pega.getUIEngine().getFVInspectorUtils().startCollectingFieldValueData() + pzCell.getInspectorDataDynamic("pxLink",".pyTemplateInputBox",".pySections(1).pySectionBody(1).pyTable.pyRows(1).pyCells(1)") + " class='content-item content-field item-" + Integer.toString(index) + " remove-left-spacing remove-right-spacing   " +  pzSection.getCustomStyle(false, "","dataValueRead","dataValueWrite")  + " flex flex-row '  STRING_TYPE='field' RESERVE_SPACE='false'>");
	pzAuto.getUIComponentRuntime().insertComponentPlaceholderPage();
...
public int simpleLayoutTemplateCell_1( int index) {
	...
	pgCells.put("automationId",new StringBuilder(" ").append(pzCell.getTestIdIfEnabled("20191101072034059513926-Label")).append(" ").toString().replace("data-test-id=","").trim());
	...
	pzAuto.getUIComponentRuntime().getRuntimeContextTree().addWhen(" o1! wxpzIsSelectedContextDisabled",pxUniqueStreamHash+"_6");
	...
	pyDLCellVisibility = pzAuto.getUIComponentRuntime().getRuntimeContextTree().beginVisibleWhen(" o1! wxpzIsSelectedContextDisabled", pxUniqueStreamHash+"_6"); 
	}
...
pzSection.getLayout().setExpandParam("SubSectionpzAddChannelB","",false);
}
...
public void simpleLayout_1() {
	...
	String paramName = "EXPANDEDSubSectionpzAddChannelB";
...
static {
	...
	oStreamProperties_1.put("pzInsKey", "RULE-HTML-SECTION EMBED-CASEPERSONA PZADDCHANNEL #20200518T083716.510 GMT");
	...
	oStreamProperties_1.put("pyStreamName", "pzAddChannel");
...
private static final com.pega.pegarules.priv.tracer.RuleTraceInfo oTraceInfo_0 = new com.pega.pegarules.priv.tracer.RuleTraceInfo("RULE-HTML-SECTION EMBED-CASEPERSONA PZADDCHANNEL #20200518T083716.510 GMT", "Embed-CasePersona pzAddChannel", "Pega-AppDefinition", "08-05-01", "20200612T143957.242 GMT");
}
