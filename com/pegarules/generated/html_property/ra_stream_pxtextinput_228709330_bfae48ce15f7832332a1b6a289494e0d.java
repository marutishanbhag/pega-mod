...
public class ra_stream_pxtextinput_228709330_bfae48ce15f7832332a1b6a289494e0d extends StreamBuilderSection implements ControlBuilder, FUASupport, FUAReusable, WebAllowable {
...
public ra_stream_pxtextinput_228709330_bfae48ce15f7832332a1b6a289494e0d(com.pega.pegarules.priv.PegaAPI aContext) {
...
controlRuntimeUtilities.markUpForReadOnlyFormatting(bOptimizedMarkup, ref, activeValue, tools, pzAuto);if(bOptimizedMarkup){propertyHandle = pzAuto.getUIComponentRuntime().changeTrackProperty(propertyHandle,nameType);}
		...
		tools.appendString(" data-keyup='[");if(isBehaviorAdded && true){ tools.appendString(","); }					{
						...
						keyMap.putString("pyActionName", "pzSearch");
						...
						try{
						pzPackageRuntime.packageLocalAction("pzSearch", offlineUsingPage , strLAClassName);
						...
						if(pega_uiengine_offlinesupport.pzIsForOfflineTemplate()){
							try{
								pzPackageRuntime.packageSection("pzStudioHeaderTools",
								...
								if("true".equals(bHasPrivilege )) {
									tools.appendString("[\"processAction\", [\"pzSearch\",\"overlay\",\":event\",\"\",\""+streamType+"\",\"pz-po-c-fluid\",\"\",\"%7B%22mobile%22%3A%7B%22reveal%22%3A%7B%22effect%22%3A%22anim-fade%22%7D%2C%22isCustomDismiss%22%3A%22false%22%7D%7D\",\"true\",\""+strLAClassName+"\",\""+strLAClassName+"\",\"false\",\"false\",\"\",\"\"]");
									} else {
		...
		tools.appendString(" data-keydown='[");if(isBehaviorAdded && true){ tools.appendString(","); }										{
											...
											if(pega_uiengine_offlinesupport.pzIsForOfflineTemplate()){
												try{
													...
													if(("").equals(usingPage)) { usingPage = tools.getStepPage().getReference(); }
													if("true".equals(bHasPrivilege )) {
														tools.appendString("[\"processAction\", [\"pzSearch\",\"overlay\",\":event\",\"\",\""+streamType+"\",\"pz-po-c-fluid\",\"\",\"%7B%22mobile%22%3A%7B%22reveal%22%3A%7B%22effect%22%3A%22anim-grow%22%7D%2C%22isCustomDismiss%22%3A%22false%22%7D%7D\",\"true\",\""+strLAClassName+"\",\""+strLAClassName+"\",\"false\",\"false\",\"\",\"\"]");
														} else {
															tools.appendString("[\"processAction\", [\"You are not authorized to perform this Flow or Local action\"]");
															}
															}
		...
		tools.appendString("]");isBehaviorAdded = true;if(isBehaviorAdded && true){ tools.appendString(","); }															tools.appendString("[\"setItemFocus\",[\"" + pzAuto.getUIComponentRuntime().generateActionToken(".pySearchText") + "\"]");

		tools.appendString(",");
		tools.appendString(",\"enter\"");
		tools.appendString("]");isBehaviorAdded = true;if(isBehaviorAdded && true){ tools.appendString(","); }															tools.appendString("[\"setValue\", [[\"" + pzAuto.getUIComponentRuntime().generateActionToken(".pySearchText") + "\", \"\", \"\", \"\", \"\"]]");
...
if(pegaValidation != null) {
	...
	tools.putActive(propActivePrev); 															} catch (Exception e) {
	 ...
	 if(pzAuto.getUIComponentRuntime().getTemplatingUIStatus() == 'Y'															&& pzAuto.getUIComponentRuntime().getRuntimeContextTree().isLiveDesignViewMode()){
		...
		IUIComponent ctrlComponent = pzAuto.getUIComponent();
		metadata.put("cellInfo", "Search text");
...
//	RULE-UTILITY-FUNCTION UIENGINE PZGETANIMATIONSJSONSTRING--(CLIPBOARDPAGE) #20190417T070159.744 GMT:20190430T131934.330 GMT
	...
	public static String getFUAContentHash() {
		return "31841e9c470c0a0e136765e5abce81e7";
	}
