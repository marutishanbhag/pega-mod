...
public class ra_validate_isfieldaproperty_4c5158f3afa491d36078916df650ec0b  extends com.pega.pegarules.priv.AbstractFUASupport implements EditValidate, FUASupport {
	private static final LogHelper  oLog = new LogHelper("Rule_Edit_Validate.IsFieldAProperty.Validate");
	public ra_validate_isfieldaproperty_4c5158f3afa491d36078916df650ec0b(PegaAPI aContext) {
		tools = aContext;
		pega = aContext;
	}
...
{
/* Instance RULE-EDIT-VALIDATE ISFIELDAPROPERTY #20180713T134052.682 GMT	Pega-Reporting:08-01-01	<no Circumstance>	<no Circumstance Date>	<no date range>	available=Final	baserule=false */
String strProperty = tools.getStepPage().getString(".pyFieldName");
...
{
	if(!tools.editValidate(tools.getStepPage().getProperty(".pyFieldName"),"pyValidateFunction"))
	{
		return false;
	}
	else
...
if (hasPrefix){
    ...
    ClipboardProperty pSubreportsFrom = null;
	...
	if(tools.getStepPage().getTopLevelPage().getIfPresent(".pzPageToDisplay") != null){
        ...
        pSubreportsFrom = tools.findPage(tools.getStepPage().getTopLevelPage().getString("pzPageToDisplay")).getProperty(".pyReportDefinition.pyUI.pySource.pySubReportFrom");
	}else{
        ...
        pSubreportsFrom =tools.getStepPage().getTopLevelPage().getProperty(".pyUI.pySource.pySubReportFrom");
	}
	...
	while (itr_SubReports.hasNext()) 
	{
		...
		if (strCurrentPrefix.equals(prefix) && pgSubReport.getBoolean(".pyUsageInfo(select)"))
		{
			if (strProperty.startsWith(".")) 
			...
			while (itr_Cols.hasNext())
			{
				...
				if (strAlias.equals(strProperty))
				{
					...
					if(fieldPrefix.indexOf(".") < 1){
						...
						if(fieldName.length() > 0 && !fieldName.startsWith("@@") && !fieldName.equals("*") && !pega_rules_utilities.isDBColumn(strCurrentClass,fieldName)) {
						...
						String[] linked = pega_rulesengine_reportingutils.pzParseLinkProperty(fieldName,pgTestMe ,tools);
	...
	//Validation for SubReport From Filed reference. 
    if (pSubreportsFrom!=null)
    {
      ClipboardPage pgSubReport =pSubreportsFrom.getPageValue();
      if (pgSubReport!=null)
      {
        String strCurrentPrefix = pgSubReport.getString(".pyPrefix");
        if (strCurrentPrefix!=null && strCurrentPrefix.equals(prefix))          
        {
              bFound = true;
              return bFound;
        }
		...
		if (strPyPrefix.equals(prefix))
		{
			...
			//propInfo = tools.getDictionary().getImmutablePropertyInfo(prefixClassName, strProperty);
  		    ...
  		    if (propInfo != null) {
				...
				charPosition = strProperty.indexOf("(");
				if (charPosition > -1) 
				    strProperty = strProperty.substring(0, charPosition);
	...
	isLinked = linked[0] != null && linked[0].equalsIgnoreCase("true");
	if(isLinked){
	          //this is a valid property, need to determine if its unexposed to put up correct
	          //warning message
	          if(!pega_rules_utilities.isDBColumn(linked[2],linked[4])) {
...
private static final String oDependencies [] = {
	"Rule-Edit-Validate:ISFIELDAPROPERTY"
	};
	...
	private static final DependentRuleInfo[] oDependentRuleInfos = {
		new DependentRuleInfo("ISFIELDAPROPERTY","Rule-Edit-Validate","",false,"","Pega-Reporting","08-01-01","RULE-EDIT-VALIDATE ISFIELDAPROPERTY #20180713T134052.682 GMT","ISFIELDAPROPERTY",true,false,"ABSOLUTE_CLASSLESS",117042065)
	};
...
//	RULE-EDIT-VALIDATE ISFIELDAPROPERTY #20180713T134052.682 GMT:20180713T134052.682 GMT
	...
	public static String getFUAContentHash() {
		return "4c2c545bb898506050de50c52a34d41d";
	}
...
private static final com.pega.pegarules.priv.tracer.RuleTraceInfo oTraceInfo_0 = new com.pega.pegarules.priv.tracer.RuleTraceInfo("RULE-EDIT-VALIDATE ISFIELDAPROPERTY #20180713T134052.682 GMT", "IsFieldAProperty", "Pega-Reporting", "08-01-01", "20180713T134052.682 GMT");
}
