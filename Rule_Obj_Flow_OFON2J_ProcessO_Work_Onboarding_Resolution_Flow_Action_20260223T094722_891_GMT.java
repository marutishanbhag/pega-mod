...
import java.util.*;
public class Rule_Obj_Flow_OFON2J_ProcessO_Work_Onboarding_Resolution_Flow_Action_20260223T094722_891_GMT  extends com.pega.pegarules.priv.AbstractFUASupport implements Activity, FUASupport {
	private static final LogHelper  oLog = new LogHelper("Rule_Obj_Flow.Resolution_Flow.OFON2J_ProcessO_Work_Onboarding.Action");
	public Rule_Obj_Flow_OFON2J_ProcessO_Work_Onboarding_Resolution_Flow_Action_20260223T094722_891_GMT(PegaAPI aContext) {
		tools = aContext;
		pega = aContext;
	}
...
public void perform() {
		...
		mRuleSet = "ProcessO";
		mFlowHandle = "RULE-OBJ-FLOW OFON2J-PROCESSO-WORK-ONBOARDING RESOLUTION_FLOW #20260223T094722.891 GMT";
		
		flowType = "Resolution_Flow";
		...
		if (GetFlowInfo != null && GetFlowInfo.equals("true"))
		{
			...
			tools.putParamValue("FlowInsName", "OFON2J-PROCESSO-WORK-ONBOARDING!RESOLUTION_FLOW");
			return;
		}
		...
		if (strCheckFlowSecurity != null && strCheckFlowSecurity.equals("true"))
		{
			...
			tools.putParamValue("pyLabel", "Resolution");
			return;
		}
		...
		if (pyLastFlowStep.length() == 0 && pyNextFlowStep.length() == 0)
		{
			...
			if (pyCurrentFlowStep.equals(""))
			{
				...
				currentFlow.putString(".pxFlowInsKey", mFlowHandle);
				currentFlow.putString(".pyFlowTypeLabel", "Resolution");
...
// Expression: 
		...
		if (!strIssuedFromStage.equals("")){
			...
			ParamsPg.putString("CurrentProcess", "Resolution_Flow");
...
return "Action";
	...
	private void Task_Assignment1_circum0()
	{
		...
		currentFlow.putString(".pyCurrentFlowStepLabel", "Resolve case");
...
// Expression: Resolved-Completed
		try
		{
		newParamPage.putString("StatusWork", "Resolved-Completed");
		}
		...
		else
		{
			...
			if (passThrough.equals("true"))
			{
				...
				{
					actionToTake = "pyResolveAndCloseCase";
				}
		...
		newParamsPage.putString("TaskLabel","Resolve case");
...
// Expression: pyStepRoutedConfirmation
		...
		pg_newAssignPage.putString("pxProcessName", "Resolution");
		...
		if (currentFlow != null)
		{
			...
			currentFlow.putString(".pyLastFlowStepLabel", "Resolve case");
	...
	{
		if (strAction.equals("pyResolveAndCloseCase"))
			return true;
		return false;
	}
	...
	private boolean TaskFlowActions_Assignment1_circum0(ClipboardPage assignPage)
	{
		...
		if (AddFlowAction("pyResolveAndCloseCase", "FlowAction", isFirstAction, false))
			isFirstAction = false;
				assignPage.getProperty(".pyAssignActions(<LAST>).pyToShape").setValue("END52" );
				assignPage.getProperty(".pyAssignActions(<LAST>).pyStepName").setValue("pyResolveAndCloseCase" );
				if(assignPage.getString(".pyAssignActions(<LAST>).pyActionLabel").equals("")){
					assignPage.getProperty(".pyAssignActions(<LAST>).pyActionLabel").setValue("pyResolveAndCloseCase" );
				}
	...
	private void TaskConnectors_Start1_circum0()
	{
		...
		{
			// Send the user to a special assignment to pick the connector
			DraftModeFlow("DraftModeStep", "Start1", "Assignment1", "Resolve case");
			return;
		}
	...
	private void GetOrderedFlowSteps_circum0(String stepPageName) {
		...
		pgStep.putString(".pyMOId", "Assignment1");
			pgStep.putString(".pyMOName", "Resolve case");
	...
	private void GetNavigationPath_circum0(String pageName, String parentFlowPath, boolean deferredLoad)
	{
		...
		pgNavPath.putString("pyFlowType", "Resolution_Flow");
		pgNavPath.putString("pyClassName", "OFON2J-ProcessO-Work-Onboarding");
		pgNavPath.putString("pyLabel", "Resolution");
		...
		pgStep.putString("pyStepLabel", "Resolve case");
		pgStep.putString("pyStepType", "ASSIGNMENT");
		bIsInFlowPath = pega_procom_breadcrumbtrail.isInFlowPath2(tools, "Assignment1", "Resolution_Flow", parentFlowPath, flowNameToSearchForTaskIn, interestPage.getTopLevelPage());
		if (bIsInFlowPath)
		{
			if (!bFoundFlowPath)
			{
			...
			if (newAssignPage.getString("pxTaskName").equals("Assignment1") && newAssignPage.getString("pyFlowType").equals("Resolution_Flow") && newAssignPage.getString("pyParentFlowPath").equals(parentFlowPath))
	...
	private boolean CheckFlowSecurity_circum0()
	{
		...
		if (!hasSecurity && tools.getAuthorizationHandle().havePrivilege(tools, "AM:RULE-OBJ-FLOW:OFON2J-PROCESSO-WORK-ONBOARDING!RESOLUTION_FLOW",primaryPageClassName, interestPage)) 
			 hasSecurity = true;
			 if(!hasSecurity) {
			 if(tools.getAuthorizationHandle().havePrivilege(tools,  "AM:RULE-OBJ-FLOW:OFON2J-PROCESSO-WORK-ONBOARDING!RESOLUTION_FLOW",primaryPageClassName, interestPage,true))
			 isAllowedAccess = true;
			 else return false;
		}
			 if(isAllowedAccess && !hasSecurity) {		com.pega.pegarules.priv.tracer.RuleTraceInfo ruleTraceInfo = new com.pega.pegarules.priv.tracer.RuleTraceInfo                                                                   ("RULE-OBJ-FLOW OFON2J-PROCESSO-WORK-ONBOARDING RESOLUTION_FLOW #20260223T094722.891 GMT","Resolution_Flow","ProcessO","","20260223T095001.360 GMT");
