 ...
 * Builds JSP stream .
 */
public class Rule_HTML_Section_LTM_Onboarding_UIPages_OnboardingHeader_Stream_20260223T083643_804_GMT extends StreamBuilderSection implements StreamBuilder, FUASupport, FUAReusable, WebAllowable {
	private static final LogHelper  oLog = new LogHelper("Rule_HTML_Section.OnboardingHeader.LTM_Onboarding_UIPages.Stream");
private static Map<String,String[]> oCellRuntimeParamsMap = new HashMap<String,String[]>();
private static Map<String, String> oControlPathsMap = new HashMap<String, String>();
private static Map<String, AssemblyParameters> oAssemblyParamsMap = null;
private static final int pzChecksum = 1562377193;
 ...
 */
	public Rule_HTML_Section_LTM_Onboarding_UIPages_OnboardingHeader_Stream_20260223T083643_804_GMT(com.pega.pegarules.priv.PegaAPI aContext) {
...
private static String pxUniqueStreamHash = "ee3af1449ee6ca83b97b65ba4b801830392ca97f";
...
try {
pxUniqueStreamHash = "ee3af1449ee6ca83b97b65ba4b801830392ca97f";
pzAuto.getUIComponentRuntime().getRuntimeContextTree().pushSection("OnboardingHeader",pxUniqueStreamHash);pxIsNonTemplateStream = hasNonTemplateMethod_1();
...
//	RULE-UTILITY-FUNCTION UTILITIES VALIDATERULEEXISTENCE2 #20180713T131446.070 GMT:20180713T131446.070 GMT
	...
	public static String getFUAContentHash() {
		return "5b10fa2bbd6db6cc02426ad8211d4989";
	}
...
tools.putSaveValue("sectionUniqueID", ((new StringBuilder("SID")).append((((com.pega.pegarules.priv.context.PegaRequestor) tools.getRequestor()).currentTimeUnique()))).toString());
 if(pzAuto.isForOfflineTemplate()) {
  ...
  pzSection.beginSection("OnboardingHeader",1,"Rule-HTML-Section","{'type':'Section','ruleName':'OnboardingHeader','insKey':'null','sectionType':'standard'}");
pzLayout_1();
pzLayout_2();
pzLayout_3();
...
pega.getUIEngine().getUIAction("refresh", null).register();
config = new HashMap<String, String>();
config.put("className", "LTM-Onboarding-Work-Onboarding");
...
pzSection.getLayout().setExpandParam("SubSectionOnboardingHeaderBBB","",false);
}
...
String paramName = "EXPANDEDSubSectionOnboardingHeaderBBB";
metadataPage.put("paramName", paramName);
String inspectorLayoutData = pzAuto.getDataUIMetaAttribute("{'type':'Layout','subType':'SECTIONINCLUDE','pgRef':'HeaderTemplate.pySections(3)','clipboardPath':'pxMainPageSideInContainer'}");
...
if(bWrapIncSection){
tools.appendString("<span class='inspector-span' " + pzAuto.getDataUIMetaAttribute("{'type':'Layout','subType':'SECTIONINCLUDE','pgRef':'HeaderTemplate.pySections(3)','clipboardPath':'pxMainPageSideInContainer'}") + ">");
}
pzSetExpandParam_3();
...
StringBuilder actionsStringBuilder = tools.pushStreamBody();
	((PegaStreamAPI)tools).disableDirectStreaming();
		...
		tools.appendFormatted("LTM-Onboarding-Work-Onboarding",StreamBuilder.FMT_NORMAL);		tools.appendString("\",\"\",\"");
...
try {
	...
	modePage2.put("pyName","OnboardingHeader_$CTX$_5");
		String repeatIndx = "";
	String pyTooltip = "";
...
boolean bDisabled = false;boolean bOptimizedMarkup = pzAuto.getUIComponentRuntime().isHarnessTemplatized() && pzAuto.isOffline();
		...
		String hrefValue = "#";
		 ...
		 String buttonUID = "name='OnboardingHeader_"+ referenceString+ "_5'";
...
public int simpleLayoutCell_3( int index) {
	...
	tools.appendString("' " + pega.getUIEngine().getFVInspectorUtils().startCollectingFieldValueData() + pzCell.getInspectorDataDynamic("Label",null,"HeaderTemplate.pySections(2).pySectionBody(1).pyTable.pyRows(1).pyCells(1)") + " class='content-item content-label item-" + Integer.toString(index) + " remove-top-spacing remove-bottom-spacing remove-left-spacing   ");
 ...
 isSecCellReadOnly = !pzAuto.getSection().getControl().isControlEditable(false, null);
	tools.appendString(tools.getLocalizedTextForString("pyCaption","Onboarding Summary") + pega.getUIEngine().getFVInspectorUtils().stopCollectingFieldValueData() + "</div>");
	index++;
	return index;
}
...
public int simpleLayoutTemplateCell_3( int index) {
	...
	String inspectorData=pzCell.getInspectorDataDynamic("Label",null,"HeaderTemplate.pySections(2).pySectionBody(1).pyTable.pyRows(1).pyCells(1)");
	if(!inspectorData.isEmpty()) {
	pgCells.put("liveUI",inspectorData);
	}
	labelName = "Onboarding Summary";
...
public int simpleLayoutTemplateCell_4( int index) {
	...
	boolean isOffline = false;
	cc_dlCell.beginComponent("DynamicLayoutCell", pg_dlCellMeta);
	String inspectorData=pzCell.getInspectorDataDynamic("pxButton",".pyTemplateInputBox","HeaderTemplate.pySections(2).pySectionBody(1).pyTable.pyRows(1).pyCells(2)");
	if(!inspectorData.isEmpty()) {
	pgCells.put("liveUI",inspectorData);
	}
...
pzSection.getLayout().setExpandParam("SubSectionOnboardingHeaderBB","",false);
}
...
public void simpleLayout_2() {
	...
	if(currentTemplatingStatus != 'Y'){
		...
		tools.appendString("" + pzAuto.getDataUIMetaAttribute("{'type':'Layout','subType':'DYNAMICLAYOUT','pgRef':'HeaderTemplate.pySections(2)'}") + " class='" + "flex  content  layout-content-inline_middle content-inline_middle padding-lr-2x padding-t-2x padding-b-1x" + "'  " + "role='heading' tabindex='-1' ");
		...
		String inspectorData= new StringBuilder("\"").append("" + pzAuto.getDataUIMetaAttribute("{'type':'Layout','subType':'DYNAMICLAYOUT','pgRef':'HeaderTemplate.pySections(2)'}") + "").append("\"").toString();
	...
	String paramName = "EXPANDEDSubSectionOnboardingHeaderBB";
	String inspectorLayoutData = pzAuto.getDataUIMetaAttribute("{'type':'Layout','subType':'DYNAMICLAYOUT','pgRef':'HeaderTemplate.pySections(2)'}");
	if(!inspectorLayoutData.isEmpty()) {
	metadataPage.put("liveUI", inspectorLayoutData);
	}
	String pyPrefix = null;
...
StringBuilder actionsStringBuilder = tools.pushStreamBody();
	((PegaStreamAPI)tools).disableDirectStreaming();
		...
		{boolean isBehaviorAdded = false;isBehaviorAdded = false;
		tools.appendString(" data-click='[");if(isBehaviorAdded && true){ tools.appendString(","); }	tools.appendString("[\"runScript\", [\"pega.desktop.infinity.createNewWorkInDialogContainer(\\\"LTM-Onboarding-Work-Onboarding\\\",\\\"pyStartCase\\\")\"]");
...
try {
	...
	modePage2.put("pyName","OnboardingHeader_$CTX$_2");
		String repeatIndx = "";
	String pyTooltip = "";
	modePage2.put("tooltip",pyTooltip);
	modePage2.put("helpertype","none");
...
boolean bDisabled = false;boolean bOptimizedMarkup = pzAuto.getUIComponentRuntime().isHarnessTemplatized() && pzAuto.isOffline();
		...
		String hrefValue = "#";
		 ...
		 boolean isParameterizedDeclarePage =  pega.getDeclarativePageUtils().isParameterizedDPName(referenceString);
		 if(isParameterizedDeclarePage){ referenceString = tools.getStepPage().getString("pzPageNameBase"); }
		 String buttonUID = "name='OnboardingHeader_"+ referenceString+ "_2'";
	 String securedPropValue = null;
		tools.appendString("<button " + pzCell.getTestIdIfEnabled("201511051022210843267245") + "  data-ctl='Button'  type='button' ");if(!"true".equals(tools.getParamValue("pxAvoidUniqueAttribute")))tools.appendString(buttonUID);
		pega_uiengine_pzcontrol.pzAddClientValidation(tools,"","","","");
		{boolean isBehaviorAdded = false;isBehaviorAdded = false;
...
public int simpleLayoutCell_1( int index) {
	...
	String baseRef ="";
	generateClientWhenDiv_1();
	tools.appendString("' " + pega.getUIEngine().getFVInspectorUtils().startCollectingFieldValueData() + pzCell.getInspectorDataDynamic("Label",null,"HeaderTemplate.pySections(1).pySectionBody(1).pyTable.pyRows(1).pyCells(1)") + " class='content-item content-label item-" + Integer.toString(index) + " remove-right-spacing   ");
...
public int simpleLayoutTemplateCell_1( int index) {
	...
	boolean isOffline = false;
	cc_dlCell.beginComponent("DynamicLayoutCell", pg_dlCellMeta);
	String inspectorData=pzCell.getInspectorDataDynamic("Label",null,"HeaderTemplate.pySections(1).pySectionBody(1).pyTable.pyRows(1).pyCells(1)");
	if(!inspectorData.isEmpty()) {
	pgCells.put("liveUI",inspectorData);
	}
	labelName = "Onboarding Summary";
...
public int simpleLayoutCell_2( int index) {
	...
	String baseRef ="";
	generateClientWhenDiv_1();
	tools.appendString("' " + pega.getUIEngine().getFVInspectorUtils().startCollectingFieldValueData() + pzCell.getInspectorDataDynamic("pxButton",".pyTemplateInputBox","HeaderTemplate.pySections(1).pySectionBody(1).pyTable.pyRows(1).pyCells(2)") + " class='content-item content-field item-" + Integer.toString(index) + " remove-right-spacing    " + pzSection.getCustomStyle(false, "","margin-l-auto","margin-l-auto")  + " " +  pzSection.getCustomStyle(false, "","dataValueRead","dataValueWrite")  + " flex flex-row '  STRING_TYPE='field' RESERVE_SPACE='false'>");
	pzAuto.getUIComponentRuntime().insertComponentPlaceholderPage();
...
public int simpleLayoutTemplateCell_2( int index) {
	...
	boolean isOffline = false;
	cc_dlCell.beginComponent("DynamicLayoutCell", pg_dlCellMeta);
	String inspectorData=pzCell.getInspectorDataDynamic("pxButton",".pyTemplateInputBox","HeaderTemplate.pySections(1).pySectionBody(1).pyTable.pyRows(1).pyCells(2)");
	if(!inspectorData.isEmpty()) {
	pgCells.put("liveUI",inspectorData);
	}
...
pzSection.getLayout().setExpandParam("SubSectionOnboardingHeaderB","",false);
}
...
public void simpleLayout_1() {
	...
	if(currentTemplatingStatus != 'Y'){
		...
		tools.appendString("" + pzAuto.getDataUIMetaAttribute("{'type':'Layout','subType':'DYNAMICLAYOUT','pgRef':'HeaderTemplate.pySections(1)'}") + " class='" + "flex  content  layout-content-inline_middle content-inline_middle padding-lr-2x padding-t-2x padding-b-1x" + "'  " + "role='heading' tabindex='-1' ");
		if(!tools.getParamValue("pyPegaDesignMode").equals("true")){
		tools.appendString(" ");
		}
		tools.appendString(" " + refreshAttributes + ">");
		...
		pg_dlmeta.put("customClassName","padding-lr-2x padding-t-2x padding-b-1x");
		pg_dlmeta.put("format","inline_middle");
		String inspectorData= new StringBuilder("\"").append("" + pzAuto.getDataUIMetaAttribute("{'type':'Layout','subType':'DYNAMICLAYOUT','pgRef':'HeaderTemplate.pySections(1)'}") + "").append("\"").toString();
		if(!inspectorData.equals("\"\"")) {
		pg_dlmeta.put("liveUI",inspectorData);
		}
		pg_dlmeta.put("ariaRole","heading");
	...
	String paramName = "EXPANDEDSubSectionOnboardingHeaderB";
	String inspectorLayoutData = pzAuto.getDataUIMetaAttribute("{'type':'Layout','subType':'DYNAMICLAYOUT','pgRef':'HeaderTemplate.pySections(1)'}");
	if(!inspectorLayoutData.isEmpty()) {
	metadataPage.put("liveUI", inspectorLayoutData);
	}
	String pyPrefix = null;
...
static {
	...
	oStreamProperties_1.put("pyStreamName", "OnboardingHeader");
...
private static final com.pega.pegarules.priv.tracer.RuleTraceInfo oTraceInfo_0 = new com.pega.pegarules.priv.tracer.RuleTraceInfo("", "LTM-Onboarding-UIPages OnboardingHeader", "Onboarding", "01-01-01", "20210903T090257.610 GMT");
}
